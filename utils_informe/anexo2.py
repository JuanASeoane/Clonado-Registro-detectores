...
def generar_documento_anexo2(
    ...
    """Genera un .docx con una página horizontal por cada plano con detectores.

    ``mostrar_logo`` y ``mostrar_foto_exterior`` permiten liberar la columna
    lateral. Si ambos están desmarcados, el plano ocupa prácticamente todo el
    ancho útil de la página y, por tanto, se muestra a mayor tamaño.
    """
    planos = datos["planos"]
    ...
    hay_lateral = bool(mostrar_logo or mostrar_foto_exterior)
    # Con un solo elemento lateral basta una columna más estrecha. Si no se
    # quiere ni logo ni foto, todo el ancho útil queda disponible al plano.
    col_lateral_cm = 5.0 if hay_lateral else 0.0
    col_plano_cm = (ancho_util_cm - col_lateral_cm - 0.3) if hay_lateral else ancho_util_cm
    ...
    for i, clave in enumerate(nombres_con_puntos):
        ...
        if hay_lateral:
            ...
            table.alignment = WD_TABLE_ALIGNMENT.CENTER
            table.columns[0].width = Cm(col_plano_cm)
        ...
        if hay_lateral:
            ...
            if mostrar_logo:
                if logo_a_usar:
                    r = p_lat1.add_run()
                    w_logo, h_logo = _fit(logo_a_usar, col_lateral_cm - 0.5, 5.2)
            ...
            if mostrar_foto_exterior and foto_exterior:
                ...
                p_lat2.paragraph_format.space_before = Pt(14 if mostrar_logo else 0)
                r2 = p_lat2.add_run()
                w_foto, h_foto = _fit(foto_exterior, col_lateral_cm - 0.5, 8.5 if not mostrar_logo else 6.5)
            ...
            elif mostrar_foto_exterior and centro:
                # Si se pidió la foto pero no está disponible, conservamos el
                # nombre del centro como referencia.
                nombre_p = celda_lateral.add_paragraph()
                nombre_p.alignment = WD_ALIGN_PARAGRAPH.CENTER
                nombre_p.paragraph_format.space_before = Pt(14 if mostrar_logo else 0)
                nombre_run = nombre_p.add_run(centro)
                nombre_run.bold = True
                nombre_run.font.size = Pt(9.5)
