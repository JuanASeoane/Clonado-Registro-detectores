...
def _rectangulos_overlays(
    ...
    separacion = max(16, int(min(w, h) * 0.020))
    overlays = []
...
def componer_plano(
    ...
    font = _cargar_fuente(max(9, int(lado_menor * 0.0185)))
    separacion_texto = max(1, int(lado_menor * 0.0025))
    margen_etiqueta = max(2, int(lado_menor * 0.0025))
    ...
    def candidatos(px, py, bw, bh):
        ...
        for escala in (0.72, 0.95, 1.25, 1.65, 2.15, 2.8, 3.6, 4.5):
            distancia_x = pad + bw * 0.40 * escala
            distancia_y = pad + bh * 0.46 * escala
    ...
    for item in ubicaciones:
        ...
        # Fondo blanco ajustado prácticamente al contorno del texto.
        # Solo se añade un pequeño margen de 1-2 px para limpiar las líneas
        # del plano que pudieran quedar justo detrás de las letras.
        x1, y1, x2, y2 = item["caja"]
        pad_fondo = max(1, int(lado_menor * 0.0016))
        draw.rectangle(
            [
                x1 - pad_fondo,
                y1 - pad_fondo,
        ...
        # Subimos ligeramente el texto dentro del fondo blanco para compensar
        # el espacio óptico inferior de la fuente y que quede visualmente
        # centrado respecto al recuadro.
        ajuste_texto_y = max(2, int(lado_menor * 0.0036)) + 2
        draw.multiline_text(
            (item["cx"], item["cy"] - ajuste_texto_y),
            "\n".join(item["lineas"]),
            font=font,
            fill=(200, 0, 0),
    ...
    # Añadir nombre del centro entre la foto y el logotipo, centrado sobre
    # el ancho del logotipo cuando ambos elementos están presentes.
    draw_final = ImageDraw.Draw(img_rgba)
    font_info = _cargar_fuente(max(10, int(lado_menor * 0.018)))
    font_centro = _cargar_fuente(max(10, int(lado_menor * 0.0195)))

    overlay_por_tipo = {tipo: (overlay, caja) for tipo, overlay, caja in overlays}
    logo_info = overlay_por_tipo.get("logo")
    foto_info = overlay_por_tipo.get("foto")

    if centro and logo_info:
        _, caja_logo = logo_info
        x1l, y1l, x2l, y2l = caja_logo
        ancho_logo = x2l - x1l
        texto_centro = str(centro).strip()
        if texto_centro:
            bbox = draw_final.textbbox((0, 0), texto_centro, font=font_centro)
            tw = bbox[2] - bbox[0]
            th = bbox[3] - bbox[1]
            cx = x1l + max(0, (ancho_logo - tw) / 2)

            if foto_info:
                _, caja_foto = foto_info
                _, _, _, y2f = caja_foto
                espacio_inicio = y2f
                espacio_fin = y1l
                hueco = max(0, espacio_fin - espacio_inicio - th)
                # Ligeramente más cerca de la foto que del logotipo para que
                # visualmente no quede pegado al logo.
                cy = espacio_inicio + hueco * 0.35
            else:
                cy = max(1, y1l - th - max(4, int(lado_menor * 0.006)))

            pad_x = max(2, int(lado_menor * 0.003))
            pad_y = max(2, int(lado_menor * 0.003))
            draw_final.rectangle(
                [cx - pad_x, cy - pad_y, cx + tw + pad_x, cy + th + pad_y],
                fill=(255, 255, 255, 245),
            )
            draw_final.text((cx, cy), texto_centro, font=font_centro, fill=(0, 0, 0))

    # Superficies abajo a la izquierda. Se muestran en una sola línea,
    # separadas entre sí para ocupar poco espacio y no interferir con el plano.
    textos_superficie = []
    if str(superficie_construida or "").strip():
        textos_superficie.append(
            unicodedata.normalize(
                "NFC",
                f"SUPERFICIE CONSTRUIDA : {str(superficie_construida).strip()} m²",
            )
        )
    if str(superficie_util or "").strip():
        textos_superficie.append(
            unicodedata.normalize(
                "NFC",
                f"SUPERFICIE ÚTIL : {str(superficie_util).strip()} m²",
            )
        )

    if textos_superficie:
        texto_sup = "        ".join(textos_superficie)
        bbox = draw_final.textbbox((0, 0), texto_sup, font=font_info)
        tw = bbox[2] - bbox[0]
        th = bbox[3] - bbox[1]
        margen_inf = max(20, int(lado_menor * 0.028))
        margen_izq = max(10, int(lado_menor * 0.014))
        x = margen_izq
        y = h - margen_inf - th

        # Fondo blanco opaco por encima del plano y texto negro.
        pad_x = max(5, int(lado_menor * 0.0045))
        pad_y = max(3, int(lado_menor * 0.003))
        draw_final.rounded_rectangle(
            [x - pad_x, y - pad_y, x + tw + pad_x, y + th + pad_y],
            radius=max(3, int(lado_menor * 0.004)),
            fill=(255, 255, 255, 255),
        )
        draw_final.text((x, y), texto_sup, font=font_info, fill=(0, 0, 0))
