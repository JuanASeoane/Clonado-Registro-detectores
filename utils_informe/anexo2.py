...
def componer_plano(
    ...
    if centro and logo_info:
        ...
        if texto_centro:
            ...
            if foto_info:
                ...
                espacio_inicio = y2f
                espacio_fin = y1l
                cy = espacio_inicio + max(0, (espacio_fin - espacio_inicio - th) / 2)
            else:
                cy = max(1, y1l - th - max(4, int(lado_menor * 0.006)))

            pad_x = max(2, int(lado_menor * 0.003))
            pad_y = max(1, int(lado_menor * 0.002))
    ...
    if textos_superficie:
        ...
        tw = bbox[2] - bbox[0]
        th = bbox[3] - bbox[1]
        margen_inf = max(8, int(lado_menor * 0.012))
        margen_izq = max(8, int(lado_menor * 0.012))
        x = margen_izq
        y = h - margen_inf - th

        pad_x = max(3, int(lado_menor * 0.003))
        pad_y = max(2, int(lado_menor * 0.002))
        draw_final.rectangle(
