...
def extraer_resultados_pdf_laboratorio(pdf_bytes) -> pd.DataFrame:
    ...
    # Ejemplo habitual:
    #   "Incertidumbre expandida Concentración de radón para k=2 (Bq/m3)"
    # Se admite coma o punto decimal (p.ej. k=1,96 / k=1.96).
    k_match = re.search(
        r"(?:incertidumbre|incerteza)[\s\S]{0,250}?"
        r"(?:concentraci[oó]n|concentraci[oó]n\s+de\s+rad[oó]n)[\s\S]{0,180}?"
        r"\bk\s*=\s*(\d+(?:[.,]\d+)?)",
        texto,
        flags=re.IGNORECASE,
    )
    if not k_match:
        # Respaldo para PDFs cuya extracción de texto altera el orden de la cabecera.
        k_match = re.search(r"\bk\s*=\s*(\d+(?:[.,]\d+)?)", texto, flags=re.IGNORECASE)
    factor_k = k_match.group(1).strip() if k_match else ""
