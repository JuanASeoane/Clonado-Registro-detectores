...
def load_planos_metadata(file) -> dict:
    ...
    etiquetas = {
        ...
        "forma de comunicación": "Medio de comunicación",
        "forma de comunicación a los trabajadores": "Medio de comunicación",
        "condiciones del edificio durante la exposición": "Condiciones del edificio durante la exposición",
        "factor k": "Factor k",
    }
...
def extraer_resultados_pdf_laboratorio(pdf_bytes) -> pd.DataFrame:
    ...
    for pagina in reader.pages:
        try:
            ...
            pass
        # Algunas cabeceras de tabla no quedan bien representadas con la
        # extracción normal de pypdf. El modo layout conserva mejor columnas
        # y rótulos como "k=2", por eso se usa como segunda fuente de texto.
        try:
    ...
    # El factor de cobertura k se toma de la cabecera de la columna de
    # incertidumbre de concentración del propio informe del laboratorio.
    # Se prueban varias representaciones porque algunos PDF separan "k", "="
    # y el número en líneas/cajas de texto diferentes.
    import unicodedata as _ud

    def _extraer_factor_k(*textos):
        candidatos = []
        for _txt in textos:
            if not _txt:
                continue
            _txt = _ud.normalize("NFKC", str(_txt))
            _txt = (
                _txt.replace("\u00a0", " ")
                    .replace("\u200b", "")
                    .replace("\ufeff", "")
            ...
            candidatos.append(_txt)

            # Versión compacta: permite reconocer k = 2 aunque el PDF haya
            # introducido saltos de línea o espacios entre cada elemento.
            _compacto = re.sub(r"\s+", "", _txt)
            candidatos.append(_compacto)

        patrones = (
            # Prioridad: k situado cerca del encabezado de incertidumbre/concentración.
            r"(?:incertidumbre|incerteza)[\s\S]{0,700}?\bk\s*(?:=|:)?\s*(\d+(?:[.,]\d+)?)",
            r"(?:concentraci[oó]n|concentracion)[\s\S]{0,500}?\bk\s*(?:=|:)?\s*(\d+(?:[.,]\d+)?)",
            # Respaldo general dentro del texto extraído.
            r"\bk\s*(?:=|:)\s*(\d+(?:[.,]\d+)?)",
            # En la versión compacta puede no quedar un límite de palabra útil
            # antes de k, por lo que se admite el contexto "para k".
            r"parak(?:=|:)?(\d+(?:[.,]\d+)?)",
        )
        for _candidato in candidatos:
            for _patron in patrones:
                _m = re.search(_patron, _candidato, flags=re.IGNORECASE)
                if _m:
                    return _m.group(1).strip()
        return ""

    factor_k = _extraer_factor_k(texto, texto_layout)
    ...
    return pd.DataFrame(filas, columns=["Código", "Resultado", "Incertidumbre", "K"])
...
def merge_resultados(df: pd.DataFrame, resultados_df: pd.DataFrame) -> pd.DataFrame:
    ...
    k_col = _find_column(list(resultados_df.columns), "K")
    ...
    for _, row in resultados_df.iterrows():
        ...
        if k_col and pd.notna(row.get(k_col)):
            valor_k = str(row[k_col]).strip()
            if valor_k and valor_k.lower() != "nan":
                lookup_k[code] = valor_k
    ...
    def apply_k(r):
        code = str(r[COL_CODIGO_DETECTOR]).strip()
        if code in lookup_k:
            return lookup_k[code]
        return r.get("K", "") if hasattr(r, "get") else ""
    ...
    merged["K"] = merged.apply(apply_k, axis=1)
    return merged
...
def traducir_es_gl(texto: str | None) -> str:
    ...
    traducido = re.sub(r"[A-Za-zÀ-ÿ]+", _repl, texto)

    # Regla general solicitada para términos terminados en -gía/-gías:
    # Urología -> Uroloxía, Cardiología -> Cardioloxía, etc.
    def _repl_gia(match: re.Match) -> str:
        palabra = match.group(0)
        sufijo_plural = palabra.lower().endswith(("gías", "gias"))
        base = palabra[:-4] if sufijo_plural else palabra[:-3]
        nuevo = base + ("xías" if sufijo_plural else "xía")
        return _capitalizar_como(palabra, nuevo)

    traducido = re.sub(r"[A-Za-zÀ-ÿ]*g[ií]as?\b", _repl_gia, traducido, flags=re.IGNORECASE)
    return traducido
...
def postos_traballo_bullets(
    ...
    for _, row in df.iterrows():
        ...
        for name, count, turno in entradas:
            es_generica = _strip_accents(name).lower().strip() == "profesionales (sin especificar)"
            if es_generica:
                # En el informe no mostramos la etiqueta interna "(sin especificar)".
                # Se presenta como una categoría normal: p.ej. "8 profesionais"
                # en gallego o "8 profesionales" en castellano.
                display_name = "profesionais" if traducir_galego else "profesionales"
            else:
                if traducir_galego:
