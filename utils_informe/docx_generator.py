...
def _quitar_turno_de_puestos(texto: str, idioma: str = "gl") -> str:
    """En la tabla del punto 7 no deben figurar los horarios. Además,
    la categoría interna "Profesionales (sin especificar)" se muestra
    solo como número + profesionales/profesionais."""
    if not texto:
        return texto
    partes = [p.strip() for p in re.split(r"[,\n]", texto) if p.strip()]
    resultado = []
    for parte in partes:
        sin_turno = parte.split(" - ", 1)[0].strip()
        m = re.match(r"^Profesionales \(sin especificar\)\s*\((\d+)\)\s*$", sin_turno, flags=re.IGNORECASE)
        if m:
            nombre = "profesionais" if idioma == "gl" else "profesionales"
            resultado.append(f"{m.group(1)} {nombre}")
        else:
            resultado.append(sin_turno)
...
def _section_7(doc, df: pd.DataFrame, ctx: ReportContext, idioma: str):
    ...
    for _, row in df.iterrows():
        ...
        puestos_txt = _quitar_turno_de_puestos(str(puestos).strip(), idioma=idioma) if puestos else ""
