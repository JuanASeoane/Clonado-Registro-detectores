...
def _procesar_resultado_camara_local(ns, result):
    """Procesa el resultado devuelto por la cámara local del navegador."""
    if result is None:
        return False
    ...
    try:
        if "," in captured:
            captured = captured.split(",", 1)[1]
        datos = base64.b64decode(captured)
...
def _renderizar_lector_camara_local(ns):
    ...
    if _procesar_resultado_camara_local(ns, result):
        st.rerun()
