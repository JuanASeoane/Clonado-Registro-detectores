        ...
        if not detectores:
            ...
            if idx_sel_r is None:
                ...
                if tecnico_distinto:
                    ...
                    opcion_nuevo_tecnico = "➕ Otro técnico"
...
def _is47_resultados_dataframe(detectores, categorias=None):
    ...
    for det in detectores:
        ...
        filas.append({
            ...
            "Incerteza expandida (Bq/m³) K=2": str(det.get("incertidumbre") or "").strip(),
...
def _is47_docx_bytes(datos, ocupacion, df_resultados, conclusion, anexos_presentes, idioma="gl"):
    ...
    cells0 = t.rows[0].cells
    cells0[0].text = "GERENCIA" if es else "XERENCIA"
    cells0[0].paragraphs[0].runs[0].bold = True
    cells0[1].merge(cells0[2]).text = str(datos.get("xerencia", "") or "")
    cells0[3].text = f"CIF: {datos.get('cif','')}"

    cells1 = t.rows[1].cells
    cells1[0].text = "CENTRO"
    cells1[0].paragraphs[0].runs[0].bold = True
    cells1[1].merge(cells1[2]).text = str(datos.get("centro", "") or "")
    cells1[3].text = f"Teléfono: {datos.get('telefono','')}"

    cells2 = t.rows[2].cells
    cells2[0].text = "SERVICIO / UNIDAD MUESTREADA" if es else "SERVIZO / UNIDADE MOSTREXADA"
    cells2[0].paragraphs[0].runs[0].bold = True
    cells2[1].merge(cells2[2]).merge(cells2[3]).text = str(datos.get("servizo", "") or "")

    cells3 = t.rows[3].cells
    cells3[0].text = "DIRECCIÓN" if es else "ENDEREZO"
    cells3[0].paragraphs[0].runs[0].bold = True
    cells3[1].merge(cells3[2]).merge(cells3[3]).text = str(datos.get("enderezo", "") or "")

    cells4 = t.rows[4].cells
    # En esta fila se reservan dos celdas iguales para las dos superficies.
    # La descripción ocupa las dos primeras columnas y cada superficie una columna.
    celda_desc = cells4[0].merge(cells4[1])
    celda_desc.text = "DESCRIPCIÓN DEL ÁREA MUESTREADA" if es else "DESCRICIÓN DA ÁREA MOSTREXADA"
    celda_desc.paragraphs[0].runs[0].bold = True
    cells4[2].text = f"{'Superficie construida' if es else 'Superficie construída'}: {datos.get('superficie_construida','')}"
    cells4[3].text = f"Superficie útil: {datos.get('superficie_util','')}"
    ...
    if df_resultados is not None and not df_resultados.empty:
        cols = ([
            ...
            "Concentración radón (Bq/m³)", "Incertidumbre expandida (Bq/m³) K=2",
            "Puesto/puestos de trabajo asociados",
        ] if es else [
            "Código zona", "Código detector", "Data inicio/Data fin",
            "Concentración radon (Bq/m³)", "Incerteza expandida (Bq/m³) K=2",
        ...
        for _, fila in df_resultados.iterrows():
            ...
            try:
                ...
                pass
            _inc_texto = str(fila.get("Incerteza expandida (Bq/m³) K=2", "") or "").strip()
            if _inc_texto:
...
def _pantalla_informe_is47(cid, centro, variante="A"):
    ...
    with _acordeon_informe("1", "DATOS DE LA EMPRESA Y DEL CENTRO DE TRABAJO", completos_1):
        ...
        with c1:
            ...
            st.text_input("Dirección", value=direccion or "", disabled=True, key=f"is47_enderezo_{cid}")
        ...
        if df_res.empty:
            ...
            editada = st.data_editor(
                ...
                column_config={
                    ...
                    "Incerteza expandida (Bq/m³) K=2": st.column_config.TextColumn(),
                },
            )
            for i, (_, fila) in enumerate(editada.iterrows()):
                try:
                    ...
                    inc = str(fila["Incerteza expandida (Bq/m³) K=2"] or "").strip()
    ...
    with _acordeon_informe("", "GENERACIÓN DEL INFORME", True):
        ...
        with col_opt1:
            generar_castellano = st.checkbox(
                "Generar versión en castellano", key=f"is47_castellano_{cid}"
            )
        with col_opt2:
            pdf_disponible = libreoffice_disponible()
            generar_pdf_completo = st.checkbox(
                "Generar PDF completo (informe + anexos)",
                disabled=not pdf_disponible, key=f"is47_pdf_completo_{cid}",
            )
            ...
            try:
                ...
                if anexos_subidos:
                    ...
                    with zipfile.ZipFile(zbuf, "w", zipfile.ZIP_DEFLATED) as zf:
                        for nom_docx, contenido_docx in docx_para_zip:
                            zf.writestr(nom_docx, contenido_docx)
                    ...
                    st.session_state[f"is47_zip_{cid}"] = (zbuf.getvalue(), nombre_base + "-con-anexos.zip")
                else:
                    st.session_state.pop(f"is47_zip_{cid}", None)
                ...
                if generar_pdf_completo and pdf_disponible:
                    ...
                    for idioma_pdf, nombre_docx, contenido_docx in docx_para_pdf:
                        try:
                            pdf = construir_pdf_completo(contenido_docx, anexos_pdf)
                            pdf_name = nombre_docx.rsplit(".", 1)[0] + "_completo.pdf"
                            st.session_state[f"is47_pdf_{idioma_pdf}_{cid}"] = (pdf, pdf_name)
                        except PdfToolsError as e:
                            st.error(f"No se ha podido generar el PDF completo ({idioma_pdf}): {e}")
                else:
                    st.session_state.pop(f"is47_pdf_gl_{cid}", None)
                    st.session_state.pop(f"is47_pdf_es_{cid}", None)
        ...
        for key, etiqueta, mime in [
            (f"is47_docx_gl_{cid}", "⬇️ Descargar informe en gallego (.docx)", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"),
            (f"is47_docx_es_{cid}", "⬇️ Descargar informe en castellano (.docx)", "application/vnd.openxmlformats-officedocument.wordprocessingml.document"),
            (f"is47_pdf_gl_{cid}", "⬇️ Descargar PDF completo en gallego", "application/pdf"),
            (f"is47_pdf_es_{cid}", "⬇️ Descargar PDF completo en castellano", "application/pdf"),
            (f"is47_zip_{cid}", "⬇️ Descargar informe(s) + anexos (.zip)", "application/zip"),
        ]:
            if key in st.session_state:
                contenido, nombre_archivo = st.session_state[key]
                st.download_button(etiqueta, data=contenido, file_name=nombre_archivo, mime=mime, key=f"dl_{key}")
