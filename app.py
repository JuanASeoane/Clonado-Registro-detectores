...
def _reconocer_codigo_detector_desde_foto(foto):
    """Reconoce en una fotografía un código de 2 letras y 4 números."""
    if foto is None:
        raise ValueError("Primero haz una foto de la etiqueta del detector.")

    contenido = foto.getvalue()

    # Preprocesado sencillo para mejorar etiquetas pequeñas y evitar imágenes enormes.
    imagen = Image.open(io.BytesIO(contenido)).convert("RGB")
    max_lado = max(imagen.size)
    if max_lado < 1400:
        escala = min(2.5, 1400 / max(1, max_lado))
        imagen = imagen.resize(
            (max(1, int(imagen.width * escala)), max(1, int(imagen.height * escala))),
            Image.Resampling.LANCZOS,
        )
    elif max_lado > 2400:
        escala = 2400 / max_lado
        imagen = imagen.resize(
            (max(1, int(imagen.width * escala)), max(1, int(imagen.height * escala))),
            Image.Resampling.LANCZOS,
        )
    ...
    buf = io.BytesIO()
    imagen.save(buf, format="JPEG", quality=95)
    datos_ocr = buf.getvalue()

    motor = _motor_ocr_detector()
    resultado = motor(datos_ocr)
    textos = _extraer_textos_resultado_rapidocr(resultado)

    # Buscar en cada línea y también en el texto concatenado, por si OCR separa
    # las dos letras de los cuatro números.
    for texto in textos:
        codigo = _normalizar_codigo_detector_ocr(texto)
        if codigo:
            return codigo

    codigo = _normalizar_codigo_detector_ocr(" ".join(textos))
    if codigo:
        return codigo
...
def _procesar_ocr_codigo_automatico(ns):
    """
    Si acaba de aparecer una fotografía nueva, realiza el OCR automáticamente.
    Al reconocer un código válido, rellena el campo y cierra la cámara.
    """
    ...
    # No volver a procesar la misma fotografía en reruns posteriores.
...
def _renderizar_campos_detector(cid, detector_id, ns):
    ...
    # camera_input provoca un rerun al capturar la foto. En ese rerun,
    # procesamos la imagen antes de crear el text_input del código, de modo
    # que podemos rellenarlo automáticamente sin pulsar ningún botón.
    _procesar_ocr_codigo_automatico(ns)
