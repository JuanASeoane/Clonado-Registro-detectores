...
div[data-testid="stCheckbox"] [data-baseweb="checkbox"] > div:first-of-type {
    ...
    /* Botones de navegación: Guardar e ir al inicio, Guardar y volver y equivalentes -> texto blanco. */
    ...
    /* Texto blanco en los botones de navegación (← Guardar e ir al inicio / ← Guardar y volver). */
    ...
    /* ===== PRUEBA ESTÉTICA MODERNA V1 - SOLO PRESENTACIÓN ===== */
    :root {
        --radon-bg: #11161B;
        --radon-surface: #1A2026;
        --radon-surface-2: #20272E;
        --radon-border: #39434C;
        --radon-text: #F4F6F8;
        --radon-muted: #AAB2BA;
        --radon-accent: #F28C28;
    ...
    .stApp { color: var(--radon-text) !important; }

    /* Tipografía menos pesada y con mejor jerarquía. */
    .stApp p, .stApp label, .stApp li,
    div[data-testid="stCaptionContainer"],
    div[data-testid="stMarkdownContainer"] p {
        font-weight: 500 !important;
    }
    .stApp label, div[data-testid="stExpander"] summary p {
    ...
    @media (max-width: 768px) {
        .titulo-centro { font-size: 31px !important; }
    }

    /* Tarjetas y recuadros. */
    div[data-testid="stVerticalBlockBorderWrapper"] {
        border-radius: 12px !important;
        border-color: var(--radon-border) !important;
        background: rgba(26, 32, 38, 0.92) !important;
        box-shadow: 0 5px 18px rgba(0,0,0,0.16) !important;
    }

    /* Inputs y selectores más sobrios. */
    div[data-baseweb="input"],
    div[data-baseweb="select"] > div,
    textarea {
        border-radius: 9px !important;
        border-color: #4A555F !important;
        box-shadow: none !important;
    }
    div[data-baseweb="input"]:focus-within,
    div[data-baseweb="select"] > div:focus-within,
    textarea:focus {
        border-color: var(--radon-accent) !important;
        box-shadow: 0 0 0 1px var(--radon-accent) !important;
    }

    /* Botones: primario naranja; secundarios carbón con borde discreto. */
    div[data-testid="stButton"] button,
    div[data-testid="stDownloadButton"] button,
    ...
    div[data-testid="stFormSubmitButton"] button[kind="primary"] {
        background: var(--radon-accent) !important;
        border-color: var(--radon-accent) !important;
        color: white !important;
    }
    div[data-testid="stButton"] button[kind="secondary"] {
        background: var(--radon-surface-2) !important;
        border: 1px solid #53606B !important;
        color: var(--radon-text) !important;
    }
    div[data-testid="stButton"] button[kind="secondary"]:hover {
    ...
    /* Acordeones modernos: menos naranja permanente, más naranja al abrir. */
    div[data-testid="stExpander"] {
        border: 1px solid var(--radon-border) !important;
        border-radius: 10px !important;
        overflow: hidden !important;
        background: rgba(26,32,38,.88) !important;
    }
    div[data-testid="stExpander"] summary {
        background: var(--radon-surface-2) !important;
        border: 0 !important;
    ...
    /* Alertas y mensajes también como tarjetas. */
...
# ============================================================
# CONFIGURACIÓN DE ALMACENAMIENTO - RAILWAY VOLUME
# ============================================================

VOLUME_PATH = "/app/datos_guardados"
...
def get_tmp_dir():
    """Carpeta temporal unificada dentro del volumen."""
    tmp_dir = os.path.join(VOLUME_PATH, "tmp")
    os.makedirs(tmp_dir, exist_ok=True)
    return tmp_dir

def get_db_path():
    return os.path.join(get_data_dir(), "radon_data.db")


def get_db_connection():
    """Abre SQLite con parámetros robustos para Railway."""
...
# ============================================================
# FUNCIONES PARA ORGANIZACIÓN POR CENTRO
# ============================================================

def _propietario_centro_almacenamiento(centro_id):
    """Devuelve el propietario estable del centro para organizar el volumen."""
    try:
        ...
        row = c.fetchone()
        conn.close()
        propietario = (row[0] or "").strip() if row else ""
    except Exception:
        propietario = ""
    # Los centros antiguos, creados antes del sistema multiusuario,
    # quedan bajo ADMIN porque solo el administrador puede verlos.
    return propietario or "ADMIN"


def _nombre_carpeta_usuario(usuario):
    """
    Conserva el nombre legible del usuario (por ejemplo «Coruña») y elimina
    únicamente caracteres que no son válidos/seguros en un nombre de carpeta.
    """
    nombre = str(usuario or "ADMIN").strip() or "ADMIN"
    nombre = re.sub(r'[\\/:*?"<>|]+', "_", nombre)
    nombre = nombre.replace("..", "_")
    return nombre.strip(" .") or "ADMIN"


def get_usuario_dir(usuario):
    """Carpeta física del usuario dentro del Railway Volume."""
    return os.path.join(VOLUME_PATH, "usuarios", _nombre_carpeta_usuario(usuario))


def get_centro_dir(centro_id):
    """Ruta del centro: /usuarios/USUARIO/ID_NOMBRE_CENTRO."""
    centro = get_centro(centro_id)
    if not centro:
        return get_data_dir()
    nombre_centro = _slug(centro[1] or f"centro_{centro_id}")
    usuario = _propietario_centro_almacenamiento(centro_id)
    return os.path.join(
        get_usuario_dir(usuario),
        f"{centro_id}_{nombre_centro}",
    )


def get_centro_subdir(centro_id, subdir):
    """Devuelve la ruta de un subdirectorio sin crearlo."""
    return os.path.join(get_centro_dir(centro_id), subdir)


def crear_centro_dir(centro_id):
    """Crea la carpeta del usuario y del centro cuando sea necesario."""
    centro_dir = get_centro_dir(centro_id)
    os.makedirs(centro_dir, exist_ok=True)
    return centro_dir


def crear_centro_subdir(centro_id, subdir):
    """Crea un subdirectorio del centro solo cuando es necesario."""
    sub_dir = get_centro_subdir(centro_id, subdir)
    os.makedirs(sub_dir, exist_ok=True)
    return sub_dir


def guardar_archivo_centro(centro_id, contenido, nombre_archivo, subdir=""):
    if centro_id:
        if subdir:
            destino_dir = crear_centro_subdir(centro_id, subdir)
        else:
            destino_dir = crear_centro_dir(centro_id)
    else:
        destino_dir = get_data_dir()
...
def guardar_bytes_imagen(file_bytes, prefijo, ext=".jpg", centro_id=None, subdir="fotos"):
    if centro_id:
        data_dir = crear_centro_subdir(centro_id, subdir)
    else:
...
def obtener_ruta_carpeta_centro(centro_id, nombre_centro):
    """Obtiene la ruta física del centro dentro de la carpeta de su usuario."""
    nombre_slug = _slug(nombre_centro or f"centro_{centro_id}")
    usuario = _propietario_centro_almacenamiento(centro_id)
    return os.path.join(
        get_usuario_dir(usuario),
        f"{centro_id}_{nombre_slug}",
    )


def migrar_carpetas_centros_a_usuarios():
    """
    Migra automáticamente las antiguas carpetas /centros/ID_NOMBRE a
    /usuarios/USUARIO/ID_NOMBRE y actualiza en SQLite todas las rutas guardadas.
    No modifica la base de datos principal ni las carpetas tmp/backups.
    """
    antigua_raiz = os.path.join(VOLUME_PATH, "centros")
    if not os.path.isdir(antigua_raiz):
        return

    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT id, nombre, COALESCE(propietario, '') FROM centros")
    centros = c.fetchall()

    for centro_id, nombre, propietario in centros:
        propietario = (propietario or "").strip() or "ADMIN"
        antigua = os.path.join(
            antigua_raiz,
            f"{centro_id}_{_slug(nombre or f'centro_{centro_id}')}",
        )
        nueva = os.path.join(
            get_usuario_dir(propietario),
            f"{centro_id}_{_slug(nombre or f'centro_{centro_id}')}",
        )
        if antigua == nueva or not os.path.isdir(antigua):
            continue
        ...
        if not os.path.exists(nueva):
            ...
            # Si ya existe destino, trasladamos únicamente lo que falte.
            for item in os.listdir(antigua):
                origen_item = os.path.join(antigua, item)
                destino_item = os.path.join(nueva, item)
        ...
        prefijo_ant = antigua + os.sep
        prefijo_nuevo = nueva + os.sep
        c.execute(
            "UPDATE centros SET imagen_exterior_path=REPLACE(imagen_exterior_path, ?, ?) WHERE id=?",
            (prefijo_ant, prefijo_nuevo, centro_id),
        )
        c.execute(
            """UPDATE detectores SET
               plano_path=REPLACE(plano_path, ?, ?),
               foto_situacion_path=REPLACE(foto_situacion_path, ?, ?),
               foto_detector_path=REPLACE(foto_detector_path, ?, ?)
               WHERE centro_id=?""",
            (prefijo_ant, prefijo_nuevo, prefijo_ant, prefijo_nuevo,
             prefijo_ant, prefijo_nuevo, centro_id),
        )
        c.execute(
            "UPDATE planos_centro SET ruta=REPLACE(ruta, ?, ?) WHERE centro_id=?",
            (prefijo_ant, prefijo_nuevo, centro_id),
        )
    ...
    try:
        if os.path.isdir(antigua_raiz) and not os.listdir(antigua_raiz):
            os.rmdir(antigua_raiz)
    except OSError:
        pass
...
def _json_seguro(valor):
    """Convierte valores SQLite a un formato serializable sin perder información útil."""
    if isinstance(valor, (bytes, bytearray)):
        return {
            "_tipo": "bytes_base64",
            "valor": base64.b64encode(bytes(valor)).decode("ascii"),
        }
...
def guardar_snapshot_centro(centro_id):
    """
    Guarda en la carpeta física del centro una copia JSON de todos los datos
    del centro presentes en SQLite. Se regenera automáticamente y facilita
    comprobar en el volumen qué datos pertenecen a cada usuario/centro.
    """
    centro_id = int(centro_id)
    carpeta = crear_centro_dir(centro_id)
    conn = get_db_connection()
    ...
    datos = {
        "centro_id": centro_id,
        "propietario": _propietario_centro_almacenamiento(centro_id),
        "actualizado": _ahora_espana().isoformat(),
        "tablas": {},
    }
    ...
    for tabla in tablas:
        try:
            ...
            columnas = [r[1] for r in c.fetchall()]
            if tabla == "centros" and "id" in columnas:
                c.execute(f'SELECT * FROM "{tabla}" WHERE id=?', (centro_id,))
            elif "centro_id" in columnas:
                c.execute(f'SELECT * FROM "{tabla}" WHERE centro_id=?', (centro_id,))
            else:
                continue

            filas = []
            for fila in c.fetchall():
                filas.append({k: _json_seguro(fila[k]) for k in fila.keys()})
            datos["tablas"][tabla] = filas
        except Exception:
            # Una tabla auxiliar no debe impedir que se guarde el resto.
            continue

    conn.close()
    ruta = os.path.join(carpeta, "datos_centro.json")

    # Escritura atómica: primero se genera un temporal en la misma carpeta y
    # solo cuando está completamente escrito se sustituye el JSON anterior.
    # Así un reinicio del contenedor durante la escritura no deja un JSON
    # truncado o parcialmente escrito.
    ruta_tmp = None
    try:
...
def sincronizar_carpetas_y_datos_centros():
    """
    Asegura que exista /usuarios/USUARIO/CENTRO para todos los centros y
    actualiza datos_centro.json en cada rerun autenticado.
    """
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT id FROM centros ORDER BY id")
    ids = [r[0] for r in c.fetchall()]
    conn.close()

    for centro_id in ids:
        try:
            guardar_snapshot_centro(centro_id)
        except Exception:
            pass


def migrar_nombres_carpetas_usuario_anteriores():
    """
    Corrige la v40, que podía crear «Coruña» como «coru_a».
    Si encuentra la carpeta antigua, la mueve al nombre legible actual.
    """
    raiz = os.path.join(VOLUME_PATH, "usuarios")
    if not os.path.isdir(raiz):
        return

    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT DISTINCT COALESCE(NULLIF(propietario,''),'ADMIN') FROM centros")
    usuarios = [r[0] for r in c.fetchall()]
    conn.close()

    for usuario in usuarios:
        antigua = os.path.join(raiz, _slug(usuario or "ADMIN"))
        nueva = get_usuario_dir(usuario)
        if antigua == nueva or not os.path.isdir(antigua):
            continue
        os.makedirs(os.path.dirname(nueva), exist_ok=True)
        if not os.path.exists(nueva):
            ...
            for item in os.listdir(antigua):
                origen = os.path.join(antigua, item)
                destino = os.path.join(nueva, item)
...
def crear_backup_db():
    """Crea una copia de seguridad consistente de SQLite."""
    db_path = get_db_path()
    if not os.path.isfile(db_path):
        return None
    backup_dir = os.path.join(VOLUME_PATH, "backups")
    os.makedirs(backup_dir, exist_ok=True)
    marca = _ahora_espana().strftime("%Y%m%d_%H%M%S_%f")
    destino = os.path.join(backup_dir, f"radon_data_{marca}.db")
    origen = None
    copia = None
    try:
        origen = get_db_connection()
...
def limpiar_tmp_detector(detector_id):
    """Elimina temporales del detector cuando se borra el detector."""
    tmp_dir = get_tmp_dir()
    if not os.path.isdir(tmp_dir):
        return
    patrones = (
        f"_tmp_plano_{detector_id}.",
        f"_tmp_plano_punto_retirada_{detector_id}.",
        f"_tmp_plano_punto_det{detector_id}.",
    )
    for nombre in os.listdir(tmp_dir):
        if any(nombre.lower().startswith(p.lower()) for p in patrones):
...
def limpiar_tmp_centro(centro_id):
    """Al borrar un centro elimina los temporales de todos sus detectores y del propio centro."""
    try:
        detectores = fetch_detectores(centro_id)
    except Exception:
        detectores = []
    for detector in detectores:
        limpiar_tmp_detector(detector[0])

    tmp_dir = get_tmp_dir()
    if os.path.isdir(tmp_dir):
        for nombre in os.listdir(tmp_dir):
            n = nombre.lower()
            if n.startswith(f"_tmp_centro_{centro_id}_") or n.startswith(f"_tmp_informe_completo_{centro_id}"):
                ruta = os.path.join(tmp_dir, nombre)
                try:
                    if os.path.isfile(ruta):
                        os.remove(ruta)
...
def archivo_referenciado_por_otro_registro(ruta, excluir_detector_id=None, excluir_plano_id=None):
    """Comprueba si un archivo sigue siendo usado antes de eliminarlo físicamente."""
    if not ruta:
        return False
    conn = get_db_connection()
    c = conn.cursor()
    try:
        if excluir_detector_id is None:
            c.execute("SELECT COUNT(*) FROM detectores WHERE plano_path=? OR foto_situacion_path=? OR foto_detector_path=?", (ruta, ruta, ruta))
        else:
            c.execute("SELECT COUNT(*) FROM detectores WHERE id<>? AND (plano_path=? OR foto_situacion_path=? OR foto_detector_path=?)", (excluir_detector_id, ruta, ruta, ruta))
        if c.fetchone()[0] > 0:
            return True
        if excluir_plano_id is None:
            c.execute("SELECT COUNT(*) FROM planos_centro WHERE ruta=?", (ruta,))
        else:
            c.execute("SELECT COUNT(*) FROM planos_centro WHERE id<>? AND ruta=?", (excluir_plano_id, ruta))
        if c.fetchone()[0] > 0:
            return True
        c.execute("SELECT COUNT(*) FROM centros WHERE imagen_exterior_path=?", (ruta,))
        return c.fetchone()[0] > 0
    finally:
        conn.close()


def comprobar_integridad_almacenamiento():
    """Devuelve incidencias entre SQLite y los archivos del Volume sin modificar nada."""
    faltantes = []
    huerfanos = []
    referencias = set()
    conn = get_db_connection()
    c = conn.cursor()
    try:
        for (ruta,) in c.execute("SELECT imagen_exterior_path FROM centros WHERE imagen_exterior_path IS NOT NULL AND imagen_exterior_path<>''"):
            referencias.add(ruta)
        for fila in c.execute("SELECT plano_path, foto_situacion_path, foto_detector_path FROM detectores"):
            referencias.update(r for r in fila if r)
        for (ruta,) in c.execute("SELECT ruta FROM planos_centro WHERE ruta IS NOT NULL AND ruta<>''"):
            referencias.add(ruta)
        for fila in c.execute("SELECT logo_path, logo_laboratorio_path FROM settings WHERE id=1"):
            referencias.update(r for r in fila if r)
    finally:
        conn.close()
    ...
    centros_root = os.path.join(VOLUME_PATH, "centros")
    if os.path.isdir(centros_root):
        for raiz, dirs, archivos in os.walk(centros_root):
            # Los informes son salidas válidas de la app y no se registran en SQLite.
            if os.path.basename(raiz).lower() == "informes":
                continue
            for nombre in archivos:
                ruta = os.path.join(raiz, nombre)
                # Los temporales se gestionan aparte y se conservan por diseño.
                if nombre.startswith("_tmp_"):
                    continue
                if ruta not in referencias:
                    huerfanos.append(ruta)

    return {"faltantes": faltantes, "huerfanos": huerfanos}


def eliminar_carpeta_centro(centro_id, nombre_centro=None):
    """
    Elimina físicamente la carpeta del centro del Railway Volume.

    El nombre del centro se puede pasar directamente para evitar depender
    de SQLite después de que el registro haya sido eliminado.
    """
    ...
    centro_dir = obtener_ruta_carpeta_centro(centro_id, nombre_centro)

    if os.path.exists(centro_dir):
        try:
            shutil.rmtree(centro_dir)
            return True
        except Exception as e:
            st.error(
                f"Error al eliminar la carpeta del centro: {e}"
            )
            return False
...
# ============================================================
# FAVICON Y FONDO DE PANTALLA
# ============================================================

_carpeta_script = os.path.dirname(os.path.abspath(__file__))

_ruta_favicon_volumen = os.path.join(VOLUME_PATH, "favicon.png")
_ruta_favicon_script = os.path.join(_carpeta_script, "favicon.png")
if os.path.exists(_ruta_favicon_volumen):
    _ruta_favicon = _ruta_favicon_volumen
elif os.path.exists(_ruta_favicon_script):
    _ruta_favicon = _ruta_favicon_script
else:
    _ruta_favicon = None
_icono_pagina = _ruta_favicon if (_ruta_favicon and os.path.exists(_ruta_favicon)) else "☢️"

st.set_page_config(page_title="Detectores Rn", page_icon=_icono_pagina, layout="wide")

_ruta_fondo_volumen = os.path.join(VOLUME_PATH, "fondo_app.jpg")
_ruta_fondo_script = os.path.join(_carpeta_script, "fondo_app.jpg")
if os.path.exists(_ruta_fondo_volumen):
    _ruta_fondo = _ruta_fondo_volumen
elif os.path.exists(_ruta_fondo_script):
    _ruta_fondo = _ruta_fondo_script
else:
    _ruta_fondo = None

_fondo_css = ""
if _ruta_fondo and os.path.exists(_ruta_fondo):
...
st.markdown(
    ...
    /* Punto 6 - cargador de PDF de resultados: fondo oscuro integrado con el acordeón. */
    div[data-testid="stElementContainer"]:has(div.marcador-uploader-resultados-p6)
      + div[data-testid="stElementContainer"] div[data-testid="stFileUploaderDropzone"] {
        background-color: #262626 !important;
    ...
    div[data-testid="stElementContainer"]:has(div.marcador-uploader-resultados-p6)
      + div[data-testid="stElementContainer"] div[data-testid="stFileUploaderDropzone"] button {
        background-color: #262626 !important;
        color: #FFFFFF !important;
        border: 1.5px solid #F28C28 !important;
    }
    div[data-testid="stElementContainer"]:has(div.marcador-uploader-resultados-p6)
      + div[data-testid="stElementContainer"] div[data-testid="stFileUploaderDropzone"] button:hover {
    ...
    /* ============================================================
       AVISOS, ACLARACIONES Y COMENTARIOS: TEXTO SIEMPRE BLANCO
       Mantiene el color de fondo de info/warning/success/error,
       pero fuerza todo su contenido a blanco para mejorar contraste.
       ============================================================ */
    div[data-testid="stAlert"],
    div[data-testid="stAlert"] *,
    div[role="alert"],
...
def init_db():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS login_intentos
                 (dispositivo TEXT PRIMARY KEY,
                  intentos INTEGER NOT NULL DEFAULT 0,
                  bloqueado_hasta REAL NOT NULL DEFAULT 0,
                  ultimo_fallo REAL NOT NULL DEFAULT 0)''')
    ...
    if "codigo_centro_zona" not in columnas_centros:
        c.execute("ALTER TABLE centros ADD COLUMN codigo_centro_zona TEXT DEFAULT ''")
    if "telefono" not in columnas_centros:
        c.execute("ALTER TABLE centros ADD COLUMN telefono TEXT DEFAULT ''")
    if "modelo_informe" not in columnas_centros:
        c.execute("ALTER TABLE centros ADD COLUMN modelo_informe TEXT DEFAULT 'sscc'")
    if "propietario" not in columnas_centros:
        # Centros existentes quedan sin propietario: solo el administrador podrá verlos.
    ...
    if "tecnico_retirada" not in columnas_detectores:
        c.execute("ALTER TABLE detectores ADD COLUMN tecnico_retirada TEXT DEFAULT ''")
    ...
    for columna_nueva in (
        ...
        "fecha_comunicacion_trab", "medio_comunicacion", "condiciones_edificio", "factor_k",
    ):
    ...
    if "nombre_laboratorio" not in columnas_settings:
        c.execute("ALTER TABLE settings ADD COLUMN nombre_laboratorio TEXT DEFAULT ''")

    # Protección de integridad del código del detector. La interfaz ya valida
    # duplicados, pero la base de datos debe impedirlos también si dos sesiones
    # intentan guardar el mismo código prácticamente a la vez.
    #
    # En instalaciones antiguas puede existir algún duplicado previo. En ese
    # caso no alteramos ni eliminamos datos existentes: los triggers bloquean
    # nuevos duplicados y el índice UNIQUE se crea automáticamente cuando ya no
    # haya duplicados heredados.
    c.execute(
        """SELECT UPPER(TRIM(detector_codigo)) AS codigo_norm, COUNT(*)
           FROM detectores
           WHERE TRIM(COALESCE(detector_codigo, '')) <> ''
           GROUP BY UPPER(TRIM(detector_codigo))
           HAVING COUNT(*) > 1
           LIMIT 1"""
    )
    hay_duplicados_heredados = c.fetchone() is not None
    if not hay_duplicados_heredados:
        c.execute(
            """CREATE UNIQUE INDEX IF NOT EXISTS ux_detectores_codigo_normalizado
               ON detectores(UPPER(TRIM(detector_codigo)))
               WHERE TRIM(COALESCE(detector_codigo, '')) <> ''"""
        )

    c.execute(
        """CREATE TRIGGER IF NOT EXISTS trg_detectores_codigo_unico_insert
           BEFORE INSERT ON detectores
           WHEN TRIM(COALESCE(NEW.detector_codigo, '')) <> ''
            AND EXISTS (
                SELECT 1 FROM detectores
                WHERE UPPER(TRIM(detector_codigo)) = UPPER(TRIM(NEW.detector_codigo))
            )
           BEGIN
               SELECT RAISE(ABORT, 'codigo_detector_duplicado');
           END"""
    )
    c.execute(
        """CREATE TRIGGER IF NOT EXISTS trg_detectores_codigo_unico_update
           BEFORE UPDATE OF detector_codigo ON detectores
           WHEN TRIM(COALESCE(NEW.detector_codigo, '')) <> ''
            AND EXISTS (
                SELECT 1 FROM detectores
                WHERE UPPER(TRIM(detector_codigo)) = UPPER(TRIM(NEW.detector_codigo))
...
def delete_plano_centro(plano_id):
    """Elimina un plano y su archivo físico únicamente si deja de estar referenciado."""
    plano = get_plano_centro(plano_id)
    if not plano:
        return False
    ruta = plano[3]
    crear_backup_db()
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("UPDATE detectores SET plano_centro_id=NULL, punto_x=-1, punto_y=-1 WHERE plano_centro_id=?", (plano_id,))
    c.execute("DELETE FROM planos_centro WHERE id=?", (plano_id,))
    conn.commit()
    conn.close()
    if ruta and not archivo_referenciado_por_otro_registro(ruta, excluir_plano_id=plano_id):
        try:
            if os.path.isfile(ruta):
                os.remove(ruta)
        except OSError:
...
def _propietario_actual():
    """Identificador estable de la clave autenticada que crea el centro."""
...
def usuario_puede_acceder_centro(centro_id):
    """Comprueba el permiso también para accesos mediante URL directa."""
    if not centro_id:
        return False
    if _es_admin_actual():
        return True
...
def crear_centro(nombre, zona="", tipo_centro="", propietario=None):
    ...
    conn.commit()
    conn.close()
    # Crear inmediatamente la carpeta física del usuario/centro y su snapshot.
    try:
        crear_centro_dir(rowid)
        guardar_snapshot_centro(rowid)
    except Exception:
        pass
    return rowid
...
def fetch_centros():
    ...
    if _es_admin_actual():
        ...
        c.execute(
            "SELECT id, nombre, zona, fecha_medicion, imagen_exterior_path "
            "FROM centros WHERE propietario=? ORDER BY id DESC",
            (_propietario_actual(),),
        )
    rows = c.fetchall()
    conn.close()
...
def get_telefono_centro(centro_id):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT COALESCE(telefono, '') FROM centros WHERE id=?", (centro_id,))
    row = c.fetchone()
    conn.close()
    return (row[0] or "").strip() if row else ""


def set_telefono_centro(centro_id, valor):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("UPDATE centros SET telefono=? WHERE id=?", ((valor or "").strip(), centro_id))
    conn.commit()
    conn.close()


def get_codigo_centro_zona(centro_id):
    """Devuelve el prefijo Centro/Área-Zona (p. ej. CARR) normalizado."""
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT COALESCE(codigo_centro_zona, '') FROM centros WHERE id=?", (centro_id,))
    row = c.fetchone()
    conn.close()
    return (row[0] or "").strip().upper() if row else ""


def _normalizar_codigo_centro_zona(valor):
    # Evita barras duplicadas y mantiene un prefijo corto y legible.
    return (valor or "").strip().upper().strip("/")


def _aplicar_prefijo_codigo_sala(codigo_sala, prefijo_nuevo, prefijo_anterior=""):
    base = (codigo_sala or "").strip()
    anterior = _normalizar_codigo_centro_zona(prefijo_anterior)
    nuevo = _normalizar_codigo_centro_zona(prefijo_nuevo)
    if anterior and base.upper().startswith(anterior + "/"):
        base = base[len(anterior)+1:]
    # Si ya tiene el nuevo prefijo, no duplicarlo.
    if nuevo and base.upper().startswith(nuevo + "/"):
        return base
    return f"{nuevo}/{base}" if nuevo and base else (nuevo if nuevo else base)


def get_modelo_informe(centro_id):
    """Modelo de informe final seleccionado para este centro."""
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT COALESCE(modelo_informe, 'sscc') FROM centros WHERE id=?", (centro_id,))
    row = c.fetchone()
    conn.close()
    modelo = (row[0] or "sscc").strip().lower() if row else "sscc"
    return modelo if modelo in {"sscc", "is47", "is47b"} else "sscc"


def set_modelo_informe(centro_id, modelo):
    """Guarda el modelo de informe del centro sin alterar el resto de sus datos."""
    modelo = (modelo or "sscc").strip().lower()
    if modelo not in {"sscc", "is47", "is47b"}:
        modelo = "sscc"
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("UPDATE centros SET modelo_informe=? WHERE id=?", (modelo, centro_id))
    conn.commit()
    conn.close()


def set_codigo_centro_zona(centro_id, valor):
    """Guarda el prefijo y sincroniza los códigos de sala ya existentes del centro."""
    nuevo = _normalizar_codigo_centro_zona(valor)
    anterior = get_codigo_centro_zona(centro_id)
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT id, COALESCE(codigo_sala, '') FROM detectores WHERE centro_id=?", (centro_id,))
    for det_id, codigo in c.fetchall():
        actualizado = _aplicar_prefijo_codigo_sala(codigo, nuevo, anterior)
        c.execute("UPDATE detectores SET codigo_sala=? WHERE id=?", (actualizado, det_id))
    c.execute("UPDATE centros SET codigo_centro_zona=? WHERE id=?", (nuevo, centro_id))
    conn.commit()
    conn.close()
...
def update_centro(centro_id, nombre, zona, fecha, imagen_path, direccion=""):
    """Actualiza el centro y mantiene sincronizado el nombre de su carpeta física."""
    centro_anterior = get_centro(centro_id)
    nombre_anterior = centro_anterior[1] if centro_anterior else nombre
    dir_anterior = obtener_ruta_carpeta_centro(centro_id, nombre_anterior)
    dir_nuevo = obtener_ruta_carpeta_centro(centro_id, nombre)

    # Si cambia el nombre, renombrar la carpeta conservando ID_NOMBRE_CENTRO.
    if dir_anterior != dir_nuevo and os.path.isdir(dir_anterior):
        if not os.path.exists(dir_nuevo):
            os.rename(dir_anterior, dir_nuevo)
            prefijo_ant = dir_anterior + os.sep
            prefijo_nuevo = dir_nuevo + os.sep
            conn_r = get_db_connection()
            cr = conn_r.cursor()
            cr.execute("UPDATE centros SET imagen_exterior_path = REPLACE(imagen_exterior_path, ?, ?) WHERE id=?", (prefijo_ant, prefijo_nuevo, centro_id))
            cr.execute("UPDATE detectores SET plano_path=REPLACE(plano_path, ?, ?), foto_situacion_path=REPLACE(foto_situacion_path, ?, ?), foto_detector_path=REPLACE(foto_detector_path, ?, ?) WHERE centro_id=?", (prefijo_ant, prefijo_nuevo, prefijo_ant, prefijo_nuevo, prefijo_ant, prefijo_nuevo, centro_id))
            cr.execute("UPDATE planos_centro SET ruta=REPLACE(ruta, ?, ?) WHERE centro_id=?", (prefijo_ant, prefijo_nuevo, centro_id))
            conn_r.commit()
            conn_r.close()
            if imagen_path and imagen_path.startswith(prefijo_ant):
                imagen_path = prefijo_nuevo + imagen_path[len(prefijo_ant):]
...
def get_datos_informe_centro(centro_id):
    ...
    c.execute(
        ...
        "fecha_comunicacion_trab, medio_comunicacion, condiciones_edificio, factor_k FROM centros WHERE id=?",
        (centro_id,),
    )
    row = c.fetchone()
    conn.close()
...
def delete_centro(centro_id):
    """Elimina un centro de forma transaccional y limpia después sus archivos."""
    centro = get_centro(centro_id)
    if not centro:
        return False
    nombre_centro = centro[1]

    # El backup se realiza antes de cualquier borrado. Después, toda la parte
    # SQLite se confirma en una única transacción. La carpeta física solo se
    # elimina cuando la base de datos ya ha quedado coherente.
    crear_backup_db()
    conn = get_db_connection()
    try:
        c = conn.cursor()
        ...
        c.execute("DELETE FROM planos_centro WHERE centro_id=?", (centro_id,))
        c.execute("DELETE FROM categorias_centro WHERE centro_id=?", (centro_id,))
    ...
    # A partir de aquí el centro ya no existe en SQLite. La limpieza física es
    # deliberadamente posterior y de mejor esfuerzo: si fallase, quedaría como
    # mucho una carpeta huérfana, nunca registros vivos apuntando a archivos
    # que ya han sido eliminados.
    try:
        limpiar_tmp_centro(centro_id)
    except Exception:
        pass
    try:
        eliminar_carpeta_centro(centro_id, nombre_centro)
    except Exception:
        pass
    return True


def existe_codigo_detector_activo(codigo, excluir_detector_id=None):
    """Comprueba si un código ya pertenece a otro detector actualmente existente."""
    ...
    if excluir_detector_id is None:
        c.execute(
            """SELECT id FROM detectores
               WHERE UPPER(TRIM(detector_codigo))=?
               LIMIT 1""",
            (codigo,),
        )
    else:
        c.execute(
            """SELECT id FROM detectores
               WHERE UPPER(TRIM(detector_codigo))=? AND id<>?
               LIMIT 1""",
            (codigo, excluir_detector_id),
        )
...
def actualizar_retirada_detector(
    detector_id, fecha_retirada_real, hora_retirada_real, tecnico_retirada=""
):
    """Guarda los datos reales de retirada y, si procede, el técnico distinto."""
    conn = get_db_connection()
    c = conn.cursor()
    c.execute(
        """UPDATE detectores
           SET fecha_retirada_real=?, hora_retirada_real=?, tecnico_retirada=?
           WHERE id=?""",
        (
            fecha_retirada_real,
            hora_retirada_real,
            str(tecnico_retirada or "").strip(),
            detector_id,
        ),
    )
    conn.commit()
...
def _generar_codigo_sala(cid, detector_id, nivel, zona, tipo_centro=None):
    ...
    hermanos = [d for d in fetch_detectores(cid) if d[16] == nivel]
    ids_hermanos = [d[0] for d in hermanos]
    if detector_id and detector_id in ids_hermanos:
        correlativo = ids_hermanos.index(detector_id) + 1
    else:
        correlativo = len(ids_hermanos) + 1
    codigo_base = f"{prefijo}/{nivel_code}/{correlativo:02d}"
    return _aplicar_prefijo_codigo_sala(codigo_base, get_codigo_centro_zona(cid))


def delete_detector(detector_id):
    """Elimina detector, fotos propias y temporales; protege archivos compartidos."""
    detector = get_detector(detector_id)
    if not detector:
        return False
    crear_backup_db()
    # plano_path (6), foto_situacion (9), foto_detector (10)
    rutas = [detector[i] for i in (6, 9, 10) if i < len(detector) and detector[i]]
    conn = get_db_connection()
    c = conn.cursor()
    ...
    for ruta in set(rutas):
        if not archivo_referenciado_por_otro_registro(ruta, excluir_detector_id=detector_id):
            try:
                if os.path.isfile(ruta):
                    os.remove(ruta)
            except OSError:
                pass
    limpiar_tmp_detector(detector_id)
    return True
...
def get_nombre_laboratorio():
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT COALESCE(nombre_laboratorio, '') FROM settings WHERE id=1")
    row = c.fetchone()
    conn.close()
    return (row[0] or "").strip() if row else ""


def set_nombre_laboratorio(valor):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("UPDATE settings SET nombre_laboratorio=? WHERE id=1", ((valor or "").strip(),))
    conn.commit()
    conn.close()
...
def _mostrar_previsualizacion(nombre_archivo, contenido_bytes):
    ...
    if extension in ("png", "jpg", "jpeg"):
        if not _miniatura_ampliable(contenido_bytes, ancho=160, alt=nombre_archivo):
        ...
        try:
            ...
            _miniatura_pdf_bytes = miniatura.tobytes("png")
            if not _miniatura_ampliable(_miniatura_pdf_bytes, ancho=160, alt=nombre_archivo):
                st.image(_miniatura_pdf_bytes, width=160)
...
def _widget_archivo_con_eliminar(clave, etiqueta_subida, tipos, valor_por_defecto=None, texto_eliminar="❌ Eliminar"):
    ...
    if actual:
        ...
        if st.button(texto_eliminar, key=f"{clave}_eliminar"):
...
def _acordeon_informe(numero, titulo, completo):
    ...
    # En los desplegables del informe no se cambia ningún color adicional:
    # si falta algún dato, únicamente se añade el punto rojo al título.
    if not completo:
        etiqueta = f"🔴 {etiqueta}"
...
LOGO_PNG_B64 = (
    ...
    "zHPfivirkgfWtXDfucvj7JZb/VgVIMsaEGvPypkf/7H69IIAgcbl1koQa/Mm99DS/DNtKf/p/tWNGsOpkk8UaHze0aNKb7XO/D2qsgp0Z/WfBf4ctSA/fhbb2H/+zc6ML4L1QodiXPBLAQS0llNe6SNCCAOqtxPCPcOnh01LSwABLZUcmC+rF9BqGfxEcMIuM2mobbRN"
    ...
    "zjhLI8OrAIHk5llYPCTx8rIhJTbqSZ4EEKyOGCs7KChrNQMIBzXXbwDEKfIuO8Xmbz4CxJEheV1nbRv8/2MYxV18sqVjm8MNgKi2EbTPPsYcINCzPsJ9BEj+8aFi8taevLsAAjm7YyDCcwI7R8KGHRPUVJpA5RTwcL71ESMeY5Np7gejT0+0/WidmNVZhXh3IVVlbqcs"
    ...
    "TB6Em4YyQNTipflrV7Pl3gIg2J2HpARa92fDg+Nf7jDxiYnAQ/iBBBUq5D7zV4JIDQZ2LG7nbvyTbCKjl1HSIbVhjYZRbgKEOzMXt7rqBZCSxvzQI4fFSBvABDd9gTHGHvdBHFtSE0TQYv3PntAwD2nRo8IjpeC4/aF0KeM0YJVkVQPhji970ZsdNEdlu2awMm2s0kY"
    ...
    "I2WwMzYBorFttioBBOIsOFtcXQ8Q0NMFswoBwpcPinsCZAposEaH6lcsl+TRozh24t0X+xh8XHIYFhvr8MQtAFlUe3wJINjh8XUB8oU03rTF3fsUEkIXMBiVdIEgUFwN27zNo6CiYeuoCpCfBBDvtErzSil4e0qzhDhtra17F5wQ2cNgbBQAYjOA8DJA4DqNQJsBpMVW"
    ...
    "lwSIngEEmt2M1zSl66vBVi1QZm6hJqtYjN+6zYyZUgtOuY/prgABewiSQZIbcs8WXQHybgBxCgkGLsluT8AM+lNf78KiZRcWueTCEvrrQ6CG0Im6MFRqnOOxzPXyGnvHWKllrSbbnKrr3vMwHjcUVg2/xp5phfnLWTKaZJpscrmQ9CUskONvAsjHHCBj49sgYqNhvfLd"
...
def generar_pdf(centro_id, output_path=None):
    try:
        import base64
        import io
        ...
        if not output_path:
            nombre_archivo = _nombre_documento(nombre or "centro", "INFORME-COLOCACIÓN") + ".pdf"
            output_path = guardar_archivo_centro(centro_id, b"", nombre_archivo, subdir="informes")
        ...
        for idx, d in enumerate(detectores, 1):
            (did, _, planta, sala, fecha_det, codigo, _plano_antiguo, px, py, foto_sit, foto_det, _,
             ...
             resultado_bq_m3, incertidumbre, *_campos_detector_extra) = d
            plano = None
                ...
                try:
                    with PILImage.open(plano) as im_plano:
                        ...
                        if px is not None and py is not None and px >= 0 and py >= 0:
                            ...
                            draw.ellipse([cx - r, cy - r, cx + r, cy + r],
                                         fill=(220, 20, 20), outline=(120, 0, 0), width=2)
                        tmp_plano_path = os.path.join(get_tmp_dir(), f"_tmp_plano_{did}.jpg")
...
# ============================================================
# CÁLCULO DE LA FECHA DE RETIRADA ÓPTIMA
# ============================================================
...
def importar_centro_desde_excel(archivo_bytes):
    ...
    # Copia de seguridad antes de incorporar datos nuevos a la base existente.
    crear_backup_db()
    ...
    if "Planos" in wb.sheetnames:
        ...
        # Recuperar también los datos del informe guardados en el Excel.
        datos_informe_importados = {}
        etiquetas_informe = {
            ...
            "Condiciones del edificio durante la exposición": "condiciones_edificio",
            "Factor k": "factor_k",
        }
        for fila_meta in range(3, min(ws_p.max_row, 8) + 1):
            etiqueta_meta = ws_p.cell(row=fila_meta, column=1).value
            valor_meta = ws_p.cell(row=fila_meta, column=2).value
            campo_meta = etiquetas_informe.get(str(etiqueta_meta).strip() if etiqueta_meta is not None else "")
            if campo_meta:
                datos_informe_importados[campo_meta] = "" if valor_meta is None else str(valor_meta)
        ...
        for fila_p in range(5, ws_p.max_row + 1):
            ...
            if not datos_img:
                continue
            ruta_guardada = guardar_bytes_imagen(datos_img, f"plano_centro_{centro_id}", centro_id=centro_id, subdir="planos")
    ...
    for fila_actual in range(header_row + 1, ws.max_row + 1):
        ...
        if col_situacion:
            ...
            if datos:
                foto_sit_path = guardar_bytes_imagen(datos, "foto_situacion", centro_id=centro_id, subdir="fotos_situacion")
        if col_detector_foto:
            datos = imagenes_por_celda.get((fila_actual - 1, col_detector_foto - 1))
            if datos:
                foto_det_path = guardar_bytes_imagen(datos, "foto_detector", centro_id=centro_id, subdir="fotos_detector")
...
def generar_excel(centro_id, output_path):
    ...
    col_idx0 = {h: i for i, h in enumerate(headers)}
    ...
    ws.column_dimensions[col_letra["Técnico de retirada"]].width = 22
    ...
    for d in detectores:
        (did, _, planta, sala, fecha_det, codigo, _plano_antiguo, punto_x, punto_y,
         ...
         resultado_bq_m3_d, incertidumbre_d, *_campos_detector_extra) = d
        tecnico_retirada_d = (
            str(_campos_detector_extra[0] or "").strip()
            if _campos_detector_extra
            else ""
        )
        plano_p = None
        nombre_plano_d = ""
            ...
            if ruta and os.path.exists(ruta):
                try:
                    with Image.open(ruta) as im_orig:
                        ...
                        # El Excel actúa también como copia transportable de las
                        # fotografías. Para evitar una pérdida acumulativa de calidad
                        # al exportar -> importar -> volver a exportar, conservamos
                        # las fotos de situación y detector con alta resolución.
                        # El tamaño visual dentro de la celda sigue siendo pequeño:
                        # XLImage.width/height solo cambia cómo se muestra, no los
                        # píxeles realmente incrustados en el archivo.
                        if col_name in ("Foto situación", "Foto detector"):
                            im.thumbnail((2000, 1600))
                            _calidad_excel = 94
                        else:
                            # El plano de esta hoja es solo una vista previa con el
                            # punto marcado; los planos completos se conservan aparte
                            # en la hoja "Planos".
                        ...
                        im.save(
                            buf,
                            format="JPEG",
                            quality=_calidad_excel,
                            subsampling=0 if col_name in ("Foto situación", "Foto detector") else -1,
                        )
                        buf.seek(0)
    ...
    filas_datos_informe = [
        ...
        ("Condiciones del edificio durante la exposición", datos_informe_xl["condiciones_edificio"]),
        ("Factor k", datos_informe_xl["factor_k"]),
    ]
...
def generar_registro_laboratorio(centro_id, output_path, tipo_firma="digital"):
    ...
    detectores = fetch_detectores(centro_id)
    empresa = get_empresa()
    condiciones_edificio = get_datos_informe_centro(centro_id).get("condiciones_edificio", "")
    ...
    def _construir_caja_cabecera():
        ...
        filas_bloque_derecho = [
            ...
            [Paragraph(
                ...
                + _escapar_pdf(condiciones_edificio or ""),
                estilo_celda_izq,
            ), ""],
        ]
    ...
    for i in range(n_filas_totales):
        ...
        if i < len(detectores):
            ...
            tecnico_retirada_d = d[22] if len(d) > 22 else ""
            ...
            fila = [
                ...
                (
                    Paragraph(
                        f"{_escapar_pdf(tecnico_retirada_d)}<br/>* Ver firma digital abajo",
                        estilo_firma_celda,
                    )
                    if tipo_firma == "digital" and str(tecnico_retirada_d or "").strip()
                    else celda_firma_tecnico
                ),
            ]
        else:
            fila = [numero_fila] + [""] * 12
...
def _crear_componente_foto_pantalla_completa():
    """Cámara adaptativa: nativa en iPhone/iPad y visor propio en Android/otros."""
    html = """
    <div class="photo-camera">
      <label class="open-photo native-photo-label" hidden>
        📷 Abrir cámara iPhone
        <input class="native-photo-input"
               type="file"
               accept="image/*"
               capture="environment"
               hidden>
      ...
      <button class="open-photo web-photo-button" type="button">📷 Abrir cámara Android</button>
    ...
    .open-photo{
      width:100%;box-sizing:border-box;background:#111;color:#f5ad22;
      border:1px solid #f5ad22;border-radius:10px;padding:.65rem .85rem;
      font-size:1rem;cursor:pointer;text-align:center;font-weight:600;
    }
    .native-photo-label{display:block}
    .photo-camera [hidden]{display:none !important}
    ...
    .photo-actions{position:absolute;left:0;right:0;
      bottom:max(22px,env(safe-area-inset-bottom));z-index:2;display:flex;
      justify-content:center;gap:18px;padding:12px}
    ...
    export default function(component){
      ...
      const webBtn=root.querySelector('.web-photo-button');
      const nativeLabel=root.querySelector('.native-photo-label');
      const nativeInput=root.querySelector('.native-photo-input');
      ...
      const stop=()=>{
        ...
        video.srcObject=null;overlay.hidden=true;webBtn.disabled=false;
      };
      ...
      const procesarArchivoNativo=(file)=>{
        if(!file)return;
        const reader=new FileReader();
        reader.onload=()=>{
          if(typeof reader.result==='string'){
            setTriggerValue('captured',reader.result);
            nativeInput.value='';
          }
        };
        reader.onerror=()=>{
          setTriggerValue('camera_error','No se pudo leer la fotografía tomada.');
          nativeInput.value='';
        };
        reader.readAsDataURL(file);
      };

      nativeInput.onchange=()=>procesarArchivoNativo(
        nativeInput.files && nativeInput.files.length ? nativeInput.files[0] : null
      );
      webBtn.onclick=startWeb;
      takeBtn.onclick=captureWeb;
      cancelBtn.onclick=stop;
      return()=>stop();
    }
    """
    return st.components.v2.component(
        "radon_adaptive_photo_camera", html=html, css=css, js=js, isolate_styles=True
    )
...
def _captura_foto_pantalla_completa(key_prefix):
    try:
        ...
        st.error("Esta versión requiere Streamlit 1.62 o superior para la cámara.")
        return None
...
def _miniatura_ampliable(imagen, ancho=160, caption=None, alt="Imagen"):
    """Muestra una miniatura que se amplía a pantalla completa al tocarla.

    Acepta una ruta, bytes/BytesIO o una imagen PIL. La ampliación usa el mismo
    comportamiento que las fotografías del detector y no altera el archivo
    original.
    """
    try:
        _datos = None
        ...
        if isinstance(imagen, (str, os.PathLike)):
            _ruta = os.fspath(imagen)
            if not os.path.exists(_ruta):
                return False
            with open(_ruta, "rb") as _f:
                _datos = _f.read()
            _ext = os.path.splitext(_ruta)[1].lower()
            _mime = (
                "image/png" if _ext == ".png"
                else "image/webp" if _ext == ".webp"
                else "image/gif" if _ext == ".gif"
                else "image/jpeg"
            )
        elif isinstance(imagen, (bytes, bytearray)):
            _datos = bytes(imagen)
        elif hasattr(imagen, "getvalue"):
            _datos = imagen.getvalue()
        else:
            try:
                _buf = io.BytesIO()
                imagen.save(_buf, format="PNG")
                _datos = _buf.getvalue()
                _mime = "image/png"
            except Exception:
                return False
        ...
        # Detectar PNG por firma cuando llegan bytes sin extensión conocida.
        if _datos[:8] == b"\x89PNG\r\n\x1a\n":
            _mime = "image/png"
        elif _datos[:4] in (b"RIFF",):
            _mime = "image/webp"

        _b64 = base64.b64encode(_datos).decode("ascii")
        _ancho = max(40, int(ancho or 160))
        _alt = html.escape(str(alt or "Imagen"))
        _caption_html = ""
        _altura_caption = 0
        if caption:
            _caption_html = (
                '<div class="radon-mini-caption">'
        ...
        _html = f"""
        <div class="radon-mini-wrap">
          <img class="radon-mini-thumb"
               src="data:{_mime};base64,{_b64}"
               alt="{_alt}" title="Toca para ampliar" />
          {_caption_html}
          <div class="radon-mini-modal" aria-hidden="true">
            <button class="radon-mini-close" type="button" aria-label="Cerrar">×</button>
            <img class="radon-mini-large" src="data:{_mime};base64,{_b64}" alt="{_alt}" />
          </div>
        </div>
        <style>
          .radon-mini-wrap {{ width:100%; }}
          .radon-mini-thumb {{
            width:min({_ancho}px, 92vw); height:auto; max-height:220px;
            object-fit:contain; cursor:zoom-in; display:block;
          }}
          .radon-mini-caption {{
            margin-top:5px; font-size:13px; line-height:1.25;
            color:inherit; max-width:min({_ancho}px, 92vw);
          }}
          .radon-mini-modal {{
          ...
          .radon-mini-modal.open {{ display:flex; }}
          .radon-mini-large {{
          ...
          .radon-mini-close {{
            ...
            width:46px; height:46px; border-radius:50%; border:1px solid #fff;
            background:rgba(0,0,0,.60); color:#fff; font-size:34px;
        ...
        <script>
          (() => {{
            ...
            const thumb = root.querySelector('.radon-mini-thumb');
            const modalLocal = root.querySelector('.radon-mini-modal');
            const closeLocal = root.querySelector('.radon-mini-close');
            const src = thumb.getAttribute('src');

            const cerrarLocal = () => {{
              modalLocal.classList.remove('open');
              modalLocal.setAttribute('aria-hidden','true');
            }};

            const abrir = () => {{
              try {{
                const pd = window.parent.document;
                const anterior = pd.getElementById('radon-photo-fullscreen-overlay');
                if (anterior) anterior.remove();

                const overlay = pd.createElement('div');
                overlay.id = 'radon-photo-fullscreen-overlay';
                overlay.style.cssText = [
                  'position:fixed','inset:0','width:100vw','height:100vh','height:100dvh',
                  'z-index:2147483647','background:rgba(0,0,0,.92)','display:flex',
                  'align-items:center','justify-content:center','padding:14px','box-sizing:border-box'
                ].join(';');

                const img = pd.createElement('img');
                img.src = src;
                img.alt = thumb.alt || 'Imagen';
                img.style.cssText = [
                  'max-width:96vw','max-height:94vh','max-height:94dvh',
                  'width:auto','height:auto','object-fit:contain','display:block'
                ].join(';');

                const cerrar = pd.createElement('button');
                cerrar.type = 'button';
                cerrar.textContent = '×';
                ...
                cerrar.style.cssText = [
                  ...
                  'border:1px solid #fff','background:rgba(0,0,0,.60)','color:#fff',
                  'font-size:34px','line-height:40px','cursor:pointer'
                ].join(';');

                const teclado = (e) => {{ if (e.key === 'Escape') cerrarOverlay(); }};
                const cerrarOverlay = () => {{
                  try {{ overlay.remove(); }} catch (_) {{}}
                  try {{ pd.removeEventListener('keydown', teclado, true); }} catch (_) {{}}
                }};

                cerrar.addEventListener('click', (e) => {{ e.stopPropagation(); cerrarOverlay(); }});
                overlay.addEventListener('click', (e) => {{ if (e.target === overlay) cerrarOverlay(); }});
                pd.addEventListener('keydown', teclado, true);
                overlay.appendChild(img);
                overlay.appendChild(cerrar);
                pd.body.appendChild(overlay);
              }} catch (_) {{
                modalLocal.classList.add('open');
                modalLocal.setAttribute('aria-hidden','false');
              }}
            }};

            thumb.addEventListener('click', abrir);
            closeLocal.addEventListener('click', cerrarLocal);
            modalLocal.addEventListener('click', (e) => {{ if (e.target === modalLocal) cerrarLocal(); }});
            document.addEventListener('keydown', (e) => {{ if (e.key === 'Escape') cerrarLocal(); }});
          }})();
        </script>
        """
        components.html(_html, height=max(60, _ancho + _altura_caption + 8), scrolling=False)
        return True
    except Exception:
        return False

def widget_imagen(label, state_key, key_prefix, con_camara=True, ancho_miniatura=None,
                   tab_por_defecto="subir", titulo_amarillo=False, centro_id=None, subdir="fotos"):
    if titulo_amarillo:
    ...
    if nuevo_bytes is not None:
        path = guardar_bytes_imagen(nuevo_bytes, key_prefix, nueva_ext, centro_id=centro_id, subdir=subdir)
    ...
    if path_actual and os.path.exists(path_actual):
        # En la ficha del detector mostramos la foto como miniatura.
        # Al tocarla/clicarla se abre una vista grande superpuesta y se puede
        # cerrar tocando fuera de la imagen, sobre la X o pulsando Escape.
        try:
            with open(path_actual, "rb") as _f_img:
                _img_bytes = _f_img.read()
            _ext_img = os.path.splitext(path_actual)[1].lower()
            _mime_img = (
                "image/png" if _ext_img == ".png"
                else "image/webp" if _ext_img == ".webp"
                else "image/jpeg"
            )
            _img_b64 = base64.b64encode(_img_bytes).decode("ascii")
            _thumb_w = int(ancho_miniatura or 150)
            _html_img = f"""
            <div class="radon-photo-wrap">
              <img class="radon-photo-thumb"
                   src="data:{_mime_img};base64,{_img_b64}"
                   alt="{html.escape(str(label))}"
                   title="Toca para ampliar" />
              <div class="radon-photo-modal" aria-hidden="true">
                <button class="radon-photo-close" type="button" aria-label="Cerrar">×</button>
                <img class="radon-photo-large"
                     src="data:{_mime_img};base64,{_img_b64}"
                     alt="{html.escape(str(label))}" />
              </div>
            </div>
            <style>
              .radon-photo-wrap {{ width:100%; }}
              .radon-photo-thumb {{
                width:min({_thumb_w}px, 48vw); height:auto; max-height:170px;
                object-fit:cover; border-radius:10px; cursor:zoom-in;
                border:1px solid rgba(255,255,255,.35); display:block;
              }}
              .radon-photo-modal {{
                display:none; position:fixed; inset:0; z-index:2147483647;
              ...
              .radon-photo-modal.open {{ display:flex; }}
              .radon-photo-large {{
              ...
              .radon-photo-close {{
                position:fixed; top:14px; right:18px; z-index:2147483647;
                width:46px; height:46px; border-radius:50%; border:1px solid #fff;
                background:rgba(0,0,0,.55); color:#fff; font-size:34px;
                line-height:38px; cursor:pointer;
              }}
            </style>
            <script>
              (() => {{
                const root = document.currentScript.previousElementSibling.previousElementSibling;
                const thumb = root.querySelector('.radon-photo-thumb');
                const modalLocal = root.querySelector('.radon-photo-modal');
                const closeLocal = root.querySelector('.radon-photo-close');
                const src = thumb.getAttribute('src');
                ...
                const abrirEnPantalla = () => {{
                  try {{
                    const pd = window.parent.document;

                    // Evitar que quede una ampliación anterior abierta.
                    const anterior = pd.getElementById('radon-photo-fullscreen-overlay');
                    if (anterior) anterior.remove();
                    ...
                    // Respaldo si el navegador impide acceder al documento padre.
                    modalLocal.classList.add('open');
                    modalLocal.setAttribute('aria-hidden','false');
                  }}
                }};

                thumb.addEventListener('click', abrirEnPantalla);
                closeLocal.addEventListener('click', cerrarLocal);
                modalLocal.addEventListener('click', (e) => {{
                  if (e.target === modalLocal) cerrarLocal();
                }});
                document.addEventListener('keydown', (e) => {{
                  if (e.key === 'Escape') cerrarLocal();
                }});
              }})();
            </script>
            """
            components.html(_html_img, height=max(120, min(190, _thumb_w + 20)), scrolling=False)
        except Exception:
            # Respaldo: si el navegador/componente no pudiera mostrar la
            # miniatura ampliable, mantenemos la visualización normal.
            st.image(path_actual, width=(ancho_miniatura or 150))
    else:
...
def _ruta_preview_plano(plano_path):
    """Ruta estable de la copia ligera usada solo en la ficha del detector."""
    raiz, _ext = os.path.splitext(plano_path)
    return raiz + "__preview_detector.jpg"


def _obtener_preview_plano(plano_path, lado_maximo=1200):
    """Crea una sola vez una copia ligera del plano para la interfaz.

    El archivo original no se modifica y sigue siendo el utilizado por Excel,
    informes y generación de planos. La miniatura mantiene exactamente la misma
    proporción, por lo que las coordenadas normalizadas (0..1) son equivalentes.
    """
    if not (plano_path and os.path.exists(plano_path)):
        return None

    preview_path = _ruta_preview_plano(plano_path)
    try:
        # Se regenera si falta o si el original ha cambiado posteriormente.
        if (
            os.path.exists(preview_path)
            and os.path.getmtime(preview_path) >= os.path.getmtime(plano_path)
        ):
            return preview_path

        with Image.open(plano_path) as _im_original:
            _im_preview = ImageOps.exif_transpose(_im_original).convert("RGB")
            _im_preview.thumbnail(
                (int(lado_maximo), int(lado_maximo)),
        ...
        # Si por cualquier motivo no puede crearse la copia, se mantiene el
        # comportamiento anterior usando el original.
        return plano_path


def _crear_componente_plano_interactivo():
    """Componente V2 ligero para marcar un punto directamente sobre el plano."""
    html = """
    <div class="radon-plan-wrap">
      <div class="radon-plan-stage">
        <img class="radon-plan-img" alt="Plano para marcar la ubicación del detector">
        <div class="radon-plan-dot" hidden></div>
      </div>
    </div>
    """
    ...
    export default function(component) {
      ...
      const wrap = parentElement.querySelector('.radon-plan-wrap');
      const stage = parentElement.querySelector('.radon-plan-stage');
      const img = parentElement.querySelector('.radon-plan-img');
      const dot = parentElement.querySelector('.radon-plan-dot');
      ...
      const maxWidth = Math.max(1, Number(data.max_width || 680));
      // Mantiene el ancho de trabajo del plano aunque la pantalla sea más
      // estrecha. El contenedor permite desplazarse lateralmente.
      img.style.width = maxWidth + 'px';
      img.style.maxWidth = 'none';

      const px = Number(data.px);
      const py = Number(data.py);
      const hasPoint =
      ...
      // Se asigna siempre la imagen recibida en esta ejecución. El navegador
      // puede cachear el data URI, por lo que no depende de una segunda carga
      // desde el servidor ni de un iframe de terceros.
      img.onload = placeDot;
      img.src = data.src;
      if (img.complete) placeDot();

      // En móvil hay que distinguir un toque de un gesto de desplazamiento.
      // Un arrastre lateral mueve el plano; solo un toque breve marca el punto.
      let downX = null;
      let downY = null;
      let moved = false;
      ...
      img.onpointerup = (ev) => {
        if (downX === null || downY === null) return;
        const fueArrastre = moved ||
          Math.hypot(ev.clientX - downX, ev.clientY - downY) > 8;
        downX = downY = null;
        moved = false;
        ...
        setTriggerValue('point', {x: nx, y: ny});
      };
    }
    """

    return st.components.v2.component(
        "radon_clickable_plan",
        html=html,
        css=css,
        js=js,
        isolate_styles=True,
...
@st.cache_resource(show_spinner=False)
def _componente_plano_interactivo():
    return _crear_componente_plano_interactivo()


def _imagen_pil_a_data_uri(img, quality=86):
    """Serializa la vista ligera del plano para enviarla una sola vez al navegador."""
    _buf = io.BytesIO()
...
def widget_seleccionar_plano_y_punto(cid, ns):
    ...
    # La ficha trabaja con una copia ligera persistente. El original queda
    # intacto para Excel, informes y Anexo II.
    plano_preview_path = _obtener_preview_plano(plano_path, lado_maximo=1200)
    try:
        with Image.open(plano_preview_path) as _img_preview_archivo:
            img_orig = _img_preview_archivo.convert("RGB")
    except Exception:
    ...
    if (disp_w, disp_h) == img_orig.size:
    ...
    # El plano que se envía al componente NO lleva el punto dibujado.
    # El punto rojo se representa una sola vez mediante el overlay HTML del
    # componente. Así evitamos que aparezcan dos puntos simultáneamente.
    img_preview = img_disp.copy()

    # Componente interactivo propio de Streamlit V2. Sustituye al componente
    # externo streamlit-image-coordinates, que en algunos móviles/navegadores
    # podía quedar en blanco al abrir o cambiar rápidamente de detector.
    st.caption("Toca sobre el plano para marcar la ubicación del detector")
    try:
        _plan_component = _componente_plano_interactivo()
        _plan_data_uri = _imagen_pil_a_data_uri(img_preview, quality=86)
        _plan_result = _plan_component(
            data={
                "src": _plan_data_uri,
                "px": px,
                "py": py,
            ...
            key=(
                ns + "_plano_v2_" + str(plano_id_actual)
                + "_" + str(st.session_state.get(f"_plano_render_nonce_{cid}", 0))
            ),
            on_point_change=lambda: None,
        )

        _point = getattr(_plan_result, "point", None)
        if _point:
            try:
                nuevo_px = max(0.0, min(1.0, float(_point.get("x"))))
                nuevo_py = max(0.0, min(1.0, float(_point.get("y"))))
            except Exception:
                nuevo_px = nuevo_py = None

            if (
                nuevo_px is not None
                and nuevo_py is not None
                and (px, py) != (nuevo_px, nuevo_py)
            ):
                st.session_state[px_key] = nuevo_px
                st.session_state[py_key] = nuevo_py
                st.rerun()
    except Exception:
        # Respaldo visual: si el navegador no soportase el componente V2,
        # el plano nunca desaparece por completo.
        st.image(img_preview, use_container_width=True)
        st.warning("No se pudo activar el marcado interactivo del plano. Recarga la ficha del detector.")
...
# ============================================================
# FUNCIONES AUXILIARES PARA DETECTORES
# ============================================================
...
def _inicializar_ns_detector(cid, detector_id, ns):
    ...
    if detector_id:
        ...
        (did, _, planta, sala, fecha, codigo, _plano_antiguo, px, py, foto_sit, foto_det, _,
         codigo_sala, profesionales_sala, hora_colocacion, turno_trabajo, nivel, plano_centro_id, fecha_retirada_real, hora_retirada_real,
         _resultado_ignorado, _incertidumbre_ignorada, *_campos_detector_extra) = d
    else:
...
def _validar_codigo_detector_inmediato(cid, detector_id, ns):
    """Valida el código al introducirlo y libera el campo si ya está usado."""
    key_codigo = ns + "_codigo"
    codigo = str(st.session_state.get(key_codigo, "") or "").strip().upper()

    # Normalizamos siempre a mayúsculas.
    st.session_state[key_codigo] = codigo

    if not codigo:
        return False

    # Al abrir un detector ya existente, su código guardado es válido y no debe
    # volver a comprobarse como si fuese una entrada nueva. Esta comprobación
    # explícita evita falsos avisos de duplicado al entrar desde "detectores
    # colocados", incluso si el ID llega a session_state como texto.
    detector_id_excluir = None
    if detector_id not in (None, "", "nuevo"):
        try:
            detector_id_excluir = int(detector_id)
        except (TypeError, ValueError):
            detector_id_excluir = detector_id
        try:
            detector_actual = get_detector(detector_id_excluir)
        except Exception:
            detector_actual = None
        if detector_actual:
            codigo_guardado = str(detector_actual[5] or "").strip().upper()
            if codigo == codigo_guardado:
                st.session_state.pop(ns + "_codigo_duplicado_mensaje", None)
                return False

    if existe_codigo_detector_activo(codigo, excluir_detector_id=detector_id_excluir):
        st.session_state[ns + "_codigo_duplicado_mensaje"] = (
            f"El código {codigo} ya está asignado a otro detector. "
            "Introduce un código diferente."
        )
        # Se borra automáticamente para que el usuario pueda escribir/leer otro
        # código sin quedar atrapado al intentar guardar o salir.
        st.session_state[key_codigo] = ""
        st.session_state.pop(ns + "_ocr_codigo_mensaje", None)
        return True
...
def _renderizar_campos_detector(cid, detector_id, ns):
    # Un duplicado detectado durante una acción de guardado se borra aquí,
    # antes de crear el widget, evitando StreamlitAPIException.
    if st.session_state.pop(ns + "_borrar_codigo_duplicado", False):
        st.session_state[ns + "_codigo"] = ""
    ...
    with c1:
        ...
        # Si el lector de cámara acaba de introducir un código, se valida antes
        # de crear el widget. Así un duplicado también se borra inmediatamente.
        _validar_codigo_detector_inmediato(cid, detector_id, ns)

        codigo_placeholder.text_input(
            "Código del detector",
            key=ns + "_codigo",
            help="Formato esperado: 2 letras y 4 números, por ejemplo GJ4306.",
            on_change=_validar_codigo_detector_inmediato,
            args=(cid, detector_id, ns),
        )

        # El aviso queda justo debajo del campo del código.
        mensaje_duplicado = st.session_state.get(ns + "_codigo_duplicado_mensaje")
        ...
        st.text_input("Denominación de la planta (Opcional)", key=ns + "_planta")
    with c2:
        st.text_input("Lugar de colocación", key=ns + "_sala")
        st.selectbox(
    ...
    with st.expander("Fotos del detector", expanded=False):
        ...
        with fc1:
            ...
            widget_imagen("Situación del detector", ns + "_foto_sit", key_prefix=ns + "_fsit",
                          tab_por_defecto="camara", titulo_amarillo=True, ancho_miniatura=140,
                          centro_id=cid, subdir="fotos_situacion")
        with fc2:
            st.markdown('<div class="marcador-imagen-exterior"></div>', unsafe_allow_html=True)
            widget_imagen("Detector", ns + "_foto_det", key_prefix=ns + "_fdet",
                          tab_por_defecto="camara", titulo_amarillo=True, ancho_miniatura=140,
                          centro_id=cid, subdir="fotos_detector")
...
def _guardar_detector_desde_ns(cid, detector_id, ns, mostrar_mensajes=True):
    ...
    codigo_val = st.session_state.get(ns + "_codigo", "").strip().upper()
    if not codigo_val:
        if mostrar_mensajes:
            st.warning("El código es obligatorio")
        return None

    # Un código solo puede pertenecer a un detector existente. Al editar un
    # detector se excluye su propio ID, pero se bloquea si el mismo código está
    # guardado en cualquier otro centro o detector. Cuando un detector se borra
    # físicamente de la base de datos, su código vuelve a quedar disponible.
    detector_id_excluir = detector_id
    if detector_id_excluir not in (None, "", "nuevo"):
        try:
            detector_id_excluir = int(detector_id_excluir)
        except (TypeError, ValueError):
            pass
    if existe_codigo_detector_activo(codigo_val, excluir_detector_id=detector_id_excluir):
        st.session_state[ns + "_codigo_duplicado_mensaje"] = (
            f"El código {codigo_val} ya está asignado a otro detector. "
            "Introduce un código diferente."
        )
        # En este punto el text_input ya ha sido creado en esta ejecución.
        # Streamlit no permite modificar directamente su session_state aquí.
        # Dejamos marcado el borrado y forzamos un rerun; al comienzo de la
        # siguiente ejecución se vacía el campo ANTES de crear el widget.
        st.session_state[ns + "_borrar_codigo_duplicado"] = True
        st.rerun()
        return None
    ...
    try:
        ...
        # Segunda barrera frente a duplicados: aunque otra sesión consiga
        # insertar el mismo código entre la comprobación anterior y este
        # guardado, SQLite lo bloquea y devolvemos al usuario a la ficha.
        texto_error = str(exc).lower()
        if "codigo_detector_duplicado" in texto_error or "unique constraint failed" in texto_error:
            st.session_state[ns + "_codigo_duplicado_mensaje"] = (
                f"El código {codigo_val} ya está asignado a otro detector. "
                "Introduce un código diferente."
            )
...
def pantalla_centro_datos():
    ...
    telefono_actual = get_telefono_centro(cid)

    st.markdown('<div class="marcador-btn-azul-claro"></div>', unsafe_allow_html=True)
    volver_clic = st.button("← Guardar y volver")
    ...
    with st.container(border=True):
        ...
        datos_informe_centro_actual = get_datos_informe_centro(cid)
        condiciones_edificio_actual = datos_informe_centro_actual.get("condiciones_edificio", "")

        c1, c2 = st.columns(2)
        with c1:
            st.text_input("Nombre", value=nombre or "", key=ns_centro + "_nombre")
            st.text_input("Área / Zona :", value=zona or "", key=ns_centro + "_zona")
            codigo_centro_zona_actual = get_codigo_centro_zona(cid)
            st.text_input(
                "Código identificador de la medición (opcional)",
                value=codigo_centro_zona_actual,
                key=ns_centro + "_codigo_centro_zona",
                help="Prefijo que se añadirá al código de las salas. Ejemplo: CARR → CARR/AP/00/01.",
            )
            st.text_input("Dirección (domicilio, municipio, provincia, código postal)", value=direccion or "", key=ns_centro + "_direccion")
            st.text_input("Teléfono", value=telefono_actual, key=ns_centro + "_telefono")
            _date_input_texto("Fecha", ns_centro + "_fecha", fecha_valor_original)
            st.text_area(
                "Condiciones del edificio durante la exposición",
                value=condiciones_edificio_actual,
                key=ns_centro + "_condiciones_edificio",
                height=90,
            )
        with c2:
            st.markdown('<div class="marcador-imagen-exterior"></div>', unsafe_allow_html=True)
            widget_imagen(
                ...
                centro_id=cid, subdir="fotos_exterior"
            )

        hay_cambios = (
            st.session_state[ns_centro + "_nombre"] != (nombre or "")
            or st.session_state[ns_centro + "_zona"] != (zona or "")
            or _normalizar_codigo_centro_zona(st.session_state[ns_centro + "_codigo_centro_zona"]) != codigo_centro_zona_actual
            or st.session_state[ns_centro + "_direccion"] != (direccion or "")
            or st.session_state[ns_centro + "_telefono"] != telefono_actual
            or st.session_state[ns_centro + "_fecha"] != fecha_valor_original
            or st.session_state[ns_centro + "_condiciones_edificio"] != condiciones_edificio_actual
        ...
        def _guardar_cambios_centro():
            ...
            set_codigo_centro_zona(
                cid, st.session_state[ns_centro + "_codigo_centro_zona"]
            )
            set_telefono_centro(cid, st.session_state[ns_centro + "_telefono"])
            set_datos_informe_centro(
                cid,
                condiciones_edificio=st.session_state[ns_centro + "_condiciones_edificio"].strip(),
            )
            return True
            ...
            with st.container(border=True):
                ...
                with pcol1:
                    if ruta_plano_c and os.path.exists(ruta_plano_c):
                        if not _miniatura_ampliable(
                            ruta_plano_c, ancho=140, alt=nombre_plano_c
                        ):
...
# ============================================================
# TRADUCCIÓN DE TEXTOS FIJOS EN INGLÉS
# ============================================================
...
def inyectar_traduccion_widgets():
    ...
    (function() {
      ...
      try {
        ...
        // Si el navegador no permite acceder al documento padre, se deja tal cual
      }
    })();
    </script>
    """
...
def pantalla_centro_detectores():
        ...
        if abierto_actual is not None:
            ...
            if es_nuevo and not codigo_nuevo:
                ...
                guardado_ok, ns_final = _guardar_y_actualizar_snapshot_detector(
                    cid, abierto_actual, ns_actual, mostrar_mensajes=True
                )
                if not guardado_ok:
                    return
    ...
    with st.container(border=True):
        ...
        # Mientras se está cubriendo una ficha de "Nuevo detector" ocultamos
        # por completo el desplegable de detectores existentes. Así no se puede
        # cambiar accidentalmente de detector a mitad de la toma de datos.
        # El selector volverá a aparecer al salir de esta pantalla o cuando el
        # usuario pulse de nuevo "Nuevo detector" (momento en el que se guarda
        # automáticamente el actual si ya tiene código).
        ocultar_selector_detectores = st.session_state.get(abierto_key) == "nuevo"

        if not detectores:
            st.info("Todavía no se han añadido detectores para este centro.")
        elif not ocultar_selector_detectores:
    ...
    if abierto is not None:
        ...
        with st.container(border=True):
            ...
            # AUTOGUARDADO DE SEGURIDAD DE LA FICHA DEL DETECTOR.
            # Streamlit actualiza los widgets al interactuar con ellos y, en
            # particular, al perder el foco (por ejemplo al abrir otro de los
            # desplegables/acordeones de esta pantalla). En cada rerun
            # comprobamos si la ficha cambió y la persistimos inmediatamente.
            # De este modo, cuando el usuario pasa de «Profesionales» a
            # «Plano», «Fotos» o «Fecha y hora», lo introducido en el bloque
            # que deja atrás ya queda guardado en la base de datos.
            #
            # En una ficha nueva esperamos a que exista código de detector,
            # porque es el campo obligatorio que identifica el registro.
            if _detector_tiene_cambios(ns):
                _codigo_autoguardado = str(st.session_state.get(ns + "_codigo", "") or "").strip()
                if detector_id_abierto is not None or _codigo_autoguardado:
                    _ok_auto, _ns_auto = _guardar_y_actualizar_snapshot_detector(
                        cid, abierto, ns, mostrar_mensajes=False
                    )
                    if _ok_auto:
                        if _ns_auto != ns:
                            # Una ficha «Nuevo detector» acaba de convertirse
                            # en un detector persistente con ID real. Rehacemos
                            # la pantalla con su namespace definitivo.
                            st.rerun()
                        ns = _ns_auto
...
def pantalla_centro_retirada():
    ...
    def _guardar_retirada_actual(did, fr_val, hr_val, tecnico_ret_val=""):
        tecnico_ret_val = str(tecnico_ret_val or "").strip()
        actualizar_retirada_detector(
            did, fr_val.strip(), hr_val.strip(), tecnico_ret_val
        )
        st.session_state[f"retirada_snapshot_{did}"] = (
            fr_val.strip(), hr_val.strip(), tecnico_ret_val
        )
    ...
    with st.container(border=True):
        if not detectores:
            ...
            if idx_sel_r is None:
                ...
                tecnico_retirada_r = d_r[22] if len(d_r) > 22 else ""
                ...
                if st.session_state[abierto_retirada_key] != did_r:
                    ...
                    tecnico_retirada_r = d_mostrar[22] if len(d_mostrar) > 22 else ""
                ...
                tecnico_ret_key = f"retirada_tecnico_{did_r}"
                tecnico_distinto_key = f"retirada_tecnico_distinto_{did_r}"
                ...
                # Técnicos de retirada distintos ya usados en este centro.
                # Si el detector actual todavía no tiene uno asignado, el último
                # utilizado pasa a ser la opción predeterminada.
                tecnicos_retirada_centro = [
                    str(det[22]).strip()
                    for det in fetch_detectores(cid)
                    if len(det) > 22 and str(det[22] or "").strip()
                ]
                ultimo_tecnico_retirada = (
                    tecnicos_retirada_centro[-1]
                    if tecnicos_retirada_centro
                    else ""
                )

                if tecnico_ret_key not in st.session_state:
                    st.session_state[tecnico_ret_key] = (
                        tecnico_retirada_r or ultimo_tecnico_retirada
                    )
                if tecnico_distinto_key not in st.session_state:
                    st.session_state[tecnico_distinto_key] = bool(
                        str(tecnico_retirada_r or ultimo_tecnico_retirada).strip()
                    )
                if snapshot_key not in st.session_state:
                    st.session_state[snapshot_key] = (
                        fecha_retirada_real_r or "",
                        hora_retirada_real_r or "",
                        tecnico_retirada_r or "",
                    )

                _date_input_texto("Fecha de retirada", fr_key)
                st.text_input("Hora de retirada", key=hr_key)

                tecnico_distinto = st.checkbox(
                    "El técnico que retira el detector es distinto del que lo colocó",
                    key=tecnico_distinto_key,
                )
                if tecnico_distinto:
                    # Los técnicos distintos ya utilizados en cualquier detector
                    # de este centro quedan disponibles para el resto de detectores
                    # de esta misma pantalla de retirada.
                    tecnicos_retirada_disponibles = sorted({
                        str(det[22]).strip()
                        for det in fetch_detectores(cid)
                        if len(det) > 22 and str(det[22] or "").strip()
                    }, key=str.casefold)

                    tecnico_guardado_actual = str(
                        st.session_state.get(tecnico_ret_key, "") or ""
                    ).strip()
                    if (
                        tecnico_guardado_actual
                        and tecnico_guardado_actual not in tecnicos_retirada_disponibles
                    ):
                        tecnicos_retirada_disponibles.append(tecnico_guardado_actual)
                        tecnicos_retirada_disponibles.sort(key=str.casefold)

                    # Si todavía no existe ningún técnico distinto guardado,
                    # el desplegable indica directamente que debe introducirse abajo.
                    opcion_nuevo_tecnico = (
                        "Introduce el nombre del técnico abajo"
                        if not tecnicos_retirada_disponibles
                        else "➕ Otro técnico"
                    )
                    opciones_tecnico_retirada = (
                        tecnicos_retirada_disponibles + [opcion_nuevo_tecnico]
                    )

                    selector_tecnico_key = f"retirada_selector_tecnico_{did_r}"
                    if selector_tecnico_key not in st.session_state:
                        tecnico_predeterminado = (
                            tecnico_guardado_actual or ultimo_tecnico_retirada
                        )
                        st.session_state[selector_tecnico_key] = (
                            tecnico_predeterminado
                            if tecnico_predeterminado in tecnicos_retirada_disponibles
                            else opcion_nuevo_tecnico
                        )

                    tecnico_seleccionado = st.selectbox(
                        "Técnico que realiza la retirada",
                        opciones_tecnico_retirada,
                        key=selector_tecnico_key,
                    )

                    if tecnico_seleccionado == opcion_nuevo_tecnico:
                        st.text_input(
                            "Nombre del nuevo técnico",
                            key=tecnico_ret_key,
                            placeholder="Nombre y apellidos",
                        )
                        tecnico_retirada_actual = st.session_state[
                            tecnico_ret_key
                        ].strip()
                    else:
                        tecnico_retirada_actual = tecnico_seleccionado
                else:
                    tecnico_retirada_actual = ""
                ...
                with fcol1:
                    ...
                    if (plano_centro_id_r and punto_x_r is not None and punto_y_r is not None
                        ...
                        if plano_info_r and os.path.exists(plano_info_r[3]):
                            ruta_tmp_punto_r = os.path.join(get_tmp_dir(), f"_tmp_plano_punto_retirada_{did_r}.jpg")
                            if generar_plano_con_punto(plano_info_r[3], punto_x_r, punto_y_r, ruta_tmp_punto_r):
                                if not _miniatura_ampliable(
                                    ruta_tmp_punto_r, ancho=180, alt=f"Plano del detector {codigo_r or did_r}"
                                ):
                                    st.image(ruta_tmp_punto_r, width=180)
                            else:
                ...
                with fcol2:
                    ...
                    if foto_sit_r and os.path.exists(foto_sit_r):
                        if not _miniatura_ampliable(
                            foto_sit_r, ancho=180, alt=f"Foto de situación del detector {codigo_r or did_r}"
                        ):
                            st.image(foto_sit_r, width=180)
                    else:
    ...
    if volver_clic:
        if did_r is not None and hay_cambios:
            _guardar_retirada_actual(
                ...
                (
                    (
                        st.session_state.get(f"retirada_selector_tecnico_{did_r}", "")
                        if st.session_state.get(f"retirada_selector_tecnico_{did_r}", "") != "➕ Otro técnico"
                        else st.session_state.get(f"retirada_tecnico_{did_r}", "")
                    )
                    if st.session_state.get(f"retirada_tecnico_distinto_{did_r}", False)
                    else ""
                ),
            )
...
# ============================================================
# PANTALLA: CENTRO - INFORMES
# ============================================================



def _generar_pdf_planos_detectores(
    centro_id,
    mostrar_logo=True,
    mostrar_foto_exterior=True,
    bases_alternativas=None,
):
    """Genera un PDF A4 con orientación automática por plano y todos los
    detectores ubicados sobre él. Reutiliza exactamente el compositor del
    Anexo II para conservar puntos, etiquetas, logo, foto exterior y planos
    base alternativos.
    """
    from utils_informe.anexo2 import Anexo2Error, extraer_datos_planos, componer_plano
    from reportlab.pdfgen import canvas as _pdf_canvas
    from reportlab.lib.pagesizes import A4 as _A4, landscape as _landscape, portrait as _portrait
    from reportlab.lib.utils import ImageReader as _ImageReader
    from PIL import Image as _PILImage

    excel_bytes, _ = _obtener_excel_descarga_centro(centro_id)
    datos_planos = extraer_datos_planos(io.BytesIO(excel_bytes))

    # Misma opción que en el Anexo II: sustituir la imagen base manteniendo
    # exactamente las dimensiones para que las coordenadas sigan siendo válidas.
    bases_alternativas = bases_alternativas or {}
    for clave_base, bytes_base in bases_alternativas.items():
        if clave_base not in datos_planos.get("planos", {}):
            continue
        imagen_original = datos_planos["planos"][clave_base]["imagen"]
        with _PILImage.open(io.BytesIO(imagen_original)) as im_original:
            tam_original = im_original.size
        with _PILImage.open(io.BytesIO(bytes_base)) as im_nueva:
            tam_nueva = im_nueva.size
        if tam_nueva != tam_original:
            nombre_base = datos_planos["planos"][clave_base].get("nombre", clave_base)
            raise Anexo2Error(
                f"El plano base alternativo «{nombre_base}» tiene un tamaño de "
                f"{tam_nueva[0]}×{tam_nueva[1]} px, pero el original es de "
                f"{tam_original[0]}×{tam_original[1]} px. Deben tener exactamente "
                "el mismo tamaño."
            )
        datos_planos["planos"][clave_base]["imagen"] = bytes_base

    centro = get_centro(centro_id)
    centro_nombre = centro[1] if centro else "Centro"
    logo_bytes, _ = get_logo_informe()
    datos_informe = get_datos_informe_centro(centro_id)
    superficie_construida = datos_informe.get("superficie_construida", "")
    superficie_util = datos_informe.get("superficie_util", "")

    claves_con_puntos = [
        clave for clave in datos_planos.get("planos", {})
        if datos_planos.get("puntos", {}).get(clave)
    ]
    if not claves_con_puntos:
        raise Anexo2Error(
    ...
    alto_titulo = 28
    buffer = io.BytesIO()
    # La orientación del A4 se adapta a cada plano.
    pdf = _pdf_canvas.Canvas(buffer, pagesize=_landscape(_A4))

    for clave in claves_con_puntos:
        info_plano = datos_planos["planos"][clave]
        with _PILImage.open(io.BytesIO(info_plano["imagen"])) as _im_base:
            _bw, _bh = _im_base.size
        if _bh > _bw:
            page_w, page_h = _portrait(_A4)
        else:
            page_w, page_h = _landscape(_A4)
        pdf.setPageSize((page_w, page_h))
        puntos_plano = datos_planos["puntos"].get(clave, [])
        ...
        titulo = f"PLANO CON UBICACIÓN DE DETECTORES — {info_plano.get('nombre', clave)}"
        pdf.setFont("Helvetica-Bold", 11)
        pdf.drawCentredString(page_w / 2, page_h - margen - 4, titulo)

        with _PILImage.open(io.BytesIO(compuesto)) as im:
            iw, ih = im.size
        max_w = page_w - 2 * margen
        max_h = page_h - 2 * margen - alto_titulo
        escala = min(max_w / iw, max_h / ih)
        dw, dh = iw * escala, ih * escala
        x = (page_w - dw) / 2
        y = margen + (max_h - dh) / 2
        pdf.drawImage(
            _ImageReader(io.BytesIO(compuesto)),
...
def pantalla_centro_informes():
    ...
    with st.container(border=True):
        if not detectores:
            ...
            for d in detectores:
                ...
                if (plano_centro_id_d and punto_x_d is not None and punto_y_d is not None
                    ...
                    if plano_info_d and os.path.exists(plano_info_d[3]):
                        ruta_tmp_punto = os.path.join(get_tmp_dir(), f"_tmp_plano_punto_det{did_d}.jpg")
            ...
            gen_planos_pdf = st.checkbox("Plano con detectores ubicados (PDF)", value=False, key=f"gen_chk_planos_pdf_{cid}")

            # Mismas opciones de composición que el Anexo II del informe final.
            mostrar_logo_planos_pdf = True
            mostrar_foto_planos_pdf = True
            usar_base_alt_planos_pdf = False
            bases_alt_planos_pdf = {}
            if gen_planos_pdf:
                with st.container(border=True):
                    st.markdown("**Opciones del plano**")
                    st.caption(
                        "El plano aprovechará prácticamente toda la hoja. El logotipo y la foto exterior, "
                        "si se seleccionan, se superponen abajo a la derecha."
                    )
                    col_plano_logo, col_plano_foto = st.columns(2)
                    with col_plano_logo:
                        mostrar_logo_planos_pdf = st.checkbox(
                            "Mostrar logotipo (superpuesto)",
                            value=True,
                            key=f"gen_planos_pdf_logo_{cid}",
                        )
                    with col_plano_foto:
                        mostrar_foto_planos_pdf = st.checkbox(
                            "Mostrar foto exterior (superpuesta)",
                            value=True,
                            key=f"gen_planos_pdf_foto_{cid}",
                        )

                    usar_base_alt_planos_pdf = st.checkbox(
                        "Utilizar otro plano base",
                        value=False,
                        key=f"gen_planos_pdf_base_alt_{cid}",
                    )
                    if usar_base_alt_planos_pdf:
                        st.caption(
                            "Sube un plano base alternativo del mismo tamaño que el original. "
                            "Los puntos y el resto de datos se colocarán sobre esta nueva imagen."
                        )
                        try:
                            from utils_informe.anexo2 import extraer_datos_planos as _extraer_planos_pdf
                            _excel_planos_bytes, _ = _obtener_excel_descarga_centro(cid)
                            _datos_planos_pdf_ui = _extraer_planos_pdf(io.BytesIO(_excel_planos_bytes))
                            for _clave_pdf, _info_pdf in _datos_planos_pdf_ui.get("planos", {}).items():
                                _nombre_pdf_base = _info_pdf.get("nombre", _clave_pdf)
                                _archivo_pdf_base = st.file_uploader(
                                    f"Plano base alternativo — {_nombre_pdf_base}",
                                    type=["png", "jpg", "jpeg"],
                                    key=f"gen_planos_pdf_base_{cid}_{_clave_pdf}",
                                )
                                if _archivo_pdf_base is not None:
                                    bases_alt_planos_pdf[_clave_pdf] = _archivo_pdf_base.getvalue()
                        except Exception as _e_planos_pdf_ui:
                            st.warning(
                                "No se han podido preparar los planos base alternativos: "
                                f"{_e_planos_pdf_ui}"
                            )
            ...
            if gen_lab:
                with st.container(border=True):
                    st.markdown("**Opciones del documento para laboratorio**")
                    ...
                    if logo_lab_actual and os.path.exists(logo_lab_actual):
                        if not _miniatura_ampliable(
                            logo_lab_actual, ancho=180, alt="Logotipo del laboratorio"
                        ):
            ...
            if generar_clic:
                if not (gen_pdf or gen_planos_pdf or gen_excel or gen_fotos or gen_lab):
                    ...
                    try:
                        ...
                        with st.spinner("Generando documentos..."):
                            if gen_pdf:
                                ...
                                ruta_pdf = guardar_archivo_centro(cid, b"", nombre_pdf, subdir="informes")
                            ...
                            if gen_planos_pdf:
                                nombre_planos_pdf = _nombre_documento(nombre, "PLANO-DETECTORES") + ".pdf"
                                ruta_planos_pdf = guardar_archivo_centro(
                                    cid, b"", nombre_planos_pdf, subdir="informes"
                                )
                                contenido_planos_pdf = _generar_pdf_planos_detectores(
                                    cid,
                                    mostrar_logo=mostrar_logo_planos_pdf,
                                    mostrar_foto_exterior=mostrar_foto_planos_pdf,
                                    bases_alternativas=bases_alt_planos_pdf,
                                )
                                with open(ruta_planos_pdf, "wb") as _f_planos_pdf:
                                    _f_planos_pdf.write(contenido_planos_pdf)
                                st.session_state["ultimo_planos_pdf"] = ruta_planos_pdf
                                st.session_state["ultimo_planos_pdf_nombre"] = nombre_planos_pdf
                                st.session_state["ultimo_planos_pdf_centro"] = cid

                            if gen_excel:
                                nombre_xlsx = _nombre_documento(nombre, "HOJA-DATOS") + ".xlsx"
                                ruta_xlsx = guardar_archivo_centro(cid, b"", nombre_xlsx, subdir="informes")
                            ...
                            if gen_fotos:
                                ...
                                ruta_zip = guardar_archivo_centro(cid, b"", nombre_zip, subdir="informes")
                            ...
                            if gen_lab:
                                ...
                                ruta_lab = guardar_archivo_centro(cid, b"", nombre_lab, subdir="informes")
            ...
            hay_planos_pdf = st.session_state.get("ultimo_planos_pdf_centro") == cid
            ...
            if hay_pdf or hay_planos_pdf or hay_excel or hay_zip or hay_lab:
                ultimo_pdf = st.session_state.get("ultimo_pdf")
                ultimo_planos_pdf = st.session_state.get("ultimo_planos_pdf")
                ...
                if hay_planos_pdf and ultimo_planos_pdf and os.path.exists(ultimo_planos_pdf):
                    with open(ultimo_planos_pdf, "rb") as f:
                        planos_pdf_bytes = f.read()
                    st.markdown('<div class="marcador-btn-descarga-amarillo"></div>', unsafe_allow_html=True)
                    st.download_button(
                        "Descargar plano con detectores ubicados (PDF)", data=planos_pdf_bytes,
                        file_name=st.session_state.get("ultimo_planos_pdf_nombre", "plano_detectores.pdf"),
                        mime="application/pdf", use_container_width=True,
                        icon=":material/map:",
                    )
                    texto_wa_planos = f"Plano con ubicación de detectores de Rn – {nombre or ''}"
                    boton_compartir_whatsapp(
                        ultimo_planos_pdf,
                        st.session_state.get("ultimo_planos_pdf_nombre", "plano_detectores.pdf"),
                        texto_wa_planos,
                        id_sufijo="planos_pdf",
                        etiqueta_boton="Enviar por WhatsApp",
                    )
...
# ============================================================
# PANTALLA: CENTRO - INFORME COMPLETO
# ============================================================
...
# ============================================================
# MODELO DE INFORME IS-47 — v6: variante B corregida
# ============================================================

_IS47_REFERENCIAS = [
    "Lei 31/1995, de 8 de novembro, de Prevención de Riscos Laborais (BOE nº 269, de 10 de novembro).",
    "Real Decreto 39/1997, de 17 de xaneiro, polo que se aproba o Regulamento dos servizos de prevención (BOE nº 27, de 31 de xaneiro).",
    "Real Decreto 732/2019, de 20 de decembro, polo que se modifica o Código Técnico da Edificación, aprobado polo Real Decreto 314/2006, de 17 de marzo.",
    "Instrución IS-47, de 9 de abril de 2025, do Consello de Seguridade Nuclear.",
    "Guía de Seguridade do CSN 11.4 Metodoloxía para a avaliación da exposición ao radon nos lugares de traballo.",
    "Estratexia para reducir a exposición ao radon en Galicia. Estratexia Reduce Radon 2025-2030. Consellería de Sanidade, Dirección Xeral de Saúde Pública, ano 2024.",
]


def _is47_asegurar_campos_db():
    """Añade únicamente los campos propios del IS-47 que necesiten persistencia."""
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("PRAGMA table_info(centros)")
    columnas = {fila[1] for fila in c.fetchall()}
    if "is47_horario_turnos" not in columnas:
        c.execute("ALTER TABLE centros ADD COLUMN is47_horario_turnos TEXT DEFAULT 'M: 08:00 – 15:00; T: ; N:'")
    conn.commit()
    conn.close()


def _is47_get_horario_turnos(centro_id):
    _is47_asegurar_campos_db()
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("SELECT COALESCE(is47_horario_turnos, '') FROM centros WHERE id=?", (centro_id,))
    row = c.fetchone()
    conn.close()
    valor = (row[0] or "").strip() if row else ""
    return valor or "M: 08:00 – 15:00; T: ; N:"


def _is47_set_horario_turnos(centro_id, valor):
    _is47_asegurar_campos_db()
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("UPDATE centros SET is47_horario_turnos=? WHERE id=?", ((valor or "").strip(), centro_id))
    conn.commit()
    conn.close()


def _is47_detectores(centro_id):
    """Devuelve los detectores como diccionarios para no depender del orden físico de columnas."""
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("PRAGMA table_info(detectores)")
    columnas = [fila[1] for fila in c.fetchall()]
    c.execute("SELECT * FROM detectores WHERE centro_id=? ORDER BY id", (centro_id,))
    filas = [dict(zip(columnas, fila)) for fila in c.fetchall()]
    conn.close()
    return filas


def _is47_fecha(valor):
    texto = str(valor or "").strip()
    if not texto:
        return ""
...
def _is47_texto_profesionais(det, categorias=None):
    """Texto para la última columna del punto 5.

    Se conserva exclusivamente «categoría (número de trabajadores)».
    El turno/horario guardado en profesionales_sala se elimina siempre.
    Ejemplos:
      «DUE (2) - Mañana (8 a 15 h)» -> «DUE (2)»
      «TSPRL (5) – 8 a 15 h»         -> «TSPRL (5)»
    """
    prof = str(det.get("profesionales_sala") or "").strip()
    sala = str(det.get("sala") or "").strip()

    if not prof:
        return sala

    salida = []
    # profesionales_sala se guarda como elementos separados por coma.
    for item in [p.strip() for p in prof.replace("\n", ",").split(",") if p.strip()]:
        limpio = item

        # Formato real de la ficha del detector:
        # «Categoría (N) - turno/horario». Se elimina todo lo posterior
        # al separador, conservando categoría y N.
        limpio = re.split(r"\s+[–—-]\s+", limpio, maxsplit=1)[0].strip()

        # Respaldo para textos antiguos: «Categoría (N) Turno ...»
        m_num = re.match(r"^(.*?)\s*\((\d+)\)", limpio)
        if m_num:
            categoria = m_num.group(1).strip(" :-–—")
            numero = m_num.group(2).strip()
            limpio = f"{categoria} ({numero})"
        else:
            # Si no hay número embebido, eliminar únicamente un horario final
            # entre paréntesis, sin borrar el nombre de la categoría.
            limpio = re.sub(
                r"\s*\([^)]*(?:\d{1,2}\s*(?::|\ba\b|-)\s*\d{1,2}|h\b|hora|turno|mañ|tarde|noite|noche)[^)]*\)\s*$",
                "",
                limpio,
                flags=re.IGNORECASE,
            ).strip()
    ...
    return "\n".join(salida) if salida else sala


def _is47_separar_categoria_numero_horarios(texto):
    """Tabla 2 IS-47(B): primera columna = profesional + turno; segunda = número.

    El número de profesionales se toma SIEMPRE del entero final de la línea,
    aunque venga separado por dos puntos, guion, raya o simplemente un espacio.
    Todo lo anterior se conserva íntegro en la primera columna.
    """
    bruto = str(texto or "").strip()
    bruto = re.sub(r"^\s*[-•]\s*", "", bruto).strip()
    if not bruto:
        return "", ""
    ...
    #   DUE (8 a 15 h) – 2
    #   DUE (8 a 15 h) 2
    # Se extrae únicamente el último entero de la línea.
    m = re.match(r"^(.*?)(?:\s*[:–—-]\s*|\s+)(\d+)\s*$", bruto)
    if m:
        izquierda = m.group(1).strip(" :-–—")
        numero = m.group(2).strip()
        return izquierda, numero

    # Respaldo para «Categoría (N): horario»: N sigue siendo el número,
    # y el horario queda junto a la categoría en la primera columna.
    m = re.match(r"^(.*?)\s*\((\d+)\)\s*(?::|[–—-])?\s*(.*)$", bruto)
    if m:
        categoria = m.group(1).strip(" :-–—")
        numero = m.group(2).strip()
        resto = m.group(3).strip(" :-–—")
        izquierda = categoria + (f" ({resto})" if resto else "")
        return izquierda.strip(), numero

    return bruto, ""

def _is47_datos_ocupacion(detectores):
    vistos = set()
    filas = []
    for det in detectores:
        sala = str(det.get("sala") or det.get("codigo_sala") or "").strip()
        prof = _is47_texto_profesionais(det)
        turno = str(det.get("turno_trabajo") or "").strip()
        clave = (sala, prof, turno)
        if clave in vistos or not any(clave):
            continue
        vistos.add(clave)
        filas.append({"Sala": sala, "Profesionais": prof, "Turno": turno})
    return filas


def _normalizar_factor_k(valor):
    """Devuelve el factor k como texto limpio, conservando coma o punto decimal."""
    if valor is None:
        return ""
    valor = str(valor).strip()
    if not valor or valor.lower() == "nan":
        return ""
    m = re.search(r"(?i)\bk\s*=\s*(\d+(?:[.,]\d+)?)", valor)
    if m:
        return m.group(1)
...
def _cabecera_incertidumbre(factor_k, idioma="gl"):
    es = str(idioma or "gl").lower().startswith("es")
    base = "Incertidumbre expandida (Bq/m³)" if es else "Incerteza expandida (Bq/m³)"
    k_txt = _normalizar_factor_k(factor_k)
    return f"{base} K={k_txt}" if k_txt else base


def _actualizar_cabecera_incertidumbre_docx(docx_bytes, factor_k, idioma="gl"):
    """Actualiza la cabecera de incertidumbre del informe SSCC ya generado."""
    from docx import Document
    entrada = io.BytesIO(docx_bytes)
    doc = Document(entrada)
    nueva = _cabecera_incertidumbre(factor_k, idioma)

    for tabla in doc.tables:
        for fila in tabla.rows:
            for celda in fila.cells:
                texto = " ".join(
                    p.text.strip() for p in celda.paragraphs if p.text.strip()
                )
                normal = texto.casefold()
                if ("incertidumbre" in normal or "incerteza" in normal) and (
                    "bq" in normal or "k=" in normal or "expandida" in normal
                ):
                    celda.text = nueva
...
def _is47_resultados_dataframe(detectores, categorias=None):
    import pandas as pd
    filas = []
    for det in detectores:
        resultado = det.get("resultado_bq_m3")
        try:
            resultado = float(resultado) if resultado not in (None, "") else None
        except Exception:
            resultado = None
        filas.append({
            "ID": det.get("id"),
            "Código zona": str(det.get("codigo_sala") or "").strip(),
            "Código detector": str(det.get("detector_codigo") or "").strip(),
            "Data inicio": _is47_fecha(det.get("fecha")),
            "Data fin": _is47_fecha(det.get("fecha_retirada_real")),
            "Concentración radon (Bq/m³)": resultado,
            "Incerteza expandida (Bq/m³)": str(det.get("incertidumbre") or "").strip(),
            "Posto/postos de traballo asociados": _is47_texto_profesionais(det, categorias),
        })
    return pd.DataFrame(filas)


def _is47_conclusion_automatica(df_resultados, idioma="gl"):
    es = str(idioma or "gl").lower().startswith("es")
    if df_resultados is None or df_resultados.empty:
        return ("No hay resultados disponibles para establecer conclusiones."
                if es else "Non hai resultados dispoñibles para establecer conclusións.")
    serie = df_resultados["Concentración radon (Bq/m³)"]
    pendientes = int(serie.isna().sum())
    if pendientes:
        if es:
            if pendientes == 1:
                return "Queda 1 medición sin resultado. La conclusión deberá revisarse cuando se disponga de todos los resultados."
            return f"Quedan {pendientes} mediciones sin resultado. La conclusión deberá revisarse cuando se disponga de todos los resultados."
        if pendientes == 1:
            return "Queda 1 medición sen resultado. A conclusión deberá revisarse cando se dispoña de todos os resultados."
        return f"Quedan {pendientes} medicións sen resultado. A conclusión deberá revisarse cando se dispoña de todos os resultados."
    superan = int((serie > 300).sum())
    if superan:
        if es:
            if superan == 1:
                return ("A la vista de los datos del apartado 5, existe 1 medición en la que se supera "
                        "el nivel de referencia de 300 Bq/m³. Deberán valorarse las medidas de remediación correspondientes.")
            return (f"A la vista de los datos del apartado 5, existen {superan} mediciones en las que se supera "
                    "el nivel de referencia de 300 Bq/m³. Deberán valorarse las medidas de remediación correspondientes.")
        if superan == 1:
            return ("Á vista dos datos do apartado 5, existe 1 medición na que se supera "
                    "o nivel de referencia de 300 Bq/m³. Deberán valorarse as medidas de remediación correspondentes.")
        return (f"Á vista dos datos do apartado 5, existen {superan} medicións nas que se supera "
                "o nivel de referencia de 300 Bq/m³. Deberán valorarse as medidas de remediación correspondentes.")
    return ("A la vista de los datos del apartado 5, no hay ningún puesto de trabajo en el que se supere el nivel de referencia de 300 Bq/m³."
            if es else "Á vista dos datos do apartado 5, non hai ningún posto de traballo no que se supere o nivel de referencia de 300 Bq/m³.")


def _is47_docx_bytes(datos, ocupacion, df_resultados, conclusion, anexos_presentes, idioma="gl"):
    from docx import Document
    ...
    # Cabecera: misma estructura visual que el informe SSCC.
    # La única diferencia del título es la referencia expresa a (IS-47 CSN).
    def _set_cell_borders_is47(cell, sz=8):
    ...
    def _set_table_fixed_is47(table, width_cm):
        table.autofit = False
    ...
    def _add_field_is47(paragraph, field_code, cached_text="1", font_size_pt=9):
    ...
    for hp in list(header.paragraphs):
        hp.text = ""

    cab = header.add_table(rows=1, cols=2, width=Cm(17))
    cab.alignment = WD_TABLE_ALIGNMENT.CENTER
    _set_table_fixed_is47(cab, 17)
    logo_w_cm, data_w_cm = 13.2, 3.8
    for col, w in zip(cab.columns, (Cm(logo_w_cm), Cm(data_w_cm))):
        col.width = w
    for cell, w in zip(cab.rows[0].cells, (Cm(logo_w_cm), Cm(data_w_cm))):
        cell.width = w
        _set_cell_borders_is47(cell)
        cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER

    logo_cell = cab.rows[0].cells[0]
    ...
    logo_bytes, _ = get_logo_informe(datos.get("xerencia"))
    if logo_bytes:
        try:
            with Image.open(io.BytesIO(logo_bytes)) as img:
                iw, ih = img.size
            max_w_cm, max_h_cm = logo_w_cm - 0.6, 1.8
            escala = min(max_w_cm / max(iw, 1), max_h_cm / max(ih, 1))
            w_cm = max(0.1, iw * escala)
            h_cm = max(0.1, ih * escala)
            logo_p.add_run().add_picture(io.BytesIO(logo_bytes), width=Cm(w_cm), height=Cm(h_cm))
        except Exception:
            logo_p.add_run("[logotipo]").italic = True
    else:
        rr = logo_p.add_run(datos.get("xerencia", "") or "UPRL · SERVIZO GALEGO DE SAÚDE · ÁREA SANITARIA")
        rr.font.size = Pt(8)
        rr.italic = True

    data_cell = cab.rows[0].cells[1]
    dp = data_cell.paragraphs[0]
    dp.alignment = WD_ALIGN_PARAGRAPH.LEFT
    r1 = dp.add_run(f"{'Fecha' if es else 'Data'}: {datos.get('data_informe','')}\n")
    r1.font.size = Pt(9)
    dp2 = data_cell.add_paragraph()
    r2 = dp2.add_run("Página " if es else "Páxina ")
    r2.font.size = Pt(9)
    _add_field_is47(dp2, "PAGE", cached_text="1", font_size_pt=9)
    r3 = dp2.add_run(" de ")
    r3.font.size = Pt(9)
    _add_field_is47(dp2, "NUMPAGES", cached_text="1", font_size_pt=9)

    title_table = header.add_table(rows=1, cols=1, width=Cm(17))
    _set_table_fixed_is47(title_table, 17)
    title_cell = title_table.rows[0].cells[0]
    _set_cell_borders_is47(title_cell)
    sombrear(title_cell, "F2F2F2")
    title_cell.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
    tp = title_cell.paragraphs[0]
    tp.alignment = WD_ALIGN_PARAGRAPH.CENTER
    centro_titulo = str(datos.get("centro", "") or "...................").upper()
    tr = tp.add_run((f"INFORME DE RESULTADOS DE LAS MEDICIONES DE RADÓN (IS-47 CSN) EN EL {centro_titulo}" if es else f"INFORME DE RESULTADOS DAS MEDICIÓNS DE RADON (IS-47 CSN) NO {centro_titulo}"))
    tr.bold = True
    ...
    titulo("OBJETO DEL INFORME" if es else "OBXECTO DO INFORME")
    doc.add_paragraph(datos.get("obxecto", ""))

    # Punto 1: mismo cuadro que el SSCC, añadiendo teléfono y eliminando N.º de plantas.
    titulo("1. DATOS DE LA EMPRESA Y DEL CENTRO DE TRABAJO" if es else "1. DATOS DA EMPRESA E DO CENTRO DE TRABALLO")
    # Rejilla de 8 columnas: 2/8 para la primera columna y 3/8 + 3/8
    # para las dos superficies de la última fila.
    t = doc.add_table(rows=5, cols=8)
    t.alignment = WD_TABLE_ALIGNMENT.CENTER
    t.style = "Table Grid"
    _set_table_fixed_is47(t, 17)
    for col in t.columns:
        col.width = Cm(17 / 8)

    r0 = t.rows[0].cells
    c00 = r0[0].merge(r0[1])
    c01 = r0[2].merge(r0[3]).merge(r0[4]).merge(r0[5])
    c02 = r0[6].merge(r0[7])
    c00.text = "GERENCIA" if es else "XERENCIA"
    c00.paragraphs[0].runs[0].bold = True
    c01.text = str(datos.get("xerencia", "") or "")
    c02.text = f"CIF: {datos.get('cif','')}"
    cells0 = (c00, c01, c02)

    r1 = t.rows[1].cells
    c10 = r1[0].merge(r1[1])
    c11 = r1[2].merge(r1[3]).merge(r1[4]).merge(r1[5])
    c12 = r1[6].merge(r1[7])
    c10.text = "CENTRO"
    c10.paragraphs[0].runs[0].bold = True
    c11.text = str(datos.get("centro", "") or "")
    c12.text = f"Teléfono: {datos.get('telefono','')}"
    cells1 = (c10, c11, c12)

    r2 = t.rows[2].cells
    c20 = r2[0].merge(r2[1])
    c21 = r2[2].merge(r2[3]).merge(r2[4]).merge(r2[5]).merge(r2[6]).merge(r2[7])
    c20.text = "SERVICIO / UNIDAD MUESTREADA" if es else "SERVIZO / UNIDADE MOSTREXADA"
    c20.paragraphs[0].runs[0].bold = True
    c21.text = str(datos.get("servizo", "") or "")
    cells2 = (c20, c21)

    r3 = t.rows[3].cells
    c30 = r3[0].merge(r3[1])
    c31 = r3[2].merge(r3[3]).merge(r3[4]).merge(r3[5]).merge(r3[6]).merge(r3[7])
    c30.text = "DIRECCIÓN" if es else "ENDEREZO"
    c30.paragraphs[0].runs[0].bold = True
    c31.text = str(datos.get("enderezo", "") or "")
    cells3 = (c30, c31)

    r4 = t.rows[4].cells
    c40 = r4[0].merge(r4[1])
    c41 = r4[2].merge(r4[3]).merge(r4[4])
    c42 = r4[5].merge(r4[6]).merge(r4[7])
    c40.text = "DESCRIPCIÓN DEL ÁREA MUESTREADA" if es else "DESCRICIÓN DA ÁREA MOSTREXADA"
    c40.paragraphs[0].runs[0].bold = True
    c41.text = f"{'Superficie construida' if es else 'Superficie construída'}: {datos.get('superficie_construida','')}"
    c42.text = f"Superficie útil: {datos.get('superficie_util','')}"
    cells4 = (c40, c41, c42)

    for row_cells in (cells0, cells1, cells2, cells3, cells4):
        sombrear(row_cells[0], "F2F2F2")
    for row in t.rows:
        # Todas las filas del punto 1 mantienen la misma altura, pero más compacta.
        row.height = Cm(0.72)
        ...
        for j, cell in enumerate(row.cells):
            ...
            for pp in cell.paragraphs:
                ...
                for rr in pp.runs:
                    # Primera columna ligeramente menor para evitar aumentar la altura.
                    rr.font.size = Pt(8 if j == 0 else 9)

    titulo("2. INFORMACIÓN SOBRE LOS/LAS TRABAJADORES/AS" if es else "2. INFORMACIÓN SOBRE OS/AS TRABALLADORES/AS")
    variante_b = str(datos.get("variante_is47", "A") or "A").upper() == "B"
    if variante_b:
        # Variante B: dos tablas independientes. El resto de la información
        # permanece como texto normal, tal como solicita el modelo alternativo.
        def _separar_columna(texto):
            ...
            # Respaldo para textos que usen guion como separador.
            if " – " in texto:
                izquierda, derecha = texto.split(" – ", 1)
                return izquierda.strip(), derecha.strip()
            return texto, ""

        def _separar_categoria_numero_horarios(texto):
            return _is47_separar_categoria_numero_horarios(texto)

        postos_tabla = list(datos.get("postos_bullets", []) or [])
        categorias_tabla = list(datos.get("categorias_bullets", []) or [])

        doc.add_paragraph("Ocupación por sala/categoría/turno:")

        # Tabla 1: sala | profesionales y horario
        tt1 = doc.add_table(rows=1, cols=2)
        tt1.style = "Table Grid"
        tt1.alignment = WD_TABLE_ALIGNMENT.CENTER
        tt1.cell(0, 0).text = "Sala"
        tt1.cell(0, 1).text = "Profesionales y turnos" if es else "Profesionais e quendas"
        for c in tt1.rows[0].cells:
            sombrear(c)
            for rr in c.paragraphs[0].runs:
                rr.bold = True
        if postos_tabla:
            for item in postos_tabla:
                sala, profesionais_horario = _separar_columna(item)
                cells = tt1.add_row().cells
                p_sala = cells[0].paragraphs[0]
                r_sala = p_sala.add_run(sala)
                r_sala.bold = True
                cells[1].text = profesionais_horario
        else:
            tt1.add_row()

        # Celdas ligeramente más amplias para mejorar la lectura.
        for fila_tt1 in tt1.rows:
            fila_tt1.height = Cm(0.82)
            fila_tt1.height_rule = WD_ROW_HEIGHT_RULE.AT_LEAST
            for celda_tt1 in fila_tt1.cells:
                celda_tt1.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
                tc = celda_tt1._tc
        ...
        # Segunda parte del punto 2: presentación narrativa, igual que en IS-47.
        p_intro_cat = doc.add_paragraph(
            "El personal adscrito al conjunto del centro dividido por categorías es el siguiente;"
            if es else
            "O persoal adscrito ao conxunto do centro dividido por categorías é o seguinte;"
        )
        p_intro_cat.paragraph_format.space_after = Pt(4)

        for item in categorias_tabla:
            texto_cat = str(item or "").strip()
            if texto_cat.startswith("-"):
                texto_cat = texto_cat[1:].strip()
            if not texto_cat:
                continue
            p_cat = doc.add_paragraph(style="List Bullet")
            # Se conserva exactamente el texto generado con el mismo criterio
            # que utiliza el IS-47 narrativo, sin intentar dividirlo en columnas.
            if ":" in texto_cat:
                izquierda, derecha = texto_cat.split(":", 1)
                r_cat = p_cat.add_run(izquierda.strip())
                r_cat.bold = True
                if derecha.strip():
                    p_cat.add_run(": " + derecha.strip())
            else:
                p_cat.add_run(texto_cat)

        # El resto del punto 2 queda fuera de tablas.
        doc.add_paragraph()
        p_total = doc.add_paragraph()
        r_total = p_total.add_run("N.º total de trabajadores adscritos: " if es else "Nº total de traballadores adscritos: ")
        r_total.bold = True
        p_total.add_run(str(datos.get("num_traballadores", "") or ""))

        texto_info = str(datos.get("texto_info_traballadores", "") or "").strip()
        if texto_info:
            doc.add_paragraph(texto_info)
    else:
        texto_p2 = str(datos.get("texto_punto2", "") or "").strip()
        _frase_ocupacion = "Ocupación por sala/categoría/turno:"
        if texto_p2:
            _lineas_p2 = texto_p2.splitlines()
            if _lineas_p2 and _lineas_p2[0].strip().lower() == _frase_ocupacion.lower():
                texto_p2 = "\n".join(_lineas_p2[1:]).lstrip()
        if texto_p2:
            for linea in texto_p2.splitlines():
                linea = linea.rstrip()
                if not linea:
                    continue
                if linea.lstrip().startswith("-"):
                    texto_linea = linea.lstrip()[1:].strip()
                    p_linea = doc.add_paragraph(style="List Bullet")
                    # Igual que SSCC: si la línea corresponde a una sala, se resalta
                    # el nombre de la sala (texto anterior a ":").
                    if ":" in texto_linea:
                        izquierda_linea, resto_linea = texto_linea.split(":", 1)
                        # Si coincide con una categoría del centro, separamos su número
                        # del nombre y dejamos número + horario a continuación.
                        _es_categoria = any(
                            str(cat or "").strip().lstrip("-").strip().startswith(izquierda_linea.strip())
                            for cat in (datos.get("categorias_bullets", []) or [])
                        )
                        if _es_categoria:
                            categoria_limpia, numero_horarios = _separar_categoria_numero_horarios(texto_linea)
                            r_cat = p_linea.add_run(categoria_limpia)
                            r_cat.bold = True
                            if numero_horarios:
                                p_linea.add_run(": " + numero_horarios)
                        else:
                            r_sala = p_linea.add_run(izquierda_linea.strip())
                            r_sala.bold = True
                            p_linea.add_run(":" + resto_linea)
                    else:
                        p_linea.add_run(texto_linea)
                else:
                    doc.add_paragraph(linea)
        else:
            doc.add_paragraph("Ocupación por sala/categoría/turno:")
            doc.add_paragraph(f"Nº traballadores adscritos ao centro: {datos.get('num_traballadores','')}")
            doc.add_paragraph(f"Horario turnos: {datos.get('horario_turnos','')}")

    titulo("3. PLANOS")
    doc.add_paragraph(datos.get("texto_planos", ""))

    titulo("4. CONDICIONES DE LA EXPOSICIÓN" if es else "4. CONDICIÓNS DA EXPOSICIÓN")
    doc.add_paragraph(datos.get("texto_condicions", ""))

    titulo("5. RESUMEN DE RESULTADOS" if es else "5. RESUMO DE RESULTADOS")

    # Párrafo de detectores utilizados.
    p5_det = doc.add_paragraph()
    p5_det.paragraph_format.space_before = Pt(0)
    p5_det.paragraph_format.space_after = Pt(7)
    p5_det.paragraph_format.line_spacing = 1.0
    r_det = p5_det.add_run("Detectores utilizados:")
    r_det.bold = True
    p5_det.add_run(
        " DETECTORES DE TRAZAS NUCLEARES EN ESTADO SÓLIDO (SSNTD) DE LARGA DURACIÓN"
        if es else
        " DETECTORES DE TRAZAS NUCLEARES EN ESTADO SÓLIDO (SSNTD) DE LONGA DURACIÓN"
    )
    p5_det.add_run().add_break()
    p5_det.add_run("TIPO/MODELO: PADC/CR-39 – RSKS. FABRICANTE: RADOSYS-HUNGRÍA")

    # Párrafo de laboratorio, separado visualmente del anterior.
    p5_lab = doc.add_paragraph()
    p5_lab.paragraph_format.space_before = Pt(0)
    p5_lab.paragraph_format.space_after = Pt(10)
    p5_lab.paragraph_format.line_spacing = 1.0
    r_lab = p5_lab.add_run("Laboratorio de medición:")
    r_lab.bold = True
    p5_lab.add_run(f" {datos.get('laboratorio','')}")
    p5_lab.paragraph_format.space_after = Pt(7)

    p5_intro = doc.add_paragraph()
    p5_intro.paragraph_format.space_before = Pt(0)
    p5_intro.paragraph_format.space_after = Pt(10)
    p5_intro.paragraph_format.line_spacing = 1.0
    p5_intro.add_run(
        "Los resultados de las mediciones de radón realizadas pueden verse en la siguiente tabla:"
        if es else
        "Os resultados das medicións de radón realizadas poden verse na seguinte táboa:"
    )

    if df_resultados is not None and not df_resultados.empty:
        cols = ([
            "Código zona", "Código detector", "Fecha inicio/Fecha fin",
            "Concentración radón (Bq/m³)", _cabecera_incertidumbre(datos.get("factor_k", ""), "es"),
            "Puesto/puestos de trabajo asociados",
        ] if es else [
            "Código zona", "Código detector", "Data inicio/Data fin",
            "Concentración radon (Bq/m³)", _cabecera_incertidumbre(datos.get("factor_k", ""), "gl"),
            "Posto/postos de traballo asociados",
        ])
        tr = doc.add_table(rows=1, cols=len(cols))
        tr.style = "Table Grid"
        tr.alignment = WD_TABLE_ALIGNMENT.CENTER
        for j, h in enumerate(cols):
            tr.cell(0, j).text = h
            sombrear(tr.cell(0, j))
            for rr in tr.cell(0, j).paragraphs[0].runs:
                rr.bold = True
        for _, fila in df_resultados.iterrows():
            cells = tr.add_row().cells
            cells[0].text = str(fila.get("Código zona", "") or "")
            cells[1].text = str(fila.get("Código detector", "") or "")
            ini = str(fila.get("Data inicio", "") or "")
            fin = str(fila.get("Data fin", "") or "")
            cells[2].text = f"{ini} a {fin}" if ini or fin else ""
            val = fila.get("Concentración radon (Bq/m³)")
            _val_texto = "" if val is None or (hasattr(val, '__class__') and str(val) == 'nan') else str(int(val) if float(val).is_integer() else val)
            cells[3].text = _val_texto
            try:
                if _val_texto and float(val) > 300:
                    from docx.shared import RGBColor
                    for _r_val in cells[3].paragraphs[0].runs:
                        _r_val.bold = True
                        _r_val.font.color.rgb = RGBColor(255, 0, 0)
            except Exception:
                pass
            _inc_texto = str(fila.get("Incerteza expandida (Bq/m³)", "") or "").strip()
            if _inc_texto:
                _inc_texto = re.sub(r"^[+\-±]\s*", "", _inc_texto).strip()
                _inc_texto = f"±{_inc_texto}" if _inc_texto else ""
            cells[4].text = _inc_texto
            cells[5].text = str(fila.get("Posto/postos de traballo asociados", "") or "")

        # Centrado completo de la tabla del punto 5: horizontal y vertical,
        # tanto encabezados como datos, para IS-47 e IS-47(B).
        for _fila_tabla in tr.rows:
            for _celda_tabla in _fila_tabla.cells:
                _celda_tabla.vertical_alignment = WD_CELL_VERTICAL_ALIGNMENT.CENTER
                for _par_tabla in _celda_tabla.paragraphs:
                    _par_tabla.alignment = WD_ALIGN_PARAGRAPH.CENTER

    titulo("6. CONCLUSIONES" if es else "6. CONCLUSIÓNS")
    doc.add_paragraph(conclusion)

    titulo("7. DOCUMENTACIÓN DE REFERENCIA")
    for referencia in _IS47_REFERENCIAS:
        doc.add_paragraph(referencia, style="List Bullet")

    # Firma con el mismo criterio de selección que SSCC y con la categoría/área debajo.
    p = doc.add_paragraph()
    p.paragraph_format.space_before = Pt(12)
    r = p.add_run("FECHA Y FIRMA DEL/DE LA TÉCNICO/A DE LA UNIDAD DE PREVENCIÓN DE RIESGOS LABORALES" if es
                  else "DATA E FIRMA DO/A TÉCNICO/A DA UNIDADE DE PREVENCIÓN DE RISCOS LABORAIS")
    r.bold = True
    pf = doc.add_paragraph()
    # Firma siguiendo el modelo SSCC, pero alineada completamente a la izquierda.
    # Se reserva espacio suficiente entre el título y el nombre para la firma manuscrita.
    pf.alignment = WD_ALIGN_PARAGRAPH.LEFT
    pf.paragraph_format.space_before = Pt(66)
    rf = pf.add_run(str(datos.get("tecnico", "") or ""))
    rf.bold = True
    rf.add_break()
    rf2 = pf.add_run(str(datos.get("texto_cargo_firma", "") or ""))
    rf2.font.size = Pt(9)
    pf.add_run().add_break()
    pf.add_run(("Fecha: " if es else "Data: ") + str(datos.get("data_informe", "") or ""))

    anexos = ([
        "ANEXO I: ESQUEMA GRÁFICO DEL EDIFICIO Y PLANOS DE CADA PLANTA",
        "ANEXO II: FORMULARIOS DE RECOGIDA DE DATOS",
        "ANEXO III: INFORME DE ENSAYO DEL LABORATORIO ACREDITADO",
        "ANEXO IV: CERTIFICADO ENAC DEL LABORATORIO ACREDITADO",
    ] if es else [
        "ANEXO I ESQUEMA GRÁFICO DO EDIFICO E PLANOS DE CADA PLANTA",
        "ANEXO II FORMULARIOS RECOLLIDA DE DATOS",
        "ANEXO III: INFORME DE ENSAIO DO LABORATORIO ACREDITADO",
        "ANEXO IV: CERTIFICADO ENAC DO LABORATORIO ACREDITADO",
    ])
    for i, etiqueta in enumerate(anexos, 1):
        presente = anexos_presentes.get(i, False)
        p = doc.add_paragraph()
        r = p.add_run(etiqueta + ("" if presente else (" (no incorporado)" if es else " (non incorporado)")))
        r.bold = True
...
def _pantalla_informe_is47(cid, centro, variante="A"):
    import pandas as pd
    from datetime import date
    from utils_informe.docx_generator import ReportContext, generar_texto_punto3_automatico
    from utils_informe.excel_parser import (
        load_workbook, group_options, filter_group, postos_traballo_bullets,
        categorias_turnos_bullets,
    )
    from utils_informe.anexo2 import (
        Anexo2Error, extraer_datos_planos, generar_documento_anexo2, componer_plano,
    )
    ...
    _, nombre, zona, _, _, tecnico_centro, direccion = centro
    detectores = _is47_detectores(cid)
    datos_centro = get_datos_informe_centro(cid)
    categorias = fetch_categorias_centro(cid)
    telefono = get_telefono_centro(cid)
    laboratorio = get_nombre_laboratorio()
    xerencia = get_empresa()
    cif = get_cif()
    ocupacion = _is47_datos_ocupacion(detectores)
    total_trab = sum(int(f[3] or 0) for f in categorias) if categorias else 0

    # Para el punto 2 se reutilizan EXACTAMENTE los mismos transformadores que usa
    # el punto 3 del SSCC. Así las quendas se muestran con su horario (p. ej. 8 a 15 h)
    # y no se duplica "mañá / Turno de mañá".
    ruta_excel_is47 = guardar_archivo_centro(cid, b"", f"_tmp_is47_planos_{cid}.xlsx", subdir="informes")
    df_center_is47 = None
    categorias_df_is47 = None
    try:
        generar_excel(cid, ruta_excel_is47)
        _wb_is47 = load_workbook(ruta_excel_is47)
        _df_is47 = _wb_is47["detectores"]
        categorias_df_is47 = _wb_is47["categorias"]
        _opciones_is47 = group_options(_df_is47, "Centro")
        if _opciones_is47:
            df_center_is47 = filter_group(_df_is47, _opciones_is47[0], "Centro")
    except Exception:
        ruta_excel_is47 = None
    ...
    if df_center_is47 is not None and categorias_df_is47 is not None:
        postos_bullets_default = postos_traballo_bullets(
            df_center_is47, es_atencion_primaria=es_ap_centro,
        )
        total_trab_sscc, categorias_bullets_default = categorias_turnos_bullets(
            categorias_df_is47, es_atencion_primaria=es_ap_centro,
        )
        total_trab = total_trab_sscc
    else:
        # Respaldo solo si no puede generarse el Excel temporal.
        postos_bullets_default = []
        for fila in ocupacion:
            sala = str(fila.get("Sala", "") or "").strip()
            profesionais = str(fila.get("Profesionais", "") or "").strip()
            if sala or profesionais:
                postos_bullets_default.append(f"{sala}: {profesionais}" if sala else profesionais)
        categorias_bullets_default = [str(c[2] or "").strip() for c in categorias if len(c) > 2 and str(c[2] or "").strip()]

    variante = str(variante or "A").upper()
    st.caption("Modelo activo: IS-47" if variante == "A" else "Modelo activo: IS-47 (B)")
    st.info(
        "Modelo adaptado a partir del informe IS-47 facilitado. Los datos ya existentes en la app se reutilizan automáticamente."
        + (" En la variante B, el punto 2 presenta las salas en tabla y las categorías en formato narrativo." if variante == "B" else "")
    )

    obxecto_auto = (
        f"O presente informe ten como obxecto recoller os resultados das medicións de radón no {str(nombre or '').upper()}, "
        "para cando a exposición a radón sexa superior ao nivel de referencia, establecer as medidas de remediación necesarias, "
        "para diminuír ou eliminar estes niveis."
    )
    k_obj = f"is47_obxecto_{cid}"
    if k_obj not in st.session_state:
        st.session_state[k_obj] = obxecto_auto
    # El IS-47 conserva este apartado introductorio, pero sin numerarlo en el título.
    with _acordeon_informe("", "OBJETO DEL INFORME", bool(st.session_state.get(k_obj, "").strip())):
        st.text_area("Texto del objeto (editable)", key=k_obj, height=125)

    completos_1 = all(str(v or "").strip() for v in [xerencia, cif, nombre, telefono, direccion, zona,
                                                      datos_centro.get("superficie_construida"), datos_centro.get("superficie_util")])
    with _acordeon_informe("1", "DATOS DE LA EMPRESA Y DEL CENTRO DE TRABAJO", completos_1):
        def _guardar_superficie_is47(campo, key_widget):
            set_datos_informe_centro(cid, **{campo: st.session_state.get(key_widget, "")})

        c1, c2 = st.columns(2)
        with c1:
            st.text_input("Gerencia / Empresa", value=xerencia, disabled=True, key=f"is47_xerencia_{cid}")
            st.text_input("Centro", value=nombre or "", disabled=True, key=f"is47_centro_{cid}")
            st.text_input("Dirección (domicilio, municipio, provincia, código postal)", value=direccion or "", disabled=True, key=f"is47_enderezo_{cid}")
            superficie_construida_is47 = st.text_input(
                "Superficie construida (m²)",
                value=datos_centro.get("superficie_construida", ""),
                key=f"is47_supc_{cid}",
                on_change=_guardar_superficie_is47,
                args=("superficie_construida", f"is47_supc_{cid}"),
            )
        with c2:
            st.text_input("CIF", value=cif, disabled=True, key=f"is47_cif_{cid}")
            st.text_input("Teléfono", value=telefono, disabled=True, key=f"is47_tel_{cid}")
            st.text_input("Servicio/Unidad muestreada", value=zona or "", disabled=True, key=f"is47_servizo_{cid}")
            superficie_util_is47 = st.text_input(
                "Superficie útil (m²)",
                value=datos_centro.get("superficie_util", ""),
                key=f"is47_supu_{cid}",
                on_change=_guardar_superficie_is47,
                args=("superficie_util", f"is47_supu_{cid}"),
            )

        datos_centro["superficie_construida"] = superficie_construida_is47
        datos_centro["superficie_util"] = superficie_util_is47
        st.caption("Las superficies pueden editarse aquí; el resto de datos se modifica en Datos del centro y Datos de la empresa.")

    # -----------------------------------------------------------------
    # 2. TRABALLADORES/AS — mismo criterio que el punto 3 del SSCC.
    # -----------------------------------------------------------------
    fecha_com = str(datos_centro.get("fecha_comunicacion_trab", "") or "").strip()
    medio_com = str(datos_centro.get("medio_comunicacion", "") or "").strip()
    completo_2 = bool(str(total_trab or "0").strip() not in ("", "0"))
    with _acordeon_informe("2", "INFORMACIÓN SOBRE LOS/LAS TRABAJADORES/AS", completo_2):
        # Comienzo específico del modelo IS-47.
        st.markdown("**Ocupación por sala/categoría/turno:**")
        col3, col4 = st.columns(2)
        with col3:
            st.markdown("**Puestos de trabajo por sala** (una línea por puesto)")
            _sincronizar_valor_auto(f"is47_postos_{cid}", "\n".join(postos_bullets_default))
            postos_text = st.text_area(
                "is47_postos_text", label_visibility="collapsed", height=120, key=f"is47_postos_{cid}",
            )
            postos_bullets = [line.strip() for line in postos_text.splitlines() if line.strip()]
            num_traballadores_total = st.text_input(
                "N.º total de trabajadores adscritos", value=str(total_trab or ""), key=f"is47_ntrab_{cid}",
            )
        with col4:
            st.markdown("**Categorías profesionales y turnos** (una línea por categoría)")
            _sincronizar_valor_auto(f"is47_categorias_{cid}", "\n".join(categorias_bullets_default))
            categorias_text = st.text_area(
                "is47_categorias_text", label_visibility="collapsed", height=120, key=f"is47_categorias_{cid}",
            )
            categorias_bullets = [line.strip() for line in categorias_text.splitlines() if line.strip()]

            def _guardar_dato_is47(campo, key_widget):
                set_datos_informe_centro(cid, **{campo: st.session_state.get(key_widget, "")})

            fecha_com = st.text_input(
                "Fecha de comunicación a los trabajadores", value=fecha_com,
                key=f"is47_data_info_trab_{cid}",
                on_change=_guardar_dato_is47, args=("fecha_comunicacion_trab", f"is47_data_info_trab_{cid}"),
            )
            medio_com = st.text_input(
                "Medio de comunicación", value=medio_com or "correo electrónico",
                key=f"is47_medio_info_{cid}",
                on_change=_guardar_dato_is47, args=("medio_comunicacion", f"is47_medio_info_{cid}"),
            )

        _ctx_is47_p2 = ReportContext(
            centro=str(nombre or ""), servizo_unidade=str(zona or ""), postos_bullets=postos_bullets,
            num_traballadores_total=num_traballadores_total, categorias_bullets=categorias_bullets,
            medio_informacion_traballadores=medio_com,
            data_informacion_traballadores=fecha_com, tipo_zona="",
        )
        try:
            _texto_sscc_p3 = generar_texto_punto3_automatico(_ctx_is47_p2)
        except Exception:
            _texto_sscc_p3 = ""
        _texto_p2_auto = "Ocupación por sala/categoría/turno:\n" + str(_texto_sscc_p3 or "").strip()
        _sincronizar_valor_auto(f"is47_punto2_{cid}", _texto_p2_auto)
        st.markdown("**Punto 2 completo del informe**")
        texto_punto2_manual = st.text_area(
            "is47_punto2_texto", label_visibility="collapsed", height=220, key=f"is47_punto2_{cid}",
        )
        punto2_es_auto = st.session_state.get(f"is47_punto2_{cid}__ultimo_auto") == texto_punto2_manual
        st.caption("Las líneas que empiecen por '-' se mostrarán como viñetas en el informe.")
        if variante == "B":
            st.markdown("**Vista previa de la variante B:**")

            def _separar_previa_b(texto):
                texto = str(texto or "").strip()
                if texto.startswith("-"):
                    texto = texto[1:].strip()
                if ":" in texto:
            ...
            def _separar_previa_categoria_b(texto):
                return _is47_separar_categoria_numero_horarios(texto)

            _filas_postos = [_separar_previa_b(x) for x in postos_bullets]
            _prev_salas = pd.DataFrame(_filas_postos or [("", "")], columns=["Sala", "Profesionales y turnos"])
            st.markdown("**Salas y profesionales**")
            st.dataframe(_prev_salas, hide_index=True, use_container_width=True)

            st.markdown(
                "El personal adscrito al conjunto del centro dividido por categorías es el siguiente;"
            )
            st.markdown("**Categorías profesionales**")
            for _cat_b in categorias_bullets:
                _cat_txt = str(_cat_b or "").strip().lstrip("-").strip()
                if _cat_txt:
                    st.markdown(f"- {_cat_txt}")

    texto_planos = (
        "O esquema gráfico do edificio e planos de cada planta móstranse no Anexo I, onde se indican as zonas de mostraxe, "
        "e as localizacións dos detectores, xunto co código de cada zona de mostraxe e o código do detector correspondente."
    )
    with _acordeon_informe("3", "PLANOS", bool(fetch_planos_centro(cid))):
        st.write(texto_planos)
        planos = fetch_planos_centro(cid)
        st.caption(f"Planos registrados en la app: {len(planos)}")

    _num_formularios_is47 = len(detectores)
    if _num_formularios_is47 == 1:
        texto_cond = (
            "No formulario de toma de datos do Anexo II recóllese o tempo de exposición do detector "
            "(data e hora de inicio e finalización da exposición), a persoa responsable da súa colocación e retirada, "
            "e as posibles anomalías detectadas."
        )
    else:
        texto_cond = (
            "Nos formularios de toma de datos do Anexo II recóllense os tempos de exposición dos detectores "
            "(data e hora de inicio e finalización da exposición), as persoas responsables da súa colocación e retirada, "
            "e as posibles anomalías detectadas."
        )
    with _acordeon_informe("4", "CONDICIONES DE LA EXPOSICIÓN", bool(detectores)):
        st.write(texto_cond)
        st.caption("Los datos de colocación y retirada se toman de las fichas de los detectores.")

    df_res = _is47_resultados_dataframe(detectores, categorias)
    pendientes = int(df_res["Concentración radon (Bq/m³)"].isna().sum()) if not df_res.empty else 0
    completo_5 = bool(not df_res.empty and pendientes == 0 and laboratorio)
    with _acordeon_informe("5", "RESUMEN DE RESULTADOS", completo_5):
        st.write("**Detectores utilizados:** DETECTORES DE TRAZAS NUCLEARES EN ESTADO SÓLIDO (SSNTD) DE LONGA DURACIÓN")
        st.write("**TIPO/MODELO:** PADC/CR-39 – RSKS. **FABRICANTE:** RADOSYS-HUNGRÍA")
        st.write(f"**Laboratorio de medición:** {laboratorio or 'Pendiente de indicar en Datos de la empresa'}")
        _datos_k_is47 = get_datos_informe_centro(cid)
        _k_guardado_is47 = _normalizar_factor_k(_datos_k_is47.get("factor_k", ""))
        _k_key_is47 = f"factor_k_{cid}"
        _k_pendiente_is47 = st.session_state.pop(f"_factor_k_pendiente_{cid}", None)
        if _k_pendiente_is47 is not None:
            st.session_state[_k_key_is47] = _k_pendiente_is47
        elif _k_key_is47 not in st.session_state:
            st.session_state[_k_key_is47] = _k_guardado_is47
        factor_k = st.text_input(
            "Valor de k",
            key=_k_key_is47,
            help="Factor de cobertura de la incertidumbre expandida. Se completa automáticamente al leer un PDF de laboratorio compatible y también puede modificarse manualmente.",
        )
        factor_k = _normalizar_factor_k(factor_k)
        if factor_k != _k_guardado_is47:
            set_datos_informe_centro(cid, factor_k=factor_k)

        resultados_pdf_is47 = st.file_uploader(
            "Informe de resultados del laboratorio (PDF)",
            type=["pdf"],
            key=f"is47_resultados_pdf_{cid}",
            help="Carga el PDF del laboratorio para importar automáticamente resultado, incertidumbre y valor de k por código de detector.",
        )
        if resultados_pdf_is47 is not None:
            try:
                _pdf_bytes_is47 = resultados_pdf_is47.getvalue()
                _firma_pdf_is47 = hashlib.sha256(_pdf_bytes_is47).hexdigest()
                _firma_key_is47 = f"is47_resultados_pdf_firma_{cid}"

                if st.session_state.get(_firma_key_is47) != _firma_pdf_is47:
                    _resultados_pdf_is47 = extraer_resultados_pdf_laboratorio(
                        io.BytesIO(_pdf_bytes_is47)
                    )
                    if _resultados_pdf_is47.empty:
                        st.warning(
                            "No se encontraron resultados reconocibles en el PDF del laboratorio."
                        )
                    else:
                        _k_pdf_is47 = ""
                        if "K" in _resultados_pdf_is47.columns:
                            _valores_k_is47 = [
                                _normalizar_factor_k(v)
                                for v in _resultados_pdf_is47["K"].tolist()
                                if _normalizar_factor_k(v)
                            ]
                            if _valores_k_is47:
                                _k_pdf_is47 = _valores_k_is47[0]

                        if _k_pdf_is47:
                            factor_k = _k_pdf_is47
                            set_datos_informe_centro(cid, factor_k=_k_pdf_is47)
                            # El widget ya está instanciado en este rerun.
                            # Guardamos el valor para aplicarlo al inicio del siguiente rerun.
                            st.session_state[f"_factor_k_pendiente_{cid}"] = _k_pdf_is47

                        _por_codigo_is47 = {
                            str(r.get("Código", "")).strip(): r
                            for _, r in _resultados_pdf_is47.iterrows()
                            if str(r.get("Código", "")).strip()
                        }
                        _coincidencias_is47 = 0
                        for _det_is47 in fetch_detectores(cid):
                            _codigo_is47 = str(_det_is47.get("codigo", "")).strip()
                            _r_is47 = _por_codigo_is47.get(_codigo_is47)
                            if _r_is47 is None:
                                continue

                            _resultado_is47 = str(_r_is47.get("Resultado", "")).strip()
                            _incertidumbre_is47 = str(_r_is47.get("Incertidumbre", "")).strip()

                            if _resultado_is47 or _incertidumbre_is47:
                                update_detector(
                                    _det_is47["id"],
                                    resultado=_resultado_is47,
                                    incertidumbre=_incertidumbre_is47,
                                )
                                _coincidencias_is47 += 1

                        st.session_state[_firma_key_is47] = _firma_pdf_is47
                        if _coincidencias_is47:
                            st.success(
                                f"Resultados importados para {_coincidencias_is47} detector(es)."
                            )
                            st.rerun()
                        else:
                            st.warning(
                                "El PDF se ha leído, pero no se encontraron códigos de detector coincidentes con este centro."
                            )
            except Exception as _e_pdf_is47:
                st.error(f"No se pudo leer el informe de resultados del laboratorio: {_e_pdf_is47}")

        if df_res.empty:
            st.info("Añade detectores para completar la tabla de resultados.")
        else:
            visibles = [c for c in df_res.columns if c != "ID"]
            editada = st.data_editor(
                df_res[visibles],
                key=f"is47_resultados_{cid}",
                hide_index=True,
                use_container_width=True,
                num_rows="fixed",
                disabled=["Código zona", "Código detector", "Data inicio", "Data fin", "Posto/postos de traballo asociados"],
                column_config={
                    "Concentración radon (Bq/m³)": st.column_config.NumberColumn(min_value=0.0, step=1.0, format="%.0f"),
                    "Incerteza expandida (Bq/m³)": st.column_config.TextColumn(_cabecera_incertidumbre(factor_k, "gl")),
                },
            )
            for i, (_, fila) in enumerate(editada.iterrows()):
                try:
                    did = int(df_res.iloc[i]["ID"])
                    valor = fila["Concentración radon (Bq/m³)"]
                    valor = float(valor) if pd.notna(valor) else None
                    inc = str(fila["Incerteza expandida (Bq/m³)"] or "").strip()
                    det = get_detector(did)
                    if det and (det[20] != valor or str(det[21] or "").strip() != inc):
                        actualizar_resultado_detector(did, valor, inc)
                except Exception:
                    pass
            df_res = editada.copy()
            df_res.insert(0, "ID", [int(v) for v in _is47_resultados_dataframe(_is47_detectores(cid), fetch_categorias_centro(cid))["ID"].tolist()])
            if pendientes:
                st.info(
                    "Queda 1 detector sin resultado."
                    if pendientes == 1 else
                    f"Quedan {pendientes} detectores sin resultado."
                )

    conclusion_auto = _is47_conclusion_automatica(df_res)
    k_conc = f"is47_conclusion_{cid}"
    ultimo_auto = st.session_state.get(k_conc + "__auto")
    if k_conc not in st.session_state or st.session_state.get(k_conc) == ultimo_auto:
        st.session_state[k_conc] = conclusion_auto
    st.session_state[k_conc + "__auto"] = conclusion_auto
    with _acordeon_informe("6", "CONCLUSIONES", bool(st.session_state.get(k_conc, "").strip()) and pendientes == 0):
        st.text_area("Texto de conclusiones (editable)", key=k_conc, height=150)

    with _acordeon_informe("7", "DOCUMENTACIÓN DE REFERENCIA", True):
        for ref in _IS47_REFERENCIAS:
            st.markdown(f"- {ref}")

    # FIRMA: misma selección de técnico que en SSCC.
    tecnico_previo = str(tecnico_centro or "").strip()
    tecnicos_firma = []
    for _nombre_tecnico in [tecnico_previo] + [
        str(_det[22]).strip() for _det in fetch_detectores(cid)
        if len(_det) > 22 and str(_det[22] or "").strip()
    ]:
        if _nombre_tecnico and _nombre_tecnico not in tecnicos_firma:
            tecnicos_firma.append(_nombre_tecnico)

    tecnico_firma_key = f"is47_tecnico_{cid}"
    tecnico_firma_actual = str(st.session_state.get(tecnico_firma_key, tecnico_previo) or "").strip()
    if tecnico_firma_actual and tecnico_firma_actual not in tecnicos_firma:
        tecnicos_firma.append(tecnico_firma_actual)

    with _acordeon_informe("", "FIRMA", bool(tecnico_firma_actual or tecnico_previo)):
        data_informe = st.date_input("Fecha del informe", value=date.today(), key=f"is47_data_informe_{cid}")
        if tecnicos_firma:
            tecnico = st.selectbox(
                "Nombre del/de la técnico/a que firma el informe", tecnicos_firma,
                index=(tecnicos_firma.index(tecnico_firma_actual) if tecnico_firma_actual in tecnicos_firma else 0),
                key=f"is47_selector_tecnico_firma_{cid}",
            )
            st.session_state[tecnico_firma_key] = tecnico
        else:
            tecnico = st.text_input("Nombre del/de la técnico/a que firma el informe", value="", key=tecnico_firma_key)

        _area_firma = re.sub(r"^(XERENCIA|GERENCIA)\s+", "", str(xerencia or "").strip(), flags=re.IGNORECASE)
        if not _area_firma:
            _area_firma = "Área Sanitaria"
        texto_cargo_firma_gl = f"Técnico da UPRL da {_area_firma}"
        texto_cargo_firma_es = f"Técnico de la UPRL del {_area_firma}"
        st.caption(texto_cargo_firma_gl)

    # ---------------------------------------------------------------
    # ANEXOS IS-47
    # El Anexo I reutiliza exactamente el generador y opciones del
    # Anexo II del SSCC, ya que su contenido es el mismo (planos).
    # ---------------------------------------------------------------
    st.markdown("### ANEXOS")
    labels = {
        1: "ANEXO I: ESQUEMA GRÁFICO DO EDIFICO E PLANOS DE CADA PLANTA",
        2: "ANEXO II: FORMULARIOS RECOGIDA DE DATOS",
        3: "ANEXO III: INFORME DE ENSAIO DO LABORATORIO ACREDITADO",
        4: "ANEXO IV: CERTIFICADO ENAC DO LABORATORIO ACREDITADO",
    }
    anexos = {}

    logo_bytes_is47, _ = get_logo_informe()

    for num, label in labels.items():
        with st.container(border=True):
            st.markdown(f"**{label}**")
            if num == 1:
                modo_anexo1 = st.radio(
                    "¿Cómo quieres aportar este anexo?",
                    options=["Generar automáticamente a partir del Excel", "Subir otro plano"],
                    key=f"is47_modo_anexo1_{cid}", horizontal=True, label_visibility="collapsed",
                )
                if modo_anexo1 == "Generar automáticamente a partir del Excel":
                    st.caption(
                        "El plano aprovechará siempre prácticamente toda la hoja. El logotipo y la foto exterior, "
                        "si se seleccionan, se superponen abajo a la derecha: el logotipo abajo y la foto justo encima."
                    )
                    col_logo, col_foto = st.columns(2)
                    with col_logo:
                        mostrar_logo = st.checkbox(
                            "Mostrar logotipo (superpuesto)", value=True, key=f"is47_anexo1_mostrar_logo_{cid}"
                        )
                    with col_foto:
                        mostrar_foto = st.checkbox(
                            "Mostrar foto exterior (superpuesta)", value=True, key=f"is47_anexo1_mostrar_foto_{cid}"
                        )
                    usar_base_alt = st.checkbox(
                        "Utilizar otro plano base", value=False, key=f"is47_anexo1_usar_base_alternativa_{cid}"
                    )
                    bases_alt = {}
                    if usar_base_alt and ruta_excel_is47:
                        st.caption(
                            "Sube un plano base alternativo del mismo tamaño que el original. "
                            "Los puntos y el resto de datos se colocarán sobre esta nueva imagen."
                        )
                        try:
                            _datos_ui = extraer_datos_planos(ruta_excel_is47)
                            for _clave, _info in _datos_ui.get("planos", {}).items():
                                _nombre = _info.get("nombre", _clave)
                                _up_base = st.file_uploader(
                                    f"Plano base alternativo — {_nombre}", type=["png", "jpg", "jpeg"],
                                    key=f"is47_anexo1_base_alt_{cid}_{_clave}",
                                )
                                if _up_base is not None:
                                    bases_alt[_clave] = _up_base.getvalue()
                        except Exception as _e:
                            st.warning(f"No se han podido preparar los planos base alternativos: {_e}")

                    if st.button("🗺️ Generar plano automáticamente", key=f"is47_generar_anexo1_{cid}"):
                        try:
                            if not ruta_excel_is47:
                                raise Anexo2Error("No se ha podido preparar la hoja de datos necesaria para generar los planos.")
                            datos_planos = extraer_datos_planos(ruta_excel_is47)
                            if usar_base_alt:
                                from PIL import Image as _PILImage
                                for _clave, _bytes in bases_alt.items():
                                    if _clave not in datos_planos.get("planos", {}):
                                        continue
                                    _orig = datos_planos["planos"][_clave]["imagen"]
                                    with _PILImage.open(io.BytesIO(_orig)) as _im_orig:
                                        _tam_orig = _im_orig.size
                                    with _PILImage.open(io.BytesIO(_bytes)) as _im_new:
                                        _tam_new = _im_new.size
                                    if _tam_new != _tam_orig:
                                        raise Anexo2Error(
                                            f"El plano base alternativo «{datos_planos['planos'][_clave].get('nombre', _clave)}» "
                                            f"tiene un tamaño de {_tam_new[0]}×{_tam_new[1]} px, pero el original es de "
                                            f"{_tam_orig[0]}×{_tam_orig[1]} px. Deben tener exactamente el mismo tamaño."
                                        )
                                    datos_planos["planos"][_clave]["imagen"] = _bytes

                            anexo1_docx = generar_documento_anexo2(
                                datos_planos,
                                logo_bytes=logo_bytes_is47,
                                centro=nombre or "",
                                mostrar_logo=mostrar_logo,
                                mostrar_foto_exterior=mostrar_foto,
                                superficie_construida=datos_centro.get("superficie_construida", ""),
                                superficie_util=datos_centro.get("superficie_util", ""),
                            )
                            nombre_generado = f"ANEXO_I_planos_{nombre or 'centro'}.docx"
                            st.session_state[f"is47_anexo1_guardado_{cid}"] = (nombre_generado, anexo1_docx)

                            miniaturas = []
                            for _clave, _info in datos_planos.get("planos", {}).items():
                                _puntos = datos_planos.get("puntos", {}).get(_clave, [])
                                try:
                                    miniaturas.append((
                                        _info.get("nombre", _clave),
                                        componer_plano(
                                            _info["imagen"], _puntos,
                                            logo_bytes=logo_bytes_is47,
                                            foto_exterior=datos_planos.get("foto_exterior"),
                                            mostrar_logo=mostrar_logo,
                                            mostrar_foto_exterior=mostrar_foto,
                                    ...
                                    pass
                            st.session_state[f"is47_anexo1_miniaturas_{cid}"] = miniaturas
                    ...
                    if st.session_state.get(f"is47_anexo1_guardado_{cid}"):
                        anexos[1] = st.session_state[f"is47_anexo1_guardado_{cid}"]
                        _nom, _ = anexos[1]
                        st.caption(f"📎 Plano disponible: **{_nom}**")
                        for _nom_mini, _img_mini in st.session_state.get(f"is47_anexo1_miniaturas_{cid}", []):
                            try:
                                if not _miniatura_ampliable(_img_mini, ancho=200, caption=_nom_mini, alt=_nom_mini):
                                    st.image(_img_mini, width=200, caption=_nom_mini)
                            except Exception:
                                st.image(_img_mini, width=200, caption=_nom_mini)
                else:
                    st.session_state.pop(f"is47_anexo1_guardado_{cid}", None)
                    st.session_state.pop(f"is47_anexo1_miniaturas_{cid}", None)
                    up = st.file_uploader(
                        "Subir documento", type=["pdf", "doc", "docx", "png", "jpg", "jpeg"],
                        key=f"is47_anexo_1_{cid}", label_visibility="collapsed"
                    )
                    if up is not None:
                        anexos[1] = (up.name, up.getvalue())
                continue

            if num in (3, 4):
                _default_fn = anexo3_por_defecto if num == 3 else anexo4_por_defecto
                _valor_defecto = _default_fn()
                _anexo_val = _widget_archivo_con_eliminar(
                    f"is47_anexo{num}_{cid}", "Sube el archivo",
                    ["pdf", "doc", "docx", "jpg", "jpeg", "png"],
                    valor_por_defecto=_valor_defecto,
                )
                if _anexo_val is not None:
                    anexos[num] = _anexo_val
            else:
                _anexo_val = _widget_archivo_con_eliminar(
                    f"is47_anexo{num}_{cid}", "Sube el archivo",
                    ["pdf", "doc", "docx", "jpg", "jpeg", "png"],
    ...
    with _acordeon_informe("", "GENERACIÓN DEL INFORME", True):
        generar_gallego = st.checkbox(
            "Xerar o informe en galego", value=True, key=f"is47_gallego_{cid}"
        )
        generar_castellano = st.checkbox(
            "Generar el informe en castellano", value=False, key=f"is47_castellano_{cid}"
        )
        generar_word = st.checkbox(
            "Generar el informe en formato WORD", value=False, key=f"is47_word_opcion_{cid}"
        )
        pdf_disponible = libreoffice_disponible()
        generar_pdf = st.checkbox(
            "Generar el informe en formato PDF", value=True,
            disabled=not pdf_disponible, key=f"is47_pdf_opcion_{cid}",
        )
        incluir_anexos_pdf = st.checkbox(
            "Incluir anexos (Solo en PDF)", value=True, key=f"is47_anexos_pdf_{cid}"
        )
        generar_zip = st.checkbox(
            "Generar ZIP con anexos", value=False, key=f"is47_zip_opcion_{cid}"
        )
        if not pdf_disponible:
            st.caption("⚠️ LibreOffice no está disponible en este entorno; no se pueden generar documentos PDF.")

        if st.button("📄 Generar informe", type="primary", use_container_width=True, key=f"is47_generar_{cid}"):
            try:
                datos_gl = {
                    "xerencia": xerencia, "cif": cif, "centro": nombre or "", "telefono": telefono,
                    "enderezo": direccion or "", "servizo": zona or "",
                    "superficie_construida": datos_centro.get("superficie_construida", ""),
                    "superficie_util": datos_centro.get("superficie_util", ""),
                    "num_traballadores": num_traballadores_total,
                    "texto_punto2": texto_punto2_manual,
                    "postos_bullets": postos_bullets,
                    "categorias_bullets": categorias_bullets,
                    "texto_info_traballadores": (
                        "Os traballadores/as da área afectada foron informados da realización das medicións e da súa finalidade"
                        + (f" a través de {medio_com}" if medio_com else "")
                        + (f" o día {fecha_com}" if fecha_com else "") + "."
                    ),
                    "variante_is47": variante,
                    "obxecto": st.session_state.get(k_obj, obxecto_auto),
                    "texto_planos": texto_planos, "texto_condicions": texto_cond,
                    "laboratorio": laboratorio, "data_informe": data_informe.strftime("%d/%m/%Y"),
                    "tecnico": tecnico, "texto_cargo_firma": texto_cargo_firma_gl,
                    "factor_k": factor_k,
                }
                anexos_presentes = {n: n in anexos for n in labels}
                conclusion_gl = st.session_state.get(k_conc, conclusion_auto)
                docx_gl = _is47_docx_bytes(datos_gl, ocupacion, df_res, conclusion_gl, anexos_presentes, idioma="gl")
                nombre_base = _nombre_documento(nombre or "centro", "INFORME-IS47")
                nombre_gl = nombre_base + "-gl.docx"
                st.session_state[f"is47_docx_gl_{cid}"] = (docx_gl, nombre_gl)
                docx_para_zip = [(nombre_gl, docx_gl)]
                docx_para_pdf = [("gl", nombre_gl, docx_gl)]

                if generar_castellano:
                    # Igual que SSCC: si el texto del punto 2 sigue siendo automático, se regenera
                    # en castellano con los mismos transformadores de puestos/categorías.
                    # Inicializar siempre estas listas: la variante B las usa para la tabla
                    # aunque el texto completo del punto 2 se haya editado manualmente.
                    postos_es = list(postos_bullets)
                    categorias_es = list(categorias_bullets)
                    texto_p2_es = texto_punto2_manual
                    if punto2_es_auto:
                        if df_center_is47 is not None and categorias_df_is47 is not None:
                            postos_es = postos_traballo_bullets(
                                df_center_is47, traducir_galego=False, es_atencion_primaria=es_ap_centro,
                            )
                            _, categorias_es = categorias_turnos_bullets(
                                categorias_df_is47, traducir_galego=False, es_atencion_primaria=es_ap_centro,
                            )
                        _ctx_es = ReportContext(
                            centro=str(nombre or ""), servizo_unidade=str(zona or ""),
                            postos_bullets=postos_es, num_traballadores_total=num_traballadores_total,
                            categorias_bullets=categorias_es, medio_informacion_traballadores=medio_com,
                            data_informacion_traballadores=fecha_com, tipo_zona="",
                        )
                        try:
                            texto_p2_es = "Ocupación por sala/categoría/turno:\n" + generar_texto_punto3_automatico(_ctx_es, idioma="es").strip()
                        except Exception:
                            pass

                    obxecto_es = (
                        f"El presente informe tiene por objeto recoger los resultados de las mediciones de radón en el {str(nombre or '').upper()}, "
                        "para que, cuando la exposición a radón sea superior al nivel de referencia, se establezcan las medidas de remediación "
                        "necesarias para disminuir o eliminar dichos niveles."
                    )
                    texto_planos_es = (
                        "El esquema gráfico del edificio y los planos de cada planta se muestran en el Anexo I, donde se indican las zonas de muestreo "
                        "y las ubicaciones de los detectores, junto con el código de cada zona de muestreo y el código del detector correspondiente."
                    )
                    if _num_formularios_is47 == 1:
                        texto_cond_es = (
                            "En el formulario de toma de datos del Anexo II se recoge el tiempo de exposición del detector "
                            "(fecha y hora de inicio y finalización de la exposición), la persona responsable de su colocación y retirada, "
                            "y las posibles anomalías detectadas."
                        )
                    else:
                        texto_cond_es = (
                            "En los formularios de toma de datos del Anexo II se recogen los tiempos de exposición de los detectores "
                            "(fecha y hora de inicio y finalización de la exposición), las personas responsables de su colocación y retirada, "
                            "y las posibles anomalías detectadas."
                        )
                    datos_es = dict(datos_gl)
                    datos_es.update({
                        "texto_punto2": texto_p2_es,
                        "postos_bullets": postos_es,
                        "categorias_bullets": categorias_es,
                        "texto_info_traballadores": (
                            "Los trabajadores/as del área afectada fueron informados de la realización de las mediciones y de su finalidad"
                            + (f" a través de {medio_com}" if medio_com else "")
                            + (f" el día {fecha_com}" if fecha_com else "") + "."
                        ),
                        "variante_is47": variante,
                        "obxecto": obxecto_es,
                        "texto_planos": texto_planos_es, "texto_condicions": texto_cond_es,
                        "texto_cargo_firma": texto_cargo_firma_es,
                    })
                    conclusion_es = _is47_conclusion_automatica(df_res, idioma="es")
                    docx_es = _is47_docx_bytes(datos_es, ocupacion, df_res, conclusion_es, anexos_presentes, idioma="es")
                    nombre_es = nombre_base + "-es.docx"
                    st.session_state[f"is47_docx_es_{cid}"] = (docx_es, nombre_es)
                    docx_para_zip.append((nombre_es, docx_es))
                    docx_para_pdf.append(("es", nombre_es, docx_es))
                else:
                    st.session_state.pop(f"is47_docx_es_{cid}", None)
                    st.session_state.pop(f"is47_pdf_es_{cid}", None)

                anexos_subidos = {n: v for n, v in anexos.items() if v is not None}
                st.session_state.pop(f"is47_zip_gl_{cid}", None)
                st.session_state.pop(f"is47_zip_es_{cid}", None)
                if generar_zip:
                    for idioma_zip, nombre_docx_zip, contenido_docx_zip in docx_para_pdf:
                        if idioma_zip == "gl" and not generar_gallego:
                            continue
                        if idioma_zip == "es" and not generar_castellano:
                            continue
                        zbuf = io.BytesIO()
                        with zipfile.ZipFile(zbuf, "w", zipfile.ZIP_DEFLATED) as zf:
                            zf.writestr(nombre_docx_zip, contenido_docx_zip)
                            for n, (nom, contenido) in anexos_subidos.items():
                                ext = os.path.splitext(nom)[1] or ".pdf"
                                zf.writestr(f"ANEXO_{['','I','II','III','IV'][n]}{ext}", contenido)
                        zip_name = nombre_docx_zip.rsplit(".", 1)[0] + "-word-con-anexos.zip"
                        st.session_state[f"is47_zip_{idioma_zip}_{cid}"] = (zbuf.getvalue(), zip_name)

                if generar_pdf and pdf_disponible:
                    anexos_pdf = [(nom, cont) for nom, cont in anexos_subidos.values()]
                    for idioma_pdf, nombre_docx, contenido_docx in docx_para_pdf:
                        if idioma_pdf == "gl" and not generar_gallego:
                            continue
                        if idioma_pdf == "es" and not generar_castellano:
                            continue
                        try:
                            pdf_solo = construir_pdf_completo(contenido_docx, [])
                            pdf_solo_name = nombre_docx.rsplit(".", 1)[0] + ".pdf"
                            st.session_state[f"is47_pdf_solo_{idioma_pdf}_{cid}"] = (pdf_solo, pdf_solo_name)
                            if incluir_anexos_pdf:
                                pdf_anexos = construir_pdf_completo(contenido_docx, anexos_pdf)
                                pdf_anexos_name = nombre_docx.rsplit(".", 1)[0] + "-con-anexos.pdf"
                                st.session_state[f"is47_pdf_anexos_{idioma_pdf}_{cid}"] = (pdf_anexos, pdf_anexos_name)
                            else:
                                st.session_state.pop(f"is47_pdf_anexos_{idioma_pdf}_{cid}", None)
                        except PdfToolsError as e:
                            st.error(f"No se ha podido generar el PDF ({idioma_pdf}): {e}")
                else:
                    for _idioma in ("gl", "es"):
                        st.session_state.pop(f"is47_pdf_solo_{_idioma}_{cid}", None)
                        st.session_state.pop(f"is47_pdf_anexos_{_idioma}_{cid}", None)

                st.success("Informe IS-47 generado correctamente.")
            except Exception as e:
                st.error(f"No se ha podido generar el informe IS-47: {e}")

        salidas_is47 = []
        if generar_word and generar_gallego:
            salidas_is47.append((f"is47_docx_gl_{cid}", "Informe en formato WORD en gallego",
                                  "application/vnd.openxmlformats-officedocument.wordprocessingml.document"))
        if generar_word and generar_castellano:
            salidas_is47.append((f"is47_docx_es_{cid}", "Informe en formato WORD en castellano",
                                  "application/vnd.openxmlformats-officedocument.wordprocessingml.document"))
        if generar_pdf and generar_gallego:
            salidas_is47.append((f"is47_pdf_solo_gl_{cid}", "Informe en formato PDF en gallego", "application/pdf"))
            if incluir_anexos_pdf:
                salidas_is47.append((f"is47_pdf_anexos_gl_{cid}", "Informe + anexos en formato PDF en gallego", "application/pdf"))
        if generar_pdf and generar_castellano:
            salidas_is47.append((f"is47_pdf_solo_es_{cid}", "Informe en formato PDF en castellano", "application/pdf"))
            if incluir_anexos_pdf:
                salidas_is47.append((f"is47_pdf_anexos_es_{cid}", "Informe + anexos en formato PDF en castellano", "application/pdf"))
        if generar_zip and generar_gallego:
            salidas_is47.append((f"is47_zip_gl_{cid}", "Informe en formato WORD + anexos, en formato ZIP en gallego", "application/zip"))
        if generar_zip and generar_castellano:
            salidas_is47.append((f"is47_zip_es_{cid}", "Informe en formato WORD + anexos, en formato ZIP en castellano", "application/zip"))

        for key, etiqueta, mime in salidas_is47:
            if key in st.session_state:
                contenido, nombre_archivo = st.session_state[key]
                st.download_button(etiqueta, data=contenido, file_name=nombre_archivo, mime=mime,
...
def pantalla_centro_informe_completo():
    ...
    def traducir_es_gl(texto):
        """Traducción ampliada con expresiones compuestas de PRL."""
        original = str(texto or "")

        # Resolver primero las expresiones completas para que la traducción
        # palabra a palabra no deje formas mixtas como "del traballo".
        original = re.sub(
            r"\bMedicina\s+del\s+trabajo\b",
            "Medicina do traballo",
            original,
            flags=re.IGNORECASE,
        )
        original = re.sub(
            r"\bEnfermer[ií]a\s+del\s+trabajo\b",
            "Enfermería do traballo",
            original,
            flags=re.IGNORECASE,
        )

        resultado = _traducir_es_gl_base(original)

        # Normalización final por si el traductor base recibe variantes ya
        # parcialmente traducidas.
        resultado = re.sub(
            r"\bMedicina\s+(?:del|do)\s+(?:trabajo|traballo)\b",
            "Medicina do traballo",
            resultado,
            flags=re.IGNORECASE,
        )
        resultado = re.sub(
            r"\bEnfermer[ií]a\s+(?:del|do)\s+(?:trabajo|traballo)\b",
            "Enfermería do traballo",
            resultado,
            flags=re.IGNORECASE,
        )
    ...
    if st.button("← Guardar y volver"):
        if st.session_state.get(f"ic_hay_cambios_{cid}"):
            with st.spinner("Actualizando la hoja Excel con los últimos cambios..."):
                ...
                ruta_xlsx_auto = guardar_archivo_centro(cid, b"", nombre_xlsx_auto, subdir="informes")
    ...
    # El modelo se guarda por centro. El SSCC conserva íntegramente el informe
    # actual; los demás modelos se incorporan como ramas independientes para
    # que puedan tener apartados, campos y anexos distintos sin afectar al SSCC.
    _modelos_informe = {
        "sscc": "Modelo SSCC",
        "is47": "Modelo IS-47",
        "is47b": "Modelo IS-47 (B)",
    }
    _modelo_actual = get_modelo_informe(cid)
    _opciones_modelo = list(_modelos_informe.keys())
    _modelo_elegido = st.selectbox(
        "Modelo de informe",
        options=_opciones_modelo,
        index=_opciones_modelo.index(_modelo_actual) if _modelo_actual in _opciones_modelo else 0,
        format_func=lambda x: _modelos_informe[x],
        key=f"selector_modelo_informe_{cid}",
        help="Cada modelo puede disponer de apartados y anexos propios.",
    )
    if _modelo_elegido != _modelo_actual:
        set_modelo_informe(cid, _modelo_elegido)
        _modelo_actual = _modelo_elegido

    if _modelo_actual in {"is47", "is47b"}:
        _pantalla_informe_is47(cid, centro, variante="B" if _modelo_actual == "is47b" else "A")
        return

    st.caption("Modelo activo: SSCC")
    ...
    ruta_excel = guardar_archivo_centro(cid, b"", f"_tmp_informe_completo_{cid}.xlsx", subdir="informes")
    try:
    ...
    with _acordeon_informe("1", "IDENTIFICACIÓN DEL CENTRO DE TRABAJO", completo_1):
        ...
        with col1:
            ...
            if _match_zona_directa:
                ...
                servizo_unidade_efectivo = servizo_unidade
            enderezo = st.text_input("Dirección (domicilio, municipio, provincia, código postal)", value=enderezo_previo, key=f"ic_enderezo_{cid}")
        with col2:
    ...
    with _acordeon_informe("6", "RESULTADO DE LAS MEDICIONES REALIZADAS", completo_6):
        _datos_k_sscc = get_datos_informe_centro(cid)
        _k_guardado_sscc = _normalizar_factor_k(_datos_k_sscc.get("factor_k", ""))
        _k_key_sscc = f"factor_k_{cid}"
        _k_pendiente_sscc = st.session_state.pop(f"_factor_k_pendiente_{cid}", None)
        if _k_pendiente_sscc is not None:
            st.session_state[_k_key_sscc] = _k_pendiente_sscc
        elif _k_key_sscc not in st.session_state:
            st.session_state[_k_key_sscc] = _k_guardado_sscc
        factor_k = st.text_input(
            "Valor de k",
            key=_k_key_sscc,
            help="Factor de cobertura de la incertidumbre expandida. Si el PDF del laboratorio contiene k=..., este campo se completa automáticamente. También puede modificarse manualmente.",
        )
        factor_k = _normalizar_factor_k(factor_k)
        if factor_k != _k_guardado_sscc:
            set_datos_informe_centro(cid, factor_k=factor_k)
        ...
        st.markdown(
            '<div class="marcador-uploader-resultados-p6"></div>',
            unsafe_allow_html=True,
        )
        ...
        if resultados_pdf_file:
            try:
                ...
                if resultados_pdf_df.empty:
                    ...
                    _k_pdf = ""
                    if "K" in resultados_pdf_df.columns:
                        _valores_k_pdf = [
                            _normalizar_factor_k(v)
                            for v in resultados_pdf_df["K"].tolist()
                            if _normalizar_factor_k(v)
                        ]
                        if _valores_k_pdf:
                            _k_pdf = _valores_k_pdf[0]

                    _firma_k_pdf = f"{resultados_pdf_file.name}_{resultados_pdf_file.size}"
                    if _k_pdf and st.session_state.get(f"ic_factor_k_pdf_fid_{cid}") != _firma_k_pdf:
                        st.session_state[f"ic_factor_k_pdf_fid_{cid}"] = _firma_k_pdf
                        factor_k = _k_pdf
                        set_datos_informe_centro(cid, factor_k=_k_pdf)
                        # El widget ya está instanciado; aplicar en el siguiente rerun.
                        st.session_state[f"_factor_k_pendiente_{cid}"] = _k_pdf
        ...
        # Punto 6: editor en formato de tabla real, con una única cuadrícula continua.
        # Las columnas informativas quedan bloqueadas y únicamente se pueden editar
        # Resultado e Incertidumbre, como celdas de una hoja de cálculo.
        columnas_visibles = [
            "Código zona", "Código detector", "Fecha inicio", "Fecha fin",
            "Sala / Puestos", "Resultado Bq/m³", "Incertidumbre",
        ]
        tabla_para_editor = tabla_resultados[columnas_visibles].copy()

        # st.data_editor se renderiza en un canvas; por eso el fondo y el tamaño
        # de letra se controlan mediante el tema de Streamlit definido antes
        # del import. Se conserva exactamente el mismo tipo de celdas/editor.

        tabla_editada_visible = st.data_editor(
            tabla_para_editor,
            key=editor_key,
            hide_index=True,
            use_container_width=True,
            ...
            row_height=38,
            disabled=[
                "Código zona", "Código detector", "Fecha inicio",
                "Fecha fin", "Sala / Puestos",
            ],
            column_config={
                "Código zona": st.column_config.TextColumn("Código zona", width="small"),
                "Código detector": st.column_config.TextColumn("Código detector", width="small"),
                "Fecha inicio": st.column_config.TextColumn("Fecha inicio", width="small"),
                "Fecha fin": st.column_config.TextColumn("Fecha fin", width="small"),
                "Sala / Puestos": st.column_config.TextColumn("Sala / Puestos", width="large"),
                "Resultado Bq/m³": st.column_config.NumberColumn(
                    "Resultado Bq/m³", min_value=0.0, step=1.0, format="%.0f", width="small", required=True
                ),
                "Incertidumbre": st.column_config.TextColumn("Incertidumbre", width="small", required=True),
            },
        )

        # Recuperamos el ID oculto para conservar el guardado en SQLite sin
        # mostrar una columna técnica al usuario.
        tabla_editada = tabla_editada_visible.copy()
        tabla_editada.insert(0, "ID", tabla_resultados["ID"].values)
    ...
    tecnico_previo = str(tecnico_centro or det_meta.get("Técnico", "") or "").strip()

    # Para la firma del informe se ofrecen tanto el técnico original del centro
    # como los técnicos distintos que hayan realizado alguna retirada.
    tecnicos_firma = []
    for _nombre_tecnico in [tecnico_previo] + [
        str(_det[22]).strip()
        for _det in fetch_detectores(cid)
        if len(_det) > 22 and str(_det[22] or "").strip()
    ]:
        if _nombre_tecnico and _nombre_tecnico not in tecnicos_firma:
            tecnicos_firma.append(_nombre_tecnico)

    tecnico_firma_key = f"ic_tecnico_{cid}"
    tecnico_firma_actual = str(
        st.session_state.get(tecnico_firma_key, tecnico_previo) or ""
    ).strip()
    if tecnico_firma_actual and tecnico_firma_actual not in tecnicos_firma:
        tecnicos_firma.append(tecnico_firma_actual)

    completo_8 = bool(tecnico_firma_actual or tecnico_previo)
    with _acordeon_informe("8", "FIRMA", completo_8):
        if tecnicos_firma:
            tecnico_nome = st.selectbox(
                ...
                tecnicos_firma,
                index=(
                    tecnicos_firma.index(tecnico_firma_actual)
                    if tecnico_firma_actual in tecnicos_firma
                    else 0
                ),
                key=f"ic_selector_tecnico_firma_{cid}",
            )
            st.session_state[tecnico_firma_key] = tecnico_nome
        else:
            tecnico_nome = st.text_input(
                "Nombre del/de la técnico/a que firma el informe",
                value="",
                key=tecnico_firma_key,
            )
            ...
            with st.container(border=True):
                ...
                if key == "anexo2":
                    ...
                    if modo_anexo2 == "Generar automáticamente a partir del Excel":
                        ...
                        # Opcionalmente se puede sustituir la imagen base de cada
                        # plano por otra imagen del MISMO tamaño en píxeles. Se conservan
                        # los puntos, coordenadas, textos, superficies, foto y logotipo:
                        # únicamente cambia el fondo sobre el que se compone el plano.
                        usar_plano_base_alternativo = st.checkbox(
                            "Utilizar otro plano base",
                            value=False,
                            key=f"ic_anexo2_usar_base_alternativa_{cid}",
                        )
                        bases_alternativas_anexo2 = {}
                        if usar_plano_base_alternativo:
                            st.caption(
                                "Sube un plano base alternativo del mismo tamaño que el original. "
                                "Los puntos y el resto de datos se colocarán sobre esta nueva imagen."
                            )
                            try:
                                _datos_planos_ui = extraer_datos_planos(ruta_excel)
                                for _clave_base, _info_base in _datos_planos_ui.get("planos", {}).items():
                                    _nombre_base = _info_base.get("nombre", _clave_base)
                                    _archivo_base = st.file_uploader(
                                        f"Plano base alternativo — {_nombre_base}",
                                        type=["png", "jpg", "jpeg"],
                                        key=f"ic_anexo2_base_alt_{cid}_{_clave_base}",
                                    )
                                    if _archivo_base is not None:
                                        bases_alternativas_anexo2[_clave_base] = _archivo_base.getvalue()
                            except Exception as _e_base_ui:
                                st.warning(
                                    "No se han podido preparar los planos base alternativos: "
                                    f"{_e_base_ui}"
                                )
                        ...
                        if st.button("🗺️ Generar plano automáticamente", key=f"ic_generar_anexo2_{cid}"):
                            try:
                                ...
                                # Si se ha aportado un fondo alternativo, comprobamos que
                                # tenga exactamente las mismas dimensiones que el original.
                                # De este modo las coordenadas normalizadas de los detectores
                                # siguen correspondiendo al mismo punto del plano.
                                if usar_plano_base_alternativo:
                                    from PIL import Image as _PILImage
                                    for _clave_base, _bytes_base in bases_alternativas_anexo2.items():
                                        if _clave_base not in datos_planos.get("planos", {}):
                                            continue
                                        _imagen_original = datos_planos["planos"][_clave_base]["imagen"]
                                        with _PILImage.open(io.BytesIO(_imagen_original)) as _im_original:
                                            _tam_original = _im_original.size
                                        with _PILImage.open(io.BytesIO(_bytes_base)) as _im_nueva:
                                            _tam_nueva = _im_nueva.size
                                        if _tam_nueva != _tam_original:
                                            raise Anexo2Error(
                                                "El plano base alternativo "
                                                f"«{datos_planos['planos'][_clave_base].get('nombre', _clave_base)}» "
                                                f"tiene un tamaño de {_tam_nueva[0]}×{_tam_nueva[1]} px, "
                                                f"pero el original es de {_tam_original[0]}×{_tam_original[1]} px. "
                                                "Deben tener exactamente el mismo tamaño."
                                            )
                                        datos_planos["planos"][_clave_base]["imagen"] = _bytes_base
                        ...
                        if st.session_state.get(f"ic_anexo2_guardado_{cid}"):
                            ...
                            for nombre_plano_mini, imagen_mini in st.session_state.get(f"ic_anexo2_miniaturas_{cid}", []):
                                if not _miniatura_ampliable(
                                    imagen_mini, ancho=200, caption=nombre_plano_mini,
                                    alt=nombre_plano_mini
                                ):
    ...
    with _acordeon_informe("", "GENERACIÓN DEL INFORME", True):
        ...
        generar_gallego = st.checkbox(
            "Xerar o informe en galego", value=True, key=f"ic_gallego_{cid}"
        )
        generar_castellano = st.checkbox(
            "Generar el informe en castellano", value=False, key=f"ic_castellano_{cid}"
        )
        generar_word = st.checkbox(
            "Generar el informe en formato WORD", value=False, key=f"ic_word_opcion_{cid}"
        )
        pdf_disponible = libreoffice_disponible()
        generar_pdf = st.checkbox(
            "Generar el informe en formato PDF", value=True,
            disabled=not pdf_disponible, key=f"ic_pdf_opcion_{cid}",
        )
        incluir_anexos_pdf = st.checkbox(
            "Incluir anexos (Solo en PDF)", value=True, key=f"ic_anexos_pdf_{cid}"
        )
        generar_zip = st.checkbox(
            "Generar ZIP con anexos", value=False, key=f"ic_zip_opcion_{cid}"
        )
        if not pdf_disponible:
            st.caption("⚠️ LibreOffice no está disponible en este entorno; no se pueden generar documentos PDF.")
        ...
        if st.button("📄 Generar informe", type="primary", use_container_width=True, key=f"ic_generar_{cid}"):
            try:
                ...
                # La tabla de resultados del informe debe mostrar también
                # los nombres de las salas traducidos al gallego.
                df_final_gl = df_final.copy()
                if "Sala" in df_final_gl.columns:
                    df_final_gl["Sala"] = df_final_gl["Sala"].apply(
                        lambda v: traducir_es_gl(v) if str(v or "").strip() else v
                    )

                buffer_gl = generate_report(ctx_gl, df_final_gl, idioma="gl")

                # En el punto 3 el texto gallego contiene los nombres ya
                # traducidos; por eso la negrita debe buscar esas variantes,
                # no únicamente los nombres originales en castellano.
                salas_gl = [traducir_es_gl(s) for s in salas]
                buffer_gl = io.BytesIO(
                    _poner_salas_negrita_punto3(
                        buffer_gl.getvalue(),
                        list(salas) + salas_gl,
                    )
                )
                buffer_gl = io.BytesIO(
                    _actualizar_cabecera_incertidumbre_docx(
                        buffer_gl.getvalue(), factor_k, "gl"
                    )
                )
                ...
                if generar_castellano:
                    ...
                    buffer_es = io.BytesIO(
                        _actualizar_cabecera_incertidumbre_docx(
                            buffer_es.getvalue(), factor_k, "es"
                        )
                    )
                ...
                st.session_state.pop(f"ic_zip_gl_{cid}", None)
                st.session_state.pop(f"ic_zip_es_{cid}", None)
                if generar_zip:
                    for idioma_zip, nombre_docx_zip, contenido_docx_zip in docx_buffers_para_pdf:
                        if idioma_zip == "gl" and not generar_gallego:
                            continue
                        if idioma_zip == "es" and not generar_castellano:
                            continue
                        ...
                        nombre_zip_final = nombre_docx_zip.rsplit(".", 1)[0] + "-word-con-anexos.zip"
                        st.session_state[f"ic_zip_{idioma_zip}_{cid}"] = (zip_buffer.getvalue(), nombre_zip_final)
                ...
                if generar_pdf and pdf_disponible:
                    ...
                    for idioma_pdf, nombre_docx, contenido_docx in docx_buffers_para_pdf:
                        ...
                        try:
                            pdf_solo = construir_pdf_completo(contenido_docx, [])
                            pdf_solo_name = nombre_docx.rsplit(".", 1)[0] + ".pdf"
                            st.session_state[f"ic_pdf_solo_{idioma_pdf}_{cid}"] = (pdf_solo, pdf_solo_name)
                            if incluir_anexos_pdf:
                                pdf_anexos = construir_pdf_completo(contenido_docx, anexos_para_pdf)
                                pdf_anexos_name = nombre_docx.rsplit(".", 1)[0] + "-con-anexos.pdf"
                                st.session_state[f"ic_pdf_anexos_{idioma_pdf}_{cid}"] = (pdf_anexos, pdf_anexos_name)
                            else:
                                st.session_state.pop(f"ic_pdf_anexos_{idioma_pdf}_{cid}", None)
                        except PdfToolsError as e:
                            st.error(f"No se ha podido generar el PDF ({idioma_pdf}): {e}")
                else:
                    for _idioma in ("gl", "es"):
                        st.session_state.pop(f"ic_pdf_solo_{_idioma}_{cid}", None)
                        st.session_state.pop(f"ic_pdf_anexos_{_idioma}_{cid}", None)
        ...
        salidas_sscc = []
        if generar_word and generar_gallego:
            salidas_sscc.append((f"ic_report_gl_{cid}", "Informe en formato WORD en gallego",
                                  "application/vnd.openxmlformats-officedocument.wordprocessingml.document"))
        if generar_word and generar_castellano:
            salidas_sscc.append((f"ic_report_es_{cid}", "Informe en formato WORD en castellano",
                                  "application/vnd.openxmlformats-officedocument.wordprocessingml.document"))
        if generar_pdf and generar_gallego:
            salidas_sscc.append((f"ic_pdf_solo_gl_{cid}", "Informe en formato PDF en gallego", "application/pdf"))
            if incluir_anexos_pdf:
                salidas_sscc.append((f"ic_pdf_anexos_gl_{cid}", "Informe + anexos en formato PDF en gallego", "application/pdf"))
        if generar_pdf and generar_castellano:
            salidas_sscc.append((f"ic_pdf_solo_es_{cid}", "Informe en formato PDF en castellano", "application/pdf"))
            if incluir_anexos_pdf:
                salidas_sscc.append((f"ic_pdf_anexos_es_{cid}", "Informe + anexos en formato PDF en castellano", "application/pdf"))
        if generar_zip and generar_gallego:
            salidas_sscc.append((f"ic_zip_gl_{cid}", "Informe en formato WORD + anexos, en formato ZIP en gallego", "application/zip"))
        if generar_zip and generar_castellano:
            salidas_sscc.append((f"ic_zip_es_{cid}", "Informe en formato WORD + anexos, en formato ZIP en castellano", "application/zip"))

        for key_ss, etiqueta, mime in salidas_sscc:
            if key_ss in st.session_state:
                contenido, nombre_archivo = st.session_state[key_ss]
                st.markdown('<div class="marcador-btn-descarga-amarillo"></div>', unsafe_allow_html=True)
                st.download_button(etiqueta, data=contenido, file_name=nombre_archivo, mime=mime,
                                   key=f"dl_{key_ss}", use_container_width=True)


# ============================================================
# PANTALLA: CENTRO (PRINCIPAL)
# ============================================================
...
def pantalla_centro():
    ...
    if not centro:
        st.error("Centro no encontrado.")
        st.markdown('<div class="marcador-btn-azul-claro"></div>', unsafe_allow_html=True)
        if st.button("← Guardar e ir al inicio"):
            st.session_state.view = "inicio"
            st.rerun()
        return
    ...
    if st.button("← Guardar y volver"):
        if _detector_tiene_cambios(ns):
            guardado_id = _guardar_detector_desde_ns(
                cid, detector_id, ns, mostrar_mensajes=True
            )
            if not guardado_id:
                return
...
def pantalla_ajustes():
    ...
    st.markdown('<div class="marcador-btn-azul-claro"></div>', unsafe_allow_html=True)
    volver_clic_ajustes = st.button("← Guardar y volver")
    ...
    if "ajustes_nombre_laboratorio" not in st.session_state:
        st.session_state["ajustes_nombre_laboratorio"] = nombre_laboratorio_actual
    ...
    st.text_input(
        "Nombre del laboratorio de análisis",
        key="ajustes_nombre_laboratorio",
        help="Nombre del laboratorio encargado del análisis de los detectores de radón.",
    )
    ...
    st.markdown("""
    <style>
    /* Ajuste exclusivo de Datos de la empresa: acerca "Cambiar logotipo" a la imagen. */
    div[data-testid="stElementContainer"]:has(.marcador-logo-cambiar) {
    ...
    """, unsafe_allow_html=True)
    st.markdown('<div class="marcador-logo-cambiar"></div>', unsafe_allow_html=True)
    _widget_archivo_con_eliminar(
        ...
        texto_eliminar="Cambiar logotipo",
    )
    ...
    hay_cambios_ajustes = (
        ...
        or st.session_state["ajustes_nombre_laboratorio"] != (nombre_laboratorio_actual or "")
    )
    ...
    def _guardar_ajustes():
        ...
        set_nombre_laboratorio(st.session_state["ajustes_nombre_laboratorio"].strip())
    ...
    st.markdown("---")
    st.markdown('<p class="subtitulo-amarillo">Mantenimiento y seguridad de los datos</p>', unsafe_allow_html=True)

    col_backup, col_integridad = st.columns(2)
    with col_backup:
        if st.button("💾 Preparar copia de seguridad SQLite", use_container_width=True):
            ruta_backup = crear_backup_db()
            if ruta_backup and os.path.exists(ruta_backup):
                with open(ruta_backup, "rb") as f:
                    st.session_state["backup_db_bytes"] = f.read()
                st.session_state["backup_db_nombre"] = os.path.basename(ruta_backup)
            else:
                st.error("No se pudo crear la copia de seguridad.")
        if st.session_state.get("backup_db_bytes"):
            st.download_button(
                "⬇️ Descargar copia de seguridad",
                data=st.session_state["backup_db_bytes"],
                file_name=st.session_state.get("backup_db_nombre", "radon_data_backup.db"),
                mime="application/octet-stream",
                use_container_width=True,
            )

    with col_integridad:
        if st.button("🔎 Comprobar almacenamiento", use_container_width=True):
            st.session_state["resultado_integridad"] = comprobar_integridad_almacenamiento()

    resultado_integridad = st.session_state.get("resultado_integridad")
    if resultado_integridad is not None:
        faltantes = resultado_integridad.get("faltantes", [])
        huerfanos = resultado_integridad.get("huerfanos", [])
        if not faltantes and not huerfanos:
            st.success("Almacenamiento correcto: no se han detectado incidencias.")
        else:
            if faltantes:
                st.warning(f"Hay {len(faltantes)} archivo(s) registrados en SQLite que no existen en el Volume.")
                with st.expander("Ver archivos que faltan"):
                    for ruta in faltantes:
                        st.code(ruta)
            if huerfanos:
                st.info(f"Hay {len(huerfanos)} archivo(s) físicos sin referencia en SQLite.")
                with st.expander("Ver posibles archivos huérfanos"):
...
def _leer_claves_acceso():
    """
    Lee las claves del archivo «Asacec» situado junto a app.py.

    Formato recomendado (una entrada por línea):
        ADMIN=AB1234KLM9
        Coruña=ZX1111PQ7R

    La contraseña válida son siempre los caracteres 3.º a 6.º de la cadena:
        ADMIN -> 1234
        Coruña -> 1111

    El texto situado a la izquierda de '=' es el identificador estable del propietario.
    Por compatibilidad, si el archivo contiene una sola línea sin '=', esa clave se
    interpreta como la clave de administrador de la versión anterior.
    """
    ...
    for i, linea in enumerate(lineas):
        if "=" in linea:
            ...
            accesos.append({
                "usuario": "__admin__" if es_admin else identificador,
                "clave": clave,
                "admin": es_admin,
            })
        else:
            # Compatibilidad con el fichero antiguo: primera línea = administrador.
            accesos.append({
                "usuario": "__admin__" if i == 0 else f"usuario_{i+1}",
                "clave": linea,
                "admin": i == 0,
            })
...
def _id_dispositivo_login():
    """Identificador estable aproximado del navegador/equipo sin JavaScript.

    Se construye con la IP que recibe Streamlit a través del proxy y el
    User-Agent del navegador. Así la pantalla de acceso no depende de que un
    iframe consiga escribir en localStorage ni de una redirección previa.
    """
    try:
        ...
        # X-Forwarded-For puede contener una cadena de proxies; la primera IP
        # corresponde normalmente al cliente original.
    ...
    # Nunca dejamos la pantalla esperando. Si el proxy no expone IP/UA,
    # usamos una huella de sesión como último recurso.
    base = f"{ip}|{ua}"
    if base == "|":
        if "_login_fallback_id" not in st.session_state:
            st.session_state["_login_fallback_id"] = hashlib.sha256(
                f"{time.time_ns()}|{os.urandom(16).hex()}".encode("utf-8")
            ).hexdigest()
        return "session:" + st.session_state["_login_fallback_id"]

    return "client:" + hashlib.sha256(base.encode("utf-8")).hexdigest()


def _estado_intentos_login(dispositivo):
    ahora = time.time()
    conn = get_db_connection()
    c = conn.cursor()
    c.execute(
        "SELECT intentos, bloqueado_hasta FROM login_intentos WHERE dispositivo=?",
        (dispositivo,),
    )
    row = c.fetchone()
    if not row:
    ...
    intentos = int(row[0] or 0)
    bloqueado_hasta = float(row[1] or 0)
    if bloqueado_hasta and ahora >= bloqueado_hasta:
        c.execute(
            "UPDATE login_intentos SET intentos=0, bloqueado_hasta=0 WHERE dispositivo=?",
            (dispositivo,),
        )
        conn.commit()
        intentos, bloqueado_hasta = 0, 0.0
    conn.close()
    return intentos, bloqueado_hasta


def _registrar_fallo_login(dispositivo):
    ahora = time.time()
    intentos, bloqueado_hasta = _estado_intentos_login(dispositivo)
    if bloqueado_hasta > ahora:
        return 0, bloqueado_hasta

    nuevos = intentos + 1
    nuevo_bloqueo = 0.0
    if nuevos >= 3:
        # Tras el tercer fallo se bloquea 5 minutos y después vuelve a 3 intentos.
        nuevo_bloqueo = ahora + 300
        nuevos = 0

    conn = get_db_connection()
    c = conn.cursor()
    c.execute(
        "INSERT INTO login_intentos (dispositivo, intentos, bloqueado_hasta, ultimo_fallo) "
        "VALUES (?,?,?,?) "
        "ON CONFLICT(dispositivo) DO UPDATE SET "
        "intentos=excluded.intentos, bloqueado_hasta=excluded.bloqueado_hasta, ultimo_fallo=excluded.ultimo_fallo",
        (dispositivo, nuevos, nuevo_bloqueo, ahora),
    )
    conn.commit()
    conn.close()
    return nuevos, nuevo_bloqueo


def _limpiar_intentos_login(dispositivo):
    conn = get_db_connection()
    c = conn.cursor()
    c.execute("DELETE FROM login_intentos WHERE dispositivo=?", (dispositivo,))
    conn.commit()
    conn.close()
...
def pantalla_login():
    ...
    if accesos is None:
        st.error(
            ...
            "en la carpeta de la app (o está vacío)."
        )
        return
    ...
    dispositivo_login = _id_dispositivo_login()

    intentos_login, bloqueado_hasta = _estado_intentos_login(dispositivo_login)
    ahora_login = time.time()
    if bloqueado_hasta > ahora_login:
        segundos = max(1, int(bloqueado_hasta - ahora_login + 0.999))
        minutos, resto = divmod(segundos, 60)
        if minutos:
            tiempo_txt = f"{minutos} min {resto:02d} s"
        else:
            tiempo_txt = f"{resto} s"
        st.error(f"Demasiados intentos incorrectos. Vuelve a intentarlo dentro de {tiempo_txt}.")
        return
    ...
    if enviar_login:
        ...
        if acceso:
            _limpiar_intentos_login(dispositivo_login)
            ...
            intentos_actuales, nuevo_bloqueo = _registrar_fallo_login(dispositivo_login)
            if nuevo_bloqueo > time.time():
                st.error("Clave incorrecta. Has alcanzado el límite de 3 intentos. Acceso bloqueado durante 5 minutos.")
            else:
                restantes = max(0, 3 - intentos_actuales)
                if restantes == 1:
                    st.error("Clave incorrecta. Queda 1 intento antes del bloqueo de 5 minutos.")
                else:
                    st.error(f"Clave incorrecta. Quedan {restantes} intentos antes del bloqueo de 5 minutos.")


def _activar_atras_navegador(view_actual, centro_actual=None):
    """Convierte Atrás del navegador/móvil en el equivalente a «← Guardar y volver»."""
    destinos = {
        "centro": "inicio",
        "centro_datos": "centro",
            ...
            if (!w.__radonBackHandlerV2) {{
              ...
              w.addEventListener("popstate", () => {{
                try {{
                  ...
                  // replace evita crear otra entrada extra durante el propio "Guardar y volver".
...
def main():
    ...
    st.markdown("""
    <style>
    /* Pantalla inicial: el campo de introducción de la clave permanece blanco. */
    ...
    st.markdown("""
    <style>
    /* Color definitivo de los campos vacíos/pending: rojo coral */
    ...
    /* Selectbox: capa interna visible */
    ...
    /* Campos precedidos por marcadores propios de la aplicación */
    .marcador-campo-obligatorio ~ div input,
    .marcador-campo-vacio ~ div input,
    .marcador-campo-rosa ~ div input,
    .marcador-campo-obligatorio ~ div textarea,
    .marcador-campo-vacio ~ div textarea,
    .marcador-campo-rosa ~ div textarea,
    .marcador-campo-obligatorio ~ div div[data-baseweb="select"] > div,
    .marcador-campo-vacio ~ div div[data-baseweb="select"] > div,
    .marcador-campo-rosa ~ div div[data-baseweb="select"] > div {
        background: #FFFFFF !important;
        background-color: #FFFFFF !important;
    }

    /* Placeholder y texto de select vacío en gris oscuro */
    ...
    # Mejora de legibilidad de campos vacíos y desplegables.
    st.markdown("""
    <style>
    /* Texto provisional de desplegables y campos vacíos: gris oscuro. */
    div[data-baseweb="select"] input::placeholder,
    ...
    /* Texto mostrado dentro de selectores cuando todavía no hay selección. */
    div[data-baseweb="select"] span,
    div[data-baseweb="select"] div {
        color: #4f4f4f !important;
    }
    ...
    /*
      Rojo claro para campos vacíos.
      Se aplica sobre los controles que la interfaz presenta sin contenido.
    */
    div[data-baseweb="input"]:has(input:placeholder-shown),
    div[data-baseweb="textarea"]:has(textarea:placeholder-shown),
    .stTextInput input:placeholder-shown,
    .stTextArea textarea:placeholder-shown {
        background-color: #FFFFFF !important;
    }

    /* Selectores sin valor: fondo rojo claro. */
    div[data-baseweb="select"] > div {
        background-color: #FFFFFF;
    }

    /* Mantener el fondo normal cuando Streamlit marca el control como deshabilitado. */
    ...
    st.markdown("""
    <style>
    /* Color definitivo para campos pendientes/sin cubrir: rojo coral. */
    div[data-testid="stTextInput"] input,
    div[data-testid="stTextArea"] textarea,
    div[data-testid="stSelectbox"] div[data-baseweb="select"] > div {
        border-color: #FFFFFF;
    ...
    /* Marcadores propios de la aplicación: máxima prioridad sobre el rosa anterior. */
    div:has(.marcador-campo-obligatorio) input,
    div:has(.marcador-campo-vacio) input,
    div:has(.marcador-campo-obligatorio) textarea,
    div:has(.marcador-campo-vacio) textarea,
    div:has(.marcador-campo-obligatorio) div[data-baseweb="select"] > div,
    div:has(.marcador-campo-vacio) div[data-baseweb="select"] > div,
    div:has(.marcador-campo-rosa) input,
    div:has(.marcador-campo-rosa) div[data-baseweb="select"] > div {
        background-color: #FFFFFF !important;
    }

    /* Texto provisional de selectores vacíos: gris oscuro. */
    div[data-baseweb="select"] input::placeholder,
    div[data-baseweb="select"] span {
        color: #4f4f4f !important;
        -webkit-text-fill-color: #4f4f4f !important;
    ...
    # Crear/verificar el esquema antes del login: el control de intentos usa SQLite.
    init_db()
    ...
    # Sincronización inicial del volumen: solo una vez por sesión autenticada.
    # Evita recorrer y reescribir todos los centros en cada rerun de Streamlit.
    if not st.session_state.get("_sincronizacion_inicial_centros_hecha", False):
        # También corrige carpetas creadas por la v40 (p. ej. coru_a -> Coruña).
        migrar_nombres_carpetas_usuario_anteriores()
        sincronizar_carpetas_y_datos_centros()
        st.session_state["_sincronizacion_inicial_centros_hecha"] = True


    st.markdown("""
    <style>
    /*
      Campos pendientes / sin cubrir.
      Streamlit coloca los marcadores en un stElementContainer separado del widget,
      por eso se selecciona el contenedor que contiene el marcador y su hermano siguiente.
    */
    div[data-testid="stElementContainer"]:has(.marcador-campo-obligatorio)
      + div[data-testid="stElementContainer"] input,
    div[data-testid="stElementContainer"]:has(.marcador-campo-vacio)
      + div[data-testid="stElementContainer"] input,
    div[data-testid="stElementContainer"]:has(.marcador-campo-rosa)
      + div[data-testid="stElementContainer"] input,
    div[data-testid="stElementContainer"]:has(.marcador-campo-obligatorio)
    ...
    div[data-testid="stElementContainer"]:has(.marcador-campo-rosa)
      + div[data-testid="stElementContainer"] textarea,
    div[data-testid="stElementContainer"]:has(.marcador-campo-obligatorio)
      + div[data-testid="stElementContainer"] div[data-baseweb="select"] > div,
    div[data-testid="stElementContainer"]:has(.marcador-campo-vacio)
      + div[data-testid="stElementContainer"] div[data-baseweb="select"] > div,
    div[data-testid="stElementContainer"]:has(.marcador-campo-rosa)
      + div[data-testid="stElementContainer"] div[data-baseweb="select"] > div {
    ...
    /*
      En varias pantallas los marcadores tienen nombres específicos.
      Se cubren todos los marcadores cuyo nombre de clase empieza por "marcador-"
      cuando preceden inmediatamente a un control.
    */
    div[data-testid="stElementContainer"]:has([class*="marcador-"])
      + div[data-testid="stElementContainer"] input:placeholder-shown,
    div[data-testid="stElementContainer"]:has([class*="marcador-"])
      + div[data-testid="stElementContainer"] textarea:placeholder-shown,
    div[data-testid="stElementContainer"]:has([class*="marcador-"])
      + div[data-testid="stElementContainer"] div[data-baseweb="select"] > div {
        background: #FFFFFF !important;
        background-color: #FFFFFF !important;
    ...
    /* Texto de ayuda/placeholder oscuro y visible */
    div[data-baseweb="select"] input::placeholder,
    div[data-baseweb="select"] span,
    input::placeholder,
    textarea::placeholder {
    ...
    migrar_carpetas_centros_a_usuarios()
    ...
    # Mantener actualizado solo el JSON del centro en el que se está trabajando.
    # El fallo de un snapshot auxiliar nunca debe bloquear el uso de la app.
    centro_snapshot = st.session_state.get("centro_actual")
    if centro_snapshot:
        try:
            guardar_snapshot_centro(centro_snapshot)
        except Exception:
            pass
    ...
    # No basta con ocultar centros en la lista: también se bloquea un ID de centro
    ...
    st.markdown("""
    <style>
    /* Ajuste definitivo de los campos marcados como pendientes/vacíos */
    div[data-testid="stElementContainer"]:has([class*="marcador-"])
      + div[data-testid="stElementContainer"] div[data-baseweb="input"],
    div[data-testid="stElementContainer"]:has([class*="marcador-"])
      + div[data-testid="stElementContainer"] div[data-baseweb="textarea"],
    div[data-testid="stElementContainer"]:has([class*="marcador-"])
      + div[data-testid="stElementContainer"] div[data-baseweb="select"] > div {
        background: #FFFFFF !important;
        background-color: #FFFFFF !important;
    ...
    /* También sobre el elemento interior que Streamlit puede pintar en blanco/rosa */
    div[data-testid="stElementContainer"]:has([class*="marcador-"])
      + div[data-testid="stElementContainer"] div[data-baseweb="input"] > div,
    div[data-testid="stElementContainer"]:has([class*="marcador-"])
      + div[data-testid="stElementContainer"] div[data-baseweb="textarea"] > div {
        background: #FFFFFF !important;
        background-color: #FFFFFF !important;
    }

    /* Texto auxiliar visible en gris oscuro */
    div[data-baseweb="select"] input::placeholder,
    div[data-baseweb="select"] span,
    input::placeholder,
    textarea::placeholder {
