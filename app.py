...
def pantalla_centro_informe_completo():
    ...
    with _acordeon_informe("6", "RESULTADO DE LAS MEDICIONES REALIZADAS", completo_6):
        ...
        tabla_editada = st.data_editor(
            ...
            column_config={
                "ID": None,
                "Código zona": st.column_config.TextColumn(
                    "Código zona", width="small"
                ),
                "Código detector": st.column_config.TextColumn(
                    "Código detector", width="small"
                ),
                "Fecha inicio": st.column_config.TextColumn(
                    "Fecha inicio", width="small"
                ),
                "Fecha fin": st.column_config.TextColumn(
                    "Fecha fin", width="small"
                ),
                "Sala / Puestos": st.column_config.TextColumn(
                    "Sala / Puestos", width="large"
                ),
                ...
                "Incertidumbre": st.column_config.TextColumn(
                    "Incertidumbre",
                    help="Incertidumbre expandida y factor de cobertura K",
                    width="small",
                ),
            },
        )
...
def inyectar_traduccion_widgets():
    ...
    (function() {
      ...
      const ROSA_CAMPO_VACIO = "#FFB8A6";

      function aplicarFondoSegunValor(campo) {
        // La clave de acceso debe quedar siempre blanca.
        if (campo.type === "password") {
          campo.style.setProperty("background-color", "#FFFFFF", "important");
          campo.style.setProperty("color", "#000000", "important");
          return;
        }
