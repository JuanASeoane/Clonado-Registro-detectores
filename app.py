...
st.markdown(
    ...
    /* Reducir el gran espacio vacío superior en ordenador */
...
def pantalla_inicio():
    ...
    # Si se acaba de eliminar un centro, limpiamos el estado del
    # desplegable ANTES de volver a crear el widget. Así no queda
    # visualmente escrito el centro eliminado tras el st.rerun().
    if st.session_state.pop("_reset_selector_centro_home", False):
    ...
    if st.session_state.get("confirmar_borrado_centro") == cid_sel:
        ...
        with cc1:
            if st.button("Sí, eliminar", key=f"confirmar_del_{cid_sel}", type="primary"):
                ...
                # En el siguiente rerun se reinicia el selectbox para
                # que desaparezca inmediatamente el nombre eliminado.
                st.session_state["_reset_selector_centro_home"] = True
                st.rerun()
        with cc2:
