...
def _forzar_camara_trasera_streamlit():
    """
    Intenta indicar al navegador móvil que use la cámara trasera en st.camera_input.

    Streamlit no expone actualmente un parámetro facingMode, por lo que se
    añade capture="environment" al input de cámara generado en el DOM.
    Si el navegador no respeta esta preferencia, seguirá mostrando su selector
    habitual de cámara.
    """
    components.html(
        """
        ...
        (() => {
          try {
            ...
            const aplicar = () => {
              // Streamlit suele envolver camera_input en stCameraInput.
              let inputs = Array.from(
                doc.querySelectorAll('[data-testid="stCameraInput"] input[type="file"]')
              );

              // Fallback por si cambia el test-id interno de Streamlit.
              if (!inputs.length) {
                inputs = Array.from(
                  doc.querySelectorAll('input[type="file"][accept*="image"]')
                ).filter(inp => {
                  const cont = inp.closest('[data-testid]') || inp.parentElement;
                  const txt = cont ? (cont.innerText || "") : "";
                  return txt.includes("Fotografía de la etiqueta del detector") ||
                         txt.includes("Take Photo") ||
                         txt.includes("Tomar foto");
                });
              }
              ...
              for (const input of inputs) {
                ...
                // También se asigna la propiedad cuando el navegador la expone.
                try { input.capture = "environment"; } catch (_) {}
              }
            };

            aplicar();

            // Streamlit puede reconstruir el widget unos milisegundos después.
            setTimeout(aplicar, 100);
            setTimeout(aplicar, 350);
            setTimeout(aplicar, 800);
          } catch (e) {
            console.debug("No se pudo aplicar cámara trasera:", e);
          }
        })();
        </script>
        """,
...
def _abrir_ocr_codigo_callback(ns):
    """Abre la cámara limpia, preparada para una nueva lectura."""
    st.session_state[ns + "_ocr_abierto"] = True
    st.session_state.pop(ns + "_ocr_codigo_foto", None)
    st.session_state.pop(ns + "_ocr_codigo_hash", None)
    st.session_state.pop(ns + "_ocr_codigo_mensaje", None)


def _procesar_ocr_codigo_automatico(ns):
    """
    Callback de camera_input. Se ejecuta automáticamente en cuanto cambia
    la fotografía, antes del rerun normal de Streamlit.
    """
    foto_key = ns + "_ocr_codigo_foto"
    hash_key = ns + "_ocr_codigo_hash"
    mensaje_key = ns + "_ocr_codigo_mensaje"

    foto = st.session_state.get(foto_key)
    if foto is None:
        return
    ...
    try:
        ...
        foto_hash = _hashlib.sha1(contenido).hexdigest()
    except Exception as exc:
        st.session_state[mensaje_key] = ("error", f"No se pudo leer la fotografía: {exc}")
        return

    # Evita procesar dos veces exactamente la misma captura.
    if st.session_state.get(hash_key) == foto_hash:
        return

    try:
        codigo = _reconocer_codigo_detector_desde_foto(foto)
        st.session_state[hash_key] = foto_hash
        st.session_state[ns + "_codigo"] = codigo
        st.session_state[mensaje_key] = ("ok", f"Código reconocido: {codigo}")
        # Al quedar False antes del rerun, la cámara desaparece inmediatamente.
        st.session_state[ns + "_ocr_abierto"] = False
    except Exception as exc:
        # Si falla, la cámara queda abierta para repetir la foto.
        st.session_state[hash_key] = foto_hash
        st.session_state[mensaje_key] = ("error", str(exc))



def _renderizar_campos_detector(cid, detector_id, ns):
    if ns + "_ocr_abierto" not in st.session_state:
        st.session_state[ns + "_ocr_abierto"] = False

    c1, c2 = st.columns(2)
    with c1:
        # Reservamos primero el lugar visual del campo. Se rellena después de
        # procesar la cámara, evitando modificar un widget ya creado.
        codigo_placeholder = st.empty()
        ocr_abierto_key = ns + "_ocr_abierto"

        st.button(
            "📷 OCR",
            key=ns + "_ocr_abrir",
            use_container_width=False,
            on_click=_abrir_ocr_codigo_callback,
            args=(ns,),
        )

        if st.session_state[ocr_abierto_key]:
            st.caption(
                "Centra la etiqueta completa en la foto. Se intentará primero leer el código de barras "
                "y, si falla, se utilizará OCR automáticamente."
            )
            st.camera_input(
                "Fotografía de la etiqueta del detector",
                key=ns + "_ocr_codigo_foto",
                on_change=_procesar_ocr_codigo_automatico,
                args=(ns,),
            )
            # Preferencia de cámara trasera en móviles.
            _forzar_camara_trasera_streamlit()
        ...
        if mensaje_ocr:
            ...
            elif st.session_state[ocr_abierto_key]:
