...
def _ocr_codigo_detector_callback(ns):
    """Callback del botón OCR: rellena directamente el campo Código del detector."""
    foto_key = ns + "_ocr_codigo_foto"
    mensaje_key = ns + "_ocr_codigo_mensaje"
    try:
        codigo = _reconocer_codigo_detector_desde_foto(st.session_state.get(foto_key))
...
def _renderizar_campos_detector(cid, detector_id, ns):
    ...
    with c1:
        ...
        if ocr_abierto_key not in st.session_state:
            st.session_state[ocr_abierto_key] = False
        ...
        if st.button(
            ...
            st.session_state[ocr_abierto_key] = not st.session_state[ocr_abierto_key]

        if st.session_state[ocr_abierto_key]:
            st.caption(
                "Haz una foto cercana y enfocada de la etiqueta. "
                "La aplicación buscará automáticamente un código de 2 letras y 4 números."
            )
            st.camera_input(
            ...
            st.button(
                "🔎 Reconocer código",
                key=ns + "_ocr_codigo_boton",
                on_click=_ocr_codigo_detector_callback,
                args=(ns,),
                use_container_width=False,
            )
