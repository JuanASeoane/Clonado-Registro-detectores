...
def widget_seleccionar_plano_y_punto(cid, ns):
    ...
    if IMG_COORD_DISPONIBLE:
        coords = streamlit_image_coordinates(
            img_disp,
            key=(
                ns + "_plano_coords_" + str(plano_id_actual) + "_"
                + str(st.session_state.get("_plano_detector_render_nonce", 0))
            ),
        )
...
def _renderizar_campos_detector(cid, detector_id, ns):
    ...
    with st.expander("Profesionales en esta sala", expanded=False):
        ...
        if not categorias_centro_form:
            st.caption(
                "No hay categorías profesionales específicas creadas para este centro. "
                "Puedes usar «Profesionales (sin especificar)» o crear categorías en «Categorías profesionales»."
            )
        else:
            st.caption(
                "Añade las categorías profesionales presentes en esta sala y su turno de trabajo. "
                "Si no quieres detallarlas una a una, selecciona «Profesionales (sin especificar)»."
            )
...
def pantalla_centro_detectores():
    ...
    def _ejecutar_accion_detector(accion):
        ...
        if tipo == "volver":
            ...
            st.session_state["_plano_detector_render_nonce"] = (
                st.session_state.get("_plano_detector_render_nonce", 0) + 1
            )
        elif tipo == "cambiar":
            ...
            # para que no reutilice la imagen/coordenadas del detector anterior.
            st.session_state["_plano_detector_render_nonce"] = (
                st.session_state.get("_plano_detector_render_nonce", 0) + 1
            )
