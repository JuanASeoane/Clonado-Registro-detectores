...
def _preparar_imagen_ocr_rapida(imagen, max_lado_objetivo):
    """Reduce el tamaño antes del OCR para acelerar mucho la inferencia."""
    ...
    buf = io.BytesIO()
    # JPEG algo más comprimido: menos bytes que transferir al motor y calidad
    # suficiente para caracteres grandes de una etiqueta cercana.
    imagen.save(buf, format="JPEG", quality=86, optimize=False)
    return buf.getvalue()
...
def _reconocer_codigo_detector_desde_foto(foto):
    ...
    Hace primero una pasada rápida con imagen reducida. Solo si falla realiza
    una segunda pasada de mayor resolución. Así la mayoría de las etiquetas
    cercanas se reconocen bastante más rápido.
    """
    if foto is None:
    ...
    # 1ª pasada: rápida. Para una etiqueta fotografiada de cerca suele bastar.
    datos_rapidos = _preparar_imagen_ocr_rapida(imagen, 1100)
    codigo = _buscar_codigo_en_resultado_ocr(motor(datos_rapidos))
    if codigo:
        return codigo

    # 2ª pasada: solo si la rápida no encontró un código válido.
    # Conservamos más detalle pero seguimos evitando mandar fotos enormes.
    datos_detalle = _preparar_imagen_ocr_rapida(imagen, 1700)
...
def _renderizar_campos_detector(cid, detector_id, ns):
    ...
    with c1:
        ...
        if st.session_state[ocr_abierto_key]:
            st.caption(
                "Haz una foto cercana y enfocada de la etiqueta. "
                "Al capturarla, el reconocimiento se realizará automáticamente."
            )
            st.camera_input(
